      ...
      <div className="mt-8">
        ...
        <div className="bg-white/5 p-6 rounded-lg border-l-4 border-green-500">
          <p className="text-lg italic mb-4">
            {/* Si no existe testimonial en el objeto, usamos este mensaje profesional por defecto */}
            "{details.testimonial || "Gracias a la participación ciudadana, estamos transformando este espacio en un pulmón verde para Berlín."}"
          </p>
          <div className="flex items-center gap-4">
            <div className="w-10 h-10 bg-green-500/20 rounded-full flex items-center justify-center">
                <span className="text-sm font-bold">V</span>
            </div>
            {/* Si no existe volunteerName, usamos este nombre por defecto */}
            <p className="font-semibold">{details.volunteerName || "Líder de Proyecto EcoNexo"}</p>
          </div>
        </div>
      </div>
      {/* BOTÓN VOLVER */}
