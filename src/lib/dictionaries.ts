
export type Locale = "en" | "de" | "es" | "fr" | "it" | "pl" | "nl";

type Dict = Record<string, string>;

export const DICTS: Record<Locale, Dict> = {
  en: {
    menu: "Menu",

    app: "EcoNexo",
    map: "Map",
    events: "Events",
    jobs: "Jobs",
    chat: "Chat",
    home: "Home",
    explore: "Explore",
    workshops: "Workshops",
    badges: "Badges",
    forum: "Forum",
    volunteering: "Volunteering",
    profile: "Profile",
    signIn: "Sign In",
    signUp: "Sign Up",
    signOut: "Sign Out",
    logout: "Logout",
    editMode: "Edit Mode",
    profileView: "Profile View",
    achievementsAndBadges: "Achievements & Badges",
    welcomeBack: "Welcome Back!",
    joinEcoNexo: "Join EcoNexo!",
    signInToContinue: "Sign in to continue",
    createAccountToStart: "Create an account to get started",
    checkEmailForLogin: "Check your email for the login link",
    checkEmailForVerification: "Check your email to verify your account",
    continueWithGoogle: "Continue with Google",
    continueWithGitHub: "Continue with GitHub",
    continueWithMicrosoft: "Continue with Microsoft",
    dontHaveAccount: "Don't have an account?",
    alreadyHaveAccount: "Already have an account?",
    signInToAccessProfile: "Sign in to access your profile",
    accessYourProfile: "Access your profile",
    signInToManageProfile: "Sign in to manage your profile",
    whatYouCanDo: "What you can do:",
    managePersonalInfo: "Manage personal information",
    setPreferences: "Set preferences",
    trackActivities: "Track activities",
    connectWithCommunity: "Connect with community",
    myAccount: "My Account",
    myEvents: "My Events",
    myJobs: "My Jobs",
    supportUs: "Support Us",
    aboutUs: "About Us",
    createProfile: "Create Profile",
    email: "Email",
    fullName: "Full Name",
    birthdate: "Birth Date",
    birthPlace: "Birth Place",
    pronouns: "Pronouns",
    gender: "Gender",
    selectPronouns: "Select pronouns",
    pronounHe: "He",
    pronounShe: "She",
    pronounThey: "They",
    heShe: "he/she",
    theyThem: "they/them",
    erSie: "er/sie",
    other: "Other",
    selectGender: "Select gender",
    male: "Male",
    female: "Female",
    nonBinary: "Non-binary",
    preferNotToSay: "Prefer not to say",
    enterEmail: "Enter your email",
    sending: "Sending...",
    loading: "Loading...",
    authenticationRequired: "Authentication Required",
    signInToAccessThisPage: "Sign in to access this page",
    whySignIn: "Why sign in?",
    personalizedExperience: "Personalized experience",
    savePreferences: "Save preferences",
    trackProgress: "Track progress",
    emailNote: "We'll use your email to verify your account",
    continueWith: "Continue with",
    google: "Google",
    outlook: "Outlook",
    checkYourEmail: "Check your email",
    cancel: "Cancel",
    saveChanges: "Save Changes",
    editProfile: "Edit Profile",
    myProfile: "My Profile",
    profileUpdated: "Profile updated successfully",
    changePhoto: "Change Photo",
    savingEs: "Guardando...",
    savingEn: "Saving...",
    savingDe: "Speichern...",
    // Page content
    welcomeMessageTitle: "Connect with sustainable projects in Europe",
    welcomeMessageDescription: "Join the community building a greener future. Discover events, projects and volunteer opportunities near you.",
    exploreEvents: "Explore Events",
    findJobs: "Find Jobs",
    interactiveMap: "Interactive Map",
    showMap: "Show Map",
    hideMap: "Hide Map",
    activeProjects: "Active Projects",

    cities: "Cities",
    connectedVolunteers: "Connected volunteers",
    availableSpots: "Available spots",
    mapSearchPlaceholder: "Search cities, projects, world...",
    mapSearchPlaceholderGlobal: "Search cities, countries, projects, places...",
    searchingGlobally: "Searching globally...",
    featuredProjects: "Featured Projects",
    recommendedForYou: "Recommended for you",
    match: "Match",
    communityProjectsTitle: "Community Projects",
    proximityReason: "Just {{distance}} km from you",
    interestReason: "Matches your interest in {{category}}",
    preferredTimeReason: "In your preferred time ({{time}})",
    difficultyReason: "Appropriate difficulty level",
    noNearbyEvents: "No nearby events found",
    eventsNearby: "Events nearby",
    viewAll: "View all",
    socialMediaDetectionHeader: "Automatic Project Detection",
    interactions: "interactions",
    sustainable: "sustainable",
    approved: "Approve",
    rejected: "Reject",
    viewPost: "View post",
    detectionActive: "Detection active",
    detectionPaused: "Detection paused",
    monitoringSocialMedia: "Monitoring social media for verified environmental projects",
    scanningSocialMedia: "Scanning social media...",
    noNewProjects: "No new projects found",
    scanning: "Scanning",
    scan: "Scan",
    detectionSettings: "Detection Settings",
    minFollowers: "Minimum followers",
    minScore: "Minimum score",
    communityProjectsPageDesc: "Discover community-driven initiatives that are building stronger, more sustainable neighborhoods across Europe",
    totalVolunteers: "Total Volunteers",
    backToHome: "Back to Home",
    basedOnPrefs: "Based on your preferences and location",
    excellent: "Excellent",
    veryGood: "Very Good",

    // Community Page
    communityPageTitle: "EcoNexo Community",
    tabFeed: "Feed",
    tabGroups: "Groups",
    tabMentoring: "Mentoring",
    tabMarketplace: "Marketplace",
    recentActivity: "Recent Activity",
    noActivityYet: "No activity yet",
    createdAnEvent: "created an event",
    localGroupsTitle: "Local Groups",
    createGroupBtn: "+ Create Group",
    localGroupsDesc: "Connect with local communities in your city or region",
    viewAllGroups: "View All Groups",
    mentoringTitle: "Mentoring",
    mentoringDesc: "Connect with experts or find learners in sustainability",
    findMentorBtn: "Find Mentor",
    findMentorDesc: "Find an expert to guide you",
    becomeMentorBtn: "Become a Mentor",
    shareKnowledgeDesc: "Share your knowledge",
    marketplaceTitle: "Eco Marketplace",
    publishBtn: "+ Publish",
    marketplaceDesc: "Buy, sell or exchange sustainable products and services",
    karma: "Karma",
    reputation: "Reputation",
    followers: "Followers",
    quickLinks: "Quick Links",
    myProfileLabel: "My Profile",

    good: "Good",
    fair: "Fair",
    howRecsWork: "How recommendations work",
    locationPrio: "Location: We prioritize events near you",
    interestsPrio: "Interests: Based on your favorite categories",
    timesPrio: "Schedules: We consider your preferred times",
    historyPrio: "History: We learn from past events",
    availabilityPrio: "Availability: Only events with free spots",
    environmentalProject: "Environmental project",
    educationalProject: "Educational project",
    healthServices: "Health services",
    communityProject: "Community project",
    foodProject: "Food project",
    activeProject: "Active project",
    exploreCategories: "Explore Categories",
    exploreDescription: "Discover regenerative projects, community events, and impact opportunities in your area.",
    featuredProjectsDesc: "Initiatives that are changing Europe right now.",
    viewProjects: "View Projects",
    coursesAndWorkshops: "Courses and Workshops",
    coursesAndWorkshopsDesc: "Learn skills for a sustainable future.",
    exploreCourses: "Explore Courses",
    impactSearch: "Impact Search",
    searchPlaceholderExplore: "What are you looking for? (e.g. Permaculture, Barcelona, Oceans...)",
    searchResults: "Search results",
    found: "found",
    noResultsFor: "No results found for",
    clearSearch: "Clear search",
    haveInitiative: "Do you have an initiative?",
    registerInitiativeDesc: "Register it on EcoNexo and connect with thousands of volunteers.",
    publishProject: "Publish Project",
    tagBiodiversity: "Biodiversity",
    tagSolarEnergy: "Solar Energy",
    tagZeroWaste: "Zero Waste",
    tagReforestation: "Reforestation",
    ecoTipsTitle: "Eco Tips",
    ecoTipsDescription: "Practical ideas to reduce your footprint and live sustainably.",
    // Map filters/search
    filters: "Filters",
    search: "Search",
    searchProjects: "Search projects, cities or countries",
    categories: "Categories",
    onlyAvailableSpots: "Only with available spots",
    maxDistance: "Max distance",
    anyDistance: "Any distance",
    dateRange: "Date range",

    type: "Type",
    permanent: "Permanent",
    frequency: "Frequency",
    once: "One-time",
    regular: "Regular",
    centerOnLocation: "Center on my location",
    clearFilters: "Clear filters",
    joinCommunity: "Join Community",
    readyToMakeDifference: "Ready to make a difference?",
    readyDescription: "Explore projects, join events and connect with a community committed to our planet's future",
    letsGo: "Let's Go!",
    viewMap: "View Map",
    addToGoogleCalendar: "Add to Google Calendar",
    addToOutlookCalendar: "Add to Outlook",
    downloadICS: "Download iCal",
    addToCalendar: "Add to Calendar",
    liveCommunityActivity: "Live Community Activity",
    mentionHashtag: "#EcoNexo on Social Media",
    questCompleted: "Quest Completed",
    questCompletedXP: "Quest Reward",
    questOceanGuardian: "Ocean Guardian",
    questStepJoinOceans: "Join 2 ocean conservation events",
    questStepShareOcean: "Share an ocean project",
    questUrbanSustainer: "Urban Sustainer",
    questStepSyncEvents: "Sync 3 events to your calendar",
    questStepJoinWorkshop: "Attend an urban gardening workshop",
    stats: "Stats",
    quests: "Quests",
    viewAllBadges: "View All Badges",
    communityRank: "Community Rank",
    karmaLabel: "Karma",
    impactLabel: "Impact",
    dailyChallenges: "Daily Challenges",
    challengeGreenerPlanet: "Greener Planet",
    challengeGreenerDesc: "Sync 3 events this week",
    challengeEcoVoice: "Eco Voice",
    challengeEcoVoiceDesc: "Share 10 projects",
    activeQuestsLabel: "Active Quests",
    newBadge: "New Badge!",
    levelLabelShort: "LVL",
    noActiveQuests: "No active quests. Stay tuned!",
    collectReward: "Collect Reward",
    // Categories
    environmentTitle: "Environment",
    environmentDescription: "Conservation, reforestation and ecosystem protection projects",
    educationTitle: "Education",
    educationDescription: "Educational initiatives and environmental training programs",
    communityTitle: "Community",
    communityDescription: "Community development and social cohesion projects",
    healthTitle: "Health",
    healthDescription: "Community health and wellness initiatives",
    oceansTitle: "Oceans",
    oceansDescription: "Marine protection and aquatic conservation projects",
    foodTitle: "Food",
    foodDescription: "Sustainable agriculture and local food systems",
    // Welcome message values
    sustainabilityTitle: "Sustainability",
    sustainabilityDescription: "We are committed to promoting sustainable practices and environmental protection across all our activities and partnerships.",
    globalImpactTitle: "Global Impact",
    globalImpactDescription: "Our projects and initiatives create measurable positive impacts for the environment and society on a global scale.",
    innovationTitle: "Innovation",
    innovationDescription: "We leverage cutting-edge technology and innovative solutions to address environmental challenges and create sustainable futures.",
    // Local Groups
    localGroupsSubtitle: "Connect with local communities in your city or region",
    createGroup: "Create Group",
    allGroups: "All",
    myGroups: "My Groups",
    nearbyGroups: "Nearby",
    searchGroups: "Search groups...",
    noGroupsFound: "No groups found",
    createFirstGroup: "Create the first group",
    eventsLabel: "events",
    // Mock Groups
    groupBerlinName: "Sustainable Berlin",
    groupBerlinDescription: "Community of environmental activists in Berlin",
    groupMadridName: "Green Madrid",
    groupMadridDescription: "Ecological initiatives in Madrid",
    // Labels
    volunteersLabelEs: "voluntarios",
    volunteersLabelEn: "volunteers",
    volunteersLabelDe: "Freiwillige",
    spotsLabelEs: "cupos",
    spotsLabelEn: "spots",
    spotsLabelDe: "Plätze",
    // Locations
    locationBerlin: "Berlin, Germany",
    locationParis: "Paris, France",
    locationMadrid: "Madrid, Spain",
    // Notifications
    notificationsNotSupported: "Notifications not supported",
    browserNotSupportPush: "Your browser doesn't support push notifications. Update your browser or use a more modern one.",
    notificationSettings: "Notification Settings",
    pushNotifications: "Push Notifications",
    receiveNotificationsAboutEvents: "Receive notifications about new events and projects",
    activated: "Activated",
    deactivating: "Deactivating...",
    deactivate: "Deactivate",
    activating: "Activating...",
    activate: "Activate",
    notificationsBlocked: "Notifications are blocked. Go to your browser settings to allow them.",
    subscribedToPushNotifications: "Subscribed to push notifications. You will receive updates about events and projects.",
    // Profile
    pleaseSelectValidImageEs: "Por favor selecciona un archivo de imagen válido",
    pleaseSelectValidImageEn: "Please select a valid image file",
    pleaseSelectValidImageDe: "Bitte wählen Sie eine gültige Bilddatei aus",
    imageTooLargeEs: "La imagen es demasiado grande. Por favor selecciona una imagen menor a 5MB",
    imageTooLargeEn: "Image is too large. Please select an image smaller than 5MB",
    imageTooLargeDe: "Bild ist zu groß. Bitte wählen Sie ein Bild kleiner als 5MB aus",
    errorLoadingImageEs: "Error al cargar la imagen. Por favor intenta de nuevo.",
    errorLoadingImageEn: "Error loading image. Please try again.",
    errorLoadingImageDe: "Fehler beim Laden des Bildes. Bitte versuchen Sie es erneut.",
    // Events
    categoryEnvironment: "Environment",
    categoryEducation: "Education",
    categoryCommunity: "Community",
    categoryTechnology: "Technology",
    welcomeToForum: "Welcome to the {{topic}} forum!",
    startDiscussionMsg: "Start a discussion or answer questions.",
    pleaseSignInFirstEs: "Por favor inicia sesión primero",
    pleaseSignInFirstEn: "Please sign in first",
    pleaseSignInFirstDe: "Bitte zuerst anmelden",
    linkCopiedEs: "Enlace copiado.",
    linkCopiedEn: "Link copied.",
    linkCopiedDe: "Link kopiert.",
    savedEs: "Guardado",
    savedEn: "Saved",
    savedDe: "Gespeichert",
    saveEs: "Guardar",
    saveEn: "Save",
    saveDe: "Speichern",
    joinEvent: "Join Event",
    fullyBooked: "Fully Booked",
    shareEvent: "Share Event",
    // Header
    emailPlaceholderEs: "email@ejemplo.com",
    emailPlaceholderEn: "email@example.com",
    emailPlaceholderDe: "email@beispiel.de",
    profileSavedMessageEs: "Perfil guardado. Ingresa tu correo y presiona login para verificar.",
    profileSavedMessageEn: "Profile saved. Enter your email and press login to verify.",
    profileSavedMessageDe: "Profil gespeichert. Gib deine E‑Mail ein und klicke auf Login, um zu verifizieren.",
    oauthExtractMessageEs: "Se extraerán automáticamente: nombre, email y foto de perfil",
    oauthExtractMessageEn: "Automatically extracted: name, email and profile picture",
    oauthExtractMessageDe: "Automatisch extrahiert: Name, E-Mail und Profilbild",
    signinNotConfiguredEs: "Inicio de sesión no configurado",
    signinNotConfiguredEn: "Sign-in not configured",
    signinNotConfiguredDe: "Anmeldung nicht konfiguriert",
    googleOAuthTitleEs: "Google - Extrae nombre, email y foto",
    googleOAuthTitleEn: "Google - Extracts name, email and photo",
    googleOAuthTitleDe: "Google - Extrahiert Name, E-Mail und Foto",
    outlookOAuthTitleEs: "Outlook/Microsoft - Extrae nombre, email y foto",
    outlookOAuthTitleEn: "Outlook/Microsoft - Extracts name, email and photo",
    outlookOAuthTitleDe: "Outlook/Microsoft - Extrahiert Name, E-Mail und Foto",
    orGoDirectlyToEs: "O ve directamente a:",
    orGoDirectlyToEn: "Or go directly to:",
    orGoDirectlyToDe: "Oder direkt zu:",
    // App metadata
    appTitle: "EcoNexo - Sustainable Projects in Europe",
    appDescription: "Connect with sustainable projects and events across Europe. Join the community building a greener future.",
    // About page
    aboutTitle: "About EcoNexo",
    aboutSubtitle: "Connecting Europe's environmental community for a sustainable future",
    aboutMissionTitle: "Our Mission",
    aboutMissionP1: "EcoNexo is dedicated to connecting environmental activists, volunteers, and organizations across Europe to create a more sustainable future. We believe that collaboration and community are key to addressing the environmental challenges of our time.",
    aboutMissionP2: "Our platform enables users to discover environmental projects, join events, find green jobs, and connect with like-minded individuals who share their passion for environmental protection.",
    aboutOfferTitle: "What We Offer",
    aboutOfferMapTitle: "Interactive Map",
    aboutOfferMapDesc: "Discover environmental projects and events near you with our interactive map interface.",
    aboutOfferEventsTitle: "Events & Activities",
    aboutOfferEventsDesc: "Join environmental events, workshops, and community activities happening across Europe.",
    aboutOfferJobsTitle: "Green Jobs",
    aboutOfferJobsDesc: "Find meaningful employment opportunities in the environmental and sustainability sector.",
    aboutOfferChatTitle: "Community Chat",
    aboutOfferChatDesc: "Connect with fellow environmentalists through our community discussion forums.",
    aboutOfferProfilesTitle: "Personal Profiles",
    aboutOfferProfilesDesc: "Create detailed profiles to showcase your environmental interests and expertise.",
    aboutOfferMultilingualTitle: "Multilingual",
    aboutOfferMultilingualDesc: "Available in English, Spanish, and German to serve diverse European communities.",
    aboutValuesTitle: "Our Values",
    aboutValueSustainabilityTitle: "Sustainability",
    aboutValueSustainabilityDesc: "We are committed to promoting sustainable practices and environmental protection across all our activities and partnerships.",
    aboutValueCommunityTitle: "Community",
    aboutValueCommunityDesc: "We believe in the power of community and collaboration to create meaningful environmental change.",
    aboutValueInclusivityTitle: "Inclusivity",
    aboutValueInclusivityDesc: "We welcome people from all backgrounds and skill levels to join our environmental community.",
    aboutValueImpactTitle: "Impact",
    aboutValueImpactDesc: "We focus on creating measurable positive impact for the environment and our communities.",
    aboutImpactTitle: "Our Impact",
    aboutCommunityMembers: "Community Members",
    aboutEuropeanCities: "European Cities",
    aboutGetInvolvedTitle: "Get Involved",
    aboutGetInvolvedDesc: "Ready to make a difference? Join our community and start contributing to environmental change today.",
    // Event details
    backToEvents: "Back to events",
    date: "Date",
    time: "Time",
    location: "Location",
    contact: "Contact",
    volunteerProgress: "Volunteer progress",
    spotsLeft: "spots left",
    requirements: "Requirements",
    benefits: "Benefits",
    rememberToTellThemTooltip: "Remember to tell them you came from EcoNexo!",
    // Onboarding
    onboardingWelcome: "Welcome to EcoNexo!",
    onboardingDescription: "Discover sustainable projects and connect with a community committed to environmental protection",
    onboardingMap: "Explore the Map",
    onboardingMapDescription: "Visualize sustainable projects across Europe and find opportunities near you",
    onboardingExploreMap: "Explore Map",
    onboardingEvents: "Environmental Events",
    onboardingEventsDescription: "Participate in volunteer events and community activities",
    onboardingViewEvents: "View Events",
    onboardingProjects: "Sustainable Projects",
    onboardingProjectsDescription: "Discover initiatives that are changing the world",
    onboardingCreateProject: "Create Project",
    onboardingJobs: "Green Jobs",
    onboardingJobsDescription: "Find career opportunities in the sustainable sector",
    onboardingViewJobs: "View Jobs",
    onboardingCommunity: "Community",
    onboardingCommunityDescription: "Connect with people who share your passion for the environment",
    onboardingJoinChat: "Join Chat",
    onboardingNext: "Next",
    onboardingSkip: "Skip",
    onboardingSelectLanguage: "Select your language",
    onboardingLanguageDescription: "EcoNexo is available in multiple languages",
    onboardingLanguageTip: "You can change the language anytime from the menu",
    // Welcome Intro
    welcomeIntroWelcomeTitle: "Welcome to EcoNexo!",
    welcomeIntroWelcomeDescription: "Your platform for sustainable projects in Europe",
    welcomeIntroLanguageTitle: "Language",
    welcomeIntroLanguageDescription: "Choose your preferred language for a better experience",
    welcomeIntroMapTitle: "Interactive Map",
    welcomeIntroMapDescription: "Explore sustainable projects across Europe",
    welcomeIntroEventsTitle: "Events",
    welcomeIntroEventsDescription: "Discover environmental events and volunteer opportunities",
    welcomeIntroJobsTitle: "Jobs",
    welcomeIntroJobsDescription: "Find career opportunities in the green sector",
    welcomeIntroChatTitle: "Chat",
    welcomeIntroChatDescription: "Connect with the environmental community",
    welcomeIntroProfileTitle: "Profile",
    welcomeIntroProfileDescription: "Manage your account and saved projects",
    welcomeIntroSkip: "Skip",
    welcomeIntroNext: "Next",
    welcomeIntroGetStarted: "Get Started!",
    welcomeIntroFinish: "Finish",
    // Profile fields
    personalInformation: "Personal Information",
    firstName: "First Name",
    lastName: "Last Name",
    phone: "Phone",
    city: "City",
    country: "Country",
    aboutMe: "About Me",
    passions: "Passions",
    areasOfExpertise: "Areas of Expertise",
    hobbies: "Hobbies",
    interests: "Interests",
    skills: "Skills",
    languages: "Languages",
    socialMedia: "Social Media",

    // OAuth Provider Information
    googleInformation: "Google Information",
    microsoftInformation: "Microsoft Information",
    providerInformation: "Provider Information",
    extractedFromOAuth: "This information was automatically extracted from your OAuth account upon login.",
    calculatedAge: "Calculated Age",
    years: "years",
    verifiedEmail: "Verified Email",
    verified: "Verified",
    preferredLanguage: "Preferred Language",
    googleWorkspaceDomain: "Google Workspace Domain",
    tenantId: "Tenant ID",
    preferredUsername: "Preferred Username",
    linkedin: "LinkedIn",
    twitter: "Twitter",
    instagram: "Instagram",
    firstNamePh: "Enter first name",
    lastNamePh: "Enter last name",
    phonePh: "Enter phone number",
    aboutMePh: "Tell us about yourself...",
    linkedinPh: "https://linkedin.com/in/your-profile",
    twitterPh: "https://twitter.com/your-username",
    instagramPh: "https://instagram.com/your-username",
    websitePh: "https://your-website.com",
    website: "Website",
    useLogo: "Use logo",
    passionsPlaceholder: "What are you passionate about?",
    expertisePlaceholder: "What are you an expert in?",
    hobbiesPlaceholder: "What are your hobbies?",
    interestsPlaceholder: "What interests you?",
    skillsPlaceholder: "What skills do you have?",
    languagesPlaceholder: "What languages do you speak?",
    spotsLabel: "spots",
    projectTitle: "Project Title",
    projectTitlePh: "Ex: Sustainable Urban Reforestation",
    locationPh: "City, Country",
    detailedDescription: "Detailed Description",
    detailedDescriptionPh: "Describe goals, impact, and methodology...",
    targetParticipants: "Target Participants",
    estimatedBudget: "Estimated Budget (€)",
    keyMilestones: "Key Milestones",
    keyMilestonesPh: "- Launch\n- First implementation phase\n- Evaluation",
    accessibilityNeedsPh: "e.g. Wheelchair, limited mobility, etc.",
    writeMessagePh: "Type a message...",
    roomDestroyed: "Room Destroyed",
    roomDestroyedDesc: "This room has self-destructed as scheduled. All messages have been permanently deleted.",
    participantsCount: "{{count}} participants",
    leaveRoom: "Leave room",
    noMessagesYet: "No messages yet. Be the first to write!",
    signInToSendMessage: "Sign in to send messages",
    goBack: "Go Back",
    searchConfiguration: "Search Configuration",
    remoteWorkOnly: "Remote work only",
    experienceYears: "Experience (years)",
    skillsCommas: "Skills (comma separated)",
    salaryMin: "Salary (Min)",
    matchingAgentWelcome: "Hello! I'm your intelligent matching assistant. I can help you find projects that perfectly match your interests and preferences. What type of projects interest you?",
    matchingAgentError: "Sorry, there was an error searching for projects. Please try again.",
    signInToUseMatching: "Please sign in to use the matching assistant.",
    suggestionsLabel: "Suggestions:",
    typeSearchPh: "Type your search...",
    searchEventsPh: "Search events in your city...",
    searchEventsTitlePh: "Title, location, organizer...",
    loadingCalendar: "Loading calendar...",
    loadingEventsIndicator: "Loading events...",
    showFiltersLabel: "Show Filters",
    hideFiltersLabel: "Hide Filters",
    noEventsFound: "No events found matching the filters",
    becomeMentor: "Become a Mentor",
    shareKnowledge: "Share your knowledge and support learners in sustainability topics.",
    expertiseAreas: "Expertise areas",
    expertiseAreasPhMentoring: "Energy, Circularity...",
    languagesPhMentoring: "German, English...",
    availabilityPhMentoring: "2 h/week",
    offerMentoring: "Offer Mentoring",
    savingEllipsis: "Saving...",
    creatingEllipsis: "Creating...",
    groupName: "Group Name",
    groupNamePh: "e.g. Berlin Sustainable",
    groupDescriptionPh: "Describe the purpose and goals of the group...",
    tagsLabel: "Tags",
    region: "Region",
    add: "Add",
    addTagPh: "Add tag...",
    publicGroup: "Public group (anyone can join)",
    backToGroups: "Back to Groups",
    mustSignInToCreateGroup: "You must sign in to create a group",
    volunteersLabel: "volunteers",
    previewLabel: "Preview",

    // Project names and descriptions
    urbanReforestationBerlin: "Urban Reforestation Berlin",
    urbanReforestationDesc: "Join our urban reforestation initiative in Berlin. Help plant trees and create green spaces in the city.",
    treesPlanted: "trees planted",
    seineRiverCleanup: "Seine River Cleanup",
    seineRiverCleanupDesc: "Participate in cleaning the Seine River in Paris. Help remove waste and protect aquatic life.",
    wasteCollected: "tons of waste collected",
    communityGardensMadrid: "Community Gardens Madrid",
    communityGardensDesc: "Get involved in community gardens across Madrid. Learn sustainable agriculture and connect with neighbors.",
    communityGardensActive: "community gardens active",

    // Eco Tips
    ecoTipCategoryFinance: "Finance",
    ecoTipBankTitle: "Choose Sustainable Banking",
    ecoTipBankDescription: "Switch to banks that invest in renewable energy and avoid fossil fuel investments.",
    ecoTipCategoryTransport: "Transport",
    ecoTipTransportTitle: "Use Public Transport",
    ecoTipTransportDescription: "Reduce your carbon footprint by using buses, trains, and bicycles instead of cars.",
    ecoTipCategoryFood: "Food",
    ecoTipVegetarianTitle: "Try Plant-Based Meals",
    ecoTipVegetarianDescription: "Reduce meat consumption and try delicious vegetarian or vegan alternatives.",
    ecoTipCategoryEnergy: "Energy",
    ecoTipEnergyTitle: "Switch to Renewable Energy",
    ecoTipEnergyDescription: "Choose energy providers that use solar, wind, or hydroelectric power.",
    ecoTipCategoryWaste: "Waste",
    ecoTipWasteTitle: "Reduce, Reuse, Recycle",
    ecoTipWasteDescription: "Minimize waste by buying less, reusing items, and properly recycling materials.",
    ecoTipCategoryWater: "Water",
    ecoTipWaterTitle: "Conserve Water",
    ecoTipWaterDescription: "Take shorter showers, fix leaks, and use water-efficient appliances.",
    ecoTipCategoryOceans: "Oceans",
    ecoTipOceanTitle: "Protect Marine Life",
    ecoTipOceanDescription: "Avoid single-use plastics and use reef-safe sunscreen to protect our oceans.",
    ecoTipCategoryHealth: "Health",
    ecoTipHealthTitle: "Natural Wellness",
    ecoTipHealthDescription: "Use eco-friendly cleaning products to improve indoor air quality and health.",
    ecoTipCategoryEducation: "Education",
    ecoTipEducationTitle: "Continuous Learning",
    ecoTipEducationDescription: "Share your knowledge about sustainability with friends and family to amplify impact.",
    ecoTips: "Eco Tips",

    // Event creation
    createEvent: "Create Event",

    // Event administrators
    eventAdministrators: "Event Administrators",
    addAdministrator: "Add Administrator",
    administratorEmail: "Email de l'Administrateur",
    searchAdministrator: "Search by email",
    addAsAdministrator: "Add as Administrator",
    removeAdministrator: "Remove Administrator",
    manageAdministrators: "Manage Administrators",
    noAdministrators: "No administrators",
    youAreAdministrator: "You are an administrator",
    eventCreator: "Event Creator",
    administrators: "Administrators",

    ofRevenueDonated: "of revenue donated to environmental causes",
    subscribe: "Subscribe",

    // Chat translations
    communityChat: "Community Chat",
    topicForums: "Topic Forums",
    generalTopic: "General",
    topicDescriptionGeneral: "General discussions about sustainability",
    environmentTopic: "Environment",
    topicDescriptionEnvironment: "Environmental protection and climate action",
    educationTopic: "Education",
    topicDescriptionEducation: "Environmental education and awareness",
    healthTopic: "Health",
    topicDescriptionHealth: "Health and environmental connections",
    communityTopic: "Community",
    topicDescriptionCommunity: "Community building and local initiatives",
    oceansTopic: "Oceans",
    topicDescriptionOceans: "Ocean conservation and marine life",
    foodTopic: "Food",
    topicDescriptionFood: "Sustainable food and agriculture",
    chatMessage1: "Great initiative! How can I get involved?",
    chatMessage2: "I've been working on similar projects in my area",
    chatMessage3: "This is exactly what we need more of!",
    chatMessage4: "Has anyone tried this approach before?",

    // Chat Rules
    chatRule1: "Respect all participants",
    chatRule2: "Keep conversations relevant",
    chatRule3: "Don't share personal information",
    chatRule4: "Use appropriate language",
    chatRule5: "No spam",
    chatRule6: "Be constructive in your comments",
    chatRule7: "Report inappropriate content",
    chatRule8: "Enjoy and learn",
    chatRules: "Chat Rules",
    chatRulesFooter: "Following these rules creates a positive community",
    chatRulesIntro: "To maintain a positive environment, please follow these rules:",
    chatRulesTitle: "Community Rules",

    // Chat Recommendations
    recommendations: "Recommendations",
    popularDiscussions: "Popular Discussions",
    joinActiveConversations: "Join Active Conversations",
    newProjects: "New Projects",
    discoverLatestInitiatives: "Discover Latest Initiatives",
    eventsNearYou: "Events Near You",
    findLocalEvents: "Find Local Events",

    // Category translations
    environment: "Environment",
    education: "Education",
    health: "Health",
    community: "Community",
    oceans: "Oceans",
    food: "Food",

    capital: "Capital",
    population: "Population",
    currency: "Currency",
    regionLabel: "Region",
    governmentAPIAvailable: "Government API Available",
    governmentAPIInDevelopment: "Government API in development",
    integratedGovernmentAPIs: "Integrated Government APIs",
    baseURL: "Base URL",

    // Additional event titles for calendar
    eventNativeTreePlanting: "Native Tree Planting",
    eventSolarEnergyWorkshop: "Solar Energy Workshop",
    eventRiverCleanup: "River Cleanup",
    eventClimateConference: "Climate Change Conference",
    eventFoodConservationWorkshop: "Food Conservation Workshop",

    // Event titles
    eventVerticalGardens: "Vertical Gardens Construction",
    eventEcoRace: "Eco-friendly 5K Race",
    eventSolarPanels: "Solar Panels Installation",
    eventEnvironmentalReflection: "Semester Environmental Reflection",
    eventSustainableSummer: "Sustainable Summer Celebration",
    eventAutumnPlanting: "Autumn Planting",
    eventForestCleanup: "Autumn Forest Cleanup",
    eventBirdShelters: "Bird Shelters Construction",
    eventWindEnergy: "Wind Energy Workshop",
    eventOrganicProducts: "Organic Products Fair",
    eventWetlandsRestoration: "Wetlands Restoration",
    eventCommunityGardens: "Community Gardens Construction",
    eventSustainableChristmas: "Sustainable Christmas Market",
    eventWinterReforestation: "Winter Reforestation",
    eventClimateSeminar: "Climate Change Seminar",
    eventEcoNewYear: "Eco-friendly New Year Celebration",
    eventMarineConservation: "Marine Species Conservation",
    eventHydroelectricWorkshop: "Hydroelectric Energy Workshop",

    // Job titles
    jobTitle1: "Urban Reforestation Specialist",
    jobTitle2: "Air Quality Data Analyst",
    jobTitle3: "Environmental Educator (STEM)",
    jobTitle4: "River Cleanup Coordinator",
    jobTitle5: "Circular Economy Architect",
    jobTitle6: "Ocean Conservation Specialist",
    jobTitle7: "Green Architecture Specialist",
    jobTitle8: "Circular Economy Researcher",
    jobTitle9: "Wind Farm Operations Engineer",

    // Job descriptions
    jobDesc1: "Lead urban reforestation initiatives in Berlin, managing tree planting projects, coordinating with local communities, and developing sustainable green spaces. You'll work with environmental scientists, city planners, and volunteers to create healthier urban environments.",
    jobDesc2: "Analyze air quality data using Python and GIS tools, monitor sensor networks across Paris, and create reports for environmental policy makers. You'll work with data scientists and environmental engineers to improve air quality monitoring systems.",
    jobDesc3: "Develop and deliver STEM-based environmental education programs in Madrid, teaching students about solar energy, robotics, and sustainability. You'll create engaging curriculum and work with schools to promote environmental awareness.",
    jobDesc4: "Coordinate river cleanup operations in Milan, managing volunteer teams, ensuring safety protocols, and organizing waste collection initiatives. You'll work with environmental organizations and local authorities to protect water ecosystems.",
    jobDesc5: "Design and implement circular economy strategies for companies in Amsterdam. Work with product teams to extend lifecycle, reduce waste, and create closed-loop material flows. Collaborate with municipalities and startups.",
    jobDesc6: "Support marine conservation programs out of Lisbon. Monitor biodiversity, analyze data from underwater sensors, and coordinate citizen‑science beach surveys with NGOs and local universities.",
    jobDesc7: "Lead sustainable architecture projects in Amsterdam. Integrate energy efficiency, passive design, and low‑carbon materials (LEED/BREEAM). Coordinate with urban planners and community stakeholders.",
    jobDesc8: "Join a Copenhagen research institute to evaluate circular economy pilots. Quantify impact (LCA), publish findings, and support policy recommendations across the Nordics.",
    jobDesc9: "Operate and optimize wind farms around Hamburg. Oversee maintenance schedules, SCADA monitoring, safety procedures, and performance improvements with OEM partners.",

    // Technical tags
    sensores: "sensors",
    silvicultura: "silviculture",
    biodiversidad: "biodiversity",
    gestióndeproyectos: "project management",
    didáctica: "pedagogy",
    energíasolar: "solar energy",
    robóticaeducativa: "educational robotics",
    gestióndevoluntariado: "volunteer management",
    residuos: "waste",
    seguridad: "safety",
    python: "Python",
    gis: "GIS",
    botánica: "botany",
    estadística: "statistics",
    logística: "logistics",
    machinelearning: "machine learning",
    comunicación: "communication",
    biologíamarina: "marine biology",
    conservación: "conservation",
    investigación: "research",
    buceo: "diving",
    reactnative: "React Native",
    nodejs: "Node.js",
    blockchain: "blockchain",
    iot: "IoT",
    uxui: "UX/UI",

    // Interface translations
    searchPlaceholderEn: "Search for jobs...",
    cityLabel: "City",
    contractLabel: "Contract type",
    contract_full_time: "Full-time",
    contract_part_time: "Part-time",
    contract_contract: "Contract",
    contract_internship: "Internship",
    remoteOnly: "Remote only",
    yearsExp: "years experience",
    applyBtn: "Apply",
    saveBtn: "Save",
    jobPostingConditions: "Job Posting Conditions",
    jobPostingInfo: "Job posting information",
    conductGuidelines: "Conduct Guidelines",
    conductGuidelinesText: "Maintain a respectful and constructive environment",

    // Job form fields
    jobTitle: "Job Title",
    jobTitlePlaceholder: "Enter the job title",
    companyName: "Company Name",
    companyNamePlaceholder: "Enter your company name",
    jobLocation: "Job Location",
    jobLocationPlaceholder: "Where is the job located?",
    jobDescription: "Job Description",
    jobDescriptionPlaceholder: "Describe the job responsibilities",
    requiredSkills: "Required Skills",
    requiredSkillsPlaceholder: "Skills needed for the position",
    benefitsPlaceholder: "Job benefits",
    contactEmail: "Contact Email",
    contactEmailPlaceholder: "email@company.com",
    salaryRange: "Salary Range",
    salaryRangePlaceholder: "Salary range offered",
    jobType: "Job Type",
    jobTypeFullTime: "Full-time",
    jobTypePartTime: "Part-time",
    jobTypeContract: "Contract",
    jobTypeInternship: "Internship",
    jobTypeRemote: "Remote",
    jobTypeHybrid: "Hybrid",
    experienceLevel: "Experience Level",
    experienceEntry: "Entry level",
    experienceMid: "Mid level",
    experienceSenior: "Senior level",
    experienceExpert: "Expert level",
    applicationDeadline: "Application Deadline",
    createJobOffer: "Create Job Offer",
    saveDraft: "Save Draft",
    publishJob: "Publish Job",
    jobCreated: "Job Created",

    // Language levels
    spanish: "Spanish",
    english: "English",
    german: "German",
    nativeLanguage: "Native language",
    selectLevel: "Select level",
    beginner: "Beginner",
    intermediate: "Intermediate",
    advanced: "Advanced",
    fluent: "Fluent",
    selectLanguage: "Select language",
    levelJunior: "Junior",
    levelMid: "Mid",
    levelSenior: "Senior",
    levelLead: "Lead",

    // Admin panel titles
    adminPanelTitle: "Administration Panel",
    adminPanelDescription: "Manage events, projects and EcoNexo users",
    adminTotalUsers: "Total Users",
    adminActiveEvents: "Active Events",
    adminProjects: "Projects",
    adminGreenJobs: "Green Jobs",
    adminActiveUsers: "Active Users",
    adminLast30Days: "Last 30 days",
    adminEventManagement: "Event Management",
    adminProjectManagement: "Project Management",
    adminUserManagement: "User Management",
    adminEvent: "Event",
    adminDate: "Date",
    adminParticipants: "Participants",
    adminStatus: "Status",
    adminActions: "Actions",
    adminProject: "Project",
    adminCreator: "Creator",
    adminVolunteers: "Volunteers",
    adminActive: "Active",
    adminCompleted: "Completed",
    adminCancelled: "Cancelled",
    adminDraft: "Draft",
    adminEdit: "Edit",
    adminDelete: "Delete",
    adminApprove: "Approve",
    adminOverview: "Overview",
    adminEvents: "Events",
    adminUsers: "Users",
    adminUserManagementInDevelopment: "User management functionality in development...",
    adminBeachCleanupBarcelona: "Beach Cleanup Barcelona",
    adminTreePlantingMadrid: "Tree Planting Madrid",
    adminCommunityGardenValencia: "Community Garden Valencia",
    adminSolarEnergyNeighborhood: "Solar Energy Neighborhood",

    // Gamification titles

    badgeEcoWarrior: "Eco Warrior",
    badgeCommunityLeader: "Community Leader",
    badgeGreenJobHunter: "Green Job Hunter",
    badgeChatMaster: "Chat Master",

    level: "Level",

    achievementEcoExplorer: "Eco Explorer",
    achievementProjectPioneer: "Project Pioneer",
    achievementCommunityConnector: "Community Connector",
    achievementCarbonNeutral: "Carbon Neutral",

    // Admin page metadata
    adminPageTitle: "Administration Panel - EcoNexo",
    adminPageDescription: "Manage events, projects and EcoNexo users",

    // Map titles
    centerOnMyLocation: "Center on my location",

    // Social media titles
    socialMediaCommunityReforestationBerlin: "Community Reforestation in Berlin",

    // Regions
    all: "All",
    northernEurope: "Northern Europe",
    westernEurope: "Western Europe",
    centralEurope: "Central Europe",
    southernEurope: "Southern Europe",
    easternEurope: "Eastern Europe",
    minExperience: "Minimum experience",
    minSalary: "Minimum salary",
    allCities: "All",
    allContracts: "All",
    results: "results",

    // Calendar and Events translations
    calendar: "Calendar",
    required: "Required",
    titlePh: "Event title",
    mainCategory: "Main Category",
    cityPh: "City name",
    countryPh: "Country name",
    optional: "Optional",
    addressPh: "Event address",
    websitePlaceholderEn: "https://example.com",
    image: "Image",
    imageUrlPlaceholderEn: "https://example.com/image.jpg",
    optionalCategories: "Optional Categories",
    capacityPh: "Maximum participants",
    notes: "Notes",
    notesPh: "Additional notes",
    save: "Save",
    existingEvents: "Existing Events",
    refresh: "Refresh",
    category: "Category",
    noParticipatedEvents: "No participated events",
    month: "Month",
    list: "List",
    viewEvent: "View Event",
    viewDetails: "View Details",
    join: "Join",
    createdEvent: "Event Created",
    eventCreatedMessage: "was created for",
    beVolunteer: "Become a volunteer",
    description: "Description",
    impact: "Impact",
    raised: "raised",
    reached: "reached",
    goal: "Goal",
    volunteers: "Volunteers",
    progressReached: "Progress reached",
    donatePaypal: "Donate with PayPal",
    donateKlarna: "Donate with Klarna",
    donateStripe: "Donate with Stripe",
    backToMap: "Back to map",
    moreInfo: "More info",
    // Auth Modal translations
    pleaseCompleteSecurityVerification: "Please complete the security verification",
    errorSendingEmail: "Error sending email",
    unexpectedError: "Unexpected error. Please try again.",
    errorStartingGoogleAuth: "Error starting Google authentication",
    errorStartingOutlookAuth: "Error starting Outlook authentication",
    securityVerificationIncorrect: "Security verification incorrect. Please try again.",
    welcomeEmailSent: "Welcome email sent! Please check your inbox and verify your account by clicking the link.",
    authenticationSuccessful: "Authentication successful!",
    orContinueWith: "Or continue with",
    registerWith: "Register with",
    or: "Or",
    emailPlaceholder: "your@email.com",
    errorInSecurityVerification: "Error in security verification",
    processing: "Processing...",
    signInButton: "Sign In",
    createAccountButton: "Create Account",
    bySigningIn: "By signing in, you accept our",
    byRegistering: "By registering, you accept our",
    termsOfService: "Terms of Service",
    and: "and",
    privacyPolicy: "Privacy Policy",
    // Captcha translations
    captchaNotConfigured: "reCAPTCHA not configured. Contact the administrator.",
    verificationCompleted: "Verification completed",
    securityVerification: "Security verification:",
    yourAnswer: "Your answer",
    verify: "Verify",
    howMuchIs: "How much is",
    // Gamification & Badges
    badgeFirstStep: "First Step",
    descFirstStep: "Completed your first action",
    badgeExplorer: "Explorer",
    descExplorer: "Explored different sections",
    badgeHelper: "Helper",
    descHelper: "Helped the community",
    descFirstEvent: "Attended your first event",
    badgeEventEnthusiast: "Enthusiast",
    descEventEnthusiast: "Attended 5 events",
    badgeEventMaster: "Event Master",
    descEventMaster: "Attended 10 events",
    badgeEventCreator: "Organizer",
    descEventCreator: "Created your first event",
    badgeProjectJoiner: "Collaborator",
    descProjectJoiner: "Joined your first project",
    badgeProjectCreator: "Creator",
    descProjectCreator: "Created your first project",
    badgeProjectVeteran: "Veteran",
    descProjectVeteran: "Completed 5 projects",
    badgeConnector: "Connector",
    descConnector: "Made your first connection",
    badgeMentor: "Mentor",
    descMentor: "Became a mentor",
    badgeMarketplaceSeller: "Green Seller",
    descMarketplaceSeller: "Posted in marketplace",
    badgeGreenCommuter: "Green Commuter",
    descGreenCommuter: "Used sustainable transport",
    badgeWasteWarrior: "Waste Warrior",
    descWasteWarrior: "Reduced waste",
    badgeCarbonFighter: "Carbon Fighter",
    descCarbonFighter: "Offset your carbon footprint",
    badgeDailyStreak7: "Weekly Streak",
    descDailyStreak7: "7 consecutive days",
    badgeDailyStreak30: "Monthly Streak",
    descDailyStreak30: "30 consecutive days",
    badgeProfileComplete: "Profile Complete",
    descProfileComplete: "Completed your profile",
    badgeSocialSharer: "Sharer",
    descSocialSharer: "Shared 10 times",
    badgeCalendarSyncMaster: "Time Manager",
    descCalendarSyncMaster: "Synced 3 events",

    // Onboarding Tour
    tourWelcomeTitle: "Welcome to EcoNexo!",
    tourWelcomeDesc: "Let's take a quick tour to help you get started with your sustainability journey.",
    tourMapTitle: "Interactive Map",
    tourMapDesc: "Explore environmental projects, events, and local groups near you.",
    tourCalendarTitle: "Events Calendar",
    tourCalendarDesc: "Find and sync workshops, cleanups, and green activities.",
    tourJobsTitle: "Green Jobs",
    tourJobsDesc: "Discover career opportunities in the sustainability sector.",
    tourChatTitle: "Community Chat",
    tourChatDesc: "Connect with other eco-conscious members and discuss topics.",
    tourMatchingTitle: "Smart Matching",
    tourMatchingDesc: "Get personalized recommendations matched to your skills.",
    tourCommunityTitle: "Community & Groups",
    tourCommunityDesc: "Join local groups, find mentors, and trade in the marketplace.",
    tourProfileTitle: "Your Profile",
    tourProfileDesc: "Track your impact, badges, and progress.",
    tourAboutTitle: "About Us",
    tourAboutDesc: "Learn more about our mission and vision.",
    tourNext: "Next",
    tourPrev: "Previous",
    tourDone: "Finish",

    // Impact Page UI
    yourImpact: "Your Impact",
    impactDescription: "You are building a greener future, step by step. Here is your progress and achievements in EcoNexo.",
    xpToLevel: "XP to Level",
    impactMedals: "Medals",
    impactStreakDays: "Streak Days",
    impactSocialImpact: "Social Impact",
    impactShared: "shared",
    impactTalleres: "Workshops",
    impactCompleted: "completed",
    impactCity: "City",
    impactRanking: "Ranking",
    impactBadgeCollection: "Badge Collection",
    impactRecentActivity: "Recent Activity",
    impactNoActivity: "No recent activity.",
    impactJustNow: "Just now",
    impactKeepGoing: "Keep going!",
    impactUnlockNextLevel: "Participate in more projects and events to unlock level {{level}}.",
    impactExploreProjects: "Explore Projects",
    impactLevel: "Level",

    // Badge Categories
    categoryStarter: "Starter",
    categoryEngagement: "Engagement",
    categoryLifestyle: "Lifestyle",

    // Onboarding
    personalizeEcoNexo: "Personalize EcoNexo",
    onboardingSubtitle: "Help us recommend the best projects for you.",
    causesTitle: "What causes move you?",
    skillsTitle: "What skills can you contribute?",
    back: "Back",
    skip: "Skip",
    next: "Next",
    ready: "Ready!",

    // Skills
    skillTech: "Technology / IT",
    skillGardening: "Gardening / Field",
    skillTeaching: "Education / Teaching",
    skillManual: "Manual Work / Repair",
    skillSocial: "Social Work / Care",
    skillArt: "Art / Creativity",


    // Volunteer Page
    volunteersPageTitle: "Volunteers Page",
    localRecords: "Local Records",
    noVolunteersYet: "No volunteers yet",
    backToProject: "Back to Project",
    volunteeringPoints: "Join project",
    infoNotAvailable: "Official website not available",
  },
  de: {
    menu: "Menü",

    app: "EcoNexo",
    map: "Karte",
    events: "Veranstaltungen",
    jobs: "Jobs",
    chat: "Chat",
    home: "Startseite",
    explore: "Erkunden",
    workshops: "Workshops",
    badges: "Abzeichen",
    forum: "Forum",
    volunteering: "Freiwilligenarbeit",
    profile: "Profil",
    signIn: "Anmelden",
    signUp: "Registrieren",
    signOut: "Abmelden",
    logout: "Abmelden",
    editMode: "Bearbeitungsmodus",
    profileView: "Profilansicht",
    achievementsAndBadges: "Abzeichen & Erfolge",
    welcomeBack: "Willkommen zurück!",
    joinEcoNexo: "Tritt EcoNexo bei!",
    signInToContinue: "Melden Sie sich an, um fortzufahren",
    createAccountToStart: "Erstellen Sie ein Konto, um zu beginnen",
    checkEmailForLogin: "Überprüfen Sie Ihre E-Mail für den Anmeldelink",
    checkEmailForVerification: "Überprüfen Sie Ihre E-Mail zur Kontoverifizierung",
    continueWithGoogle: "Mit Google fortfahren",
    continueWithGitHub: "Mit GitHub fortfahren",
    continueWithMicrosoft: "Mit Microsoft fortfahren",
    dontHaveAccount: "Haben Sie kein Konto?",
    alreadyHaveAccount: "Haben Sie bereits ein Konto?",
    signInToAccessProfile: "Melden Sie sich an, um auf Ihr Profil zuzugreifen",
    accessYourProfile: "Auf Ihr Profil zugreifen",
    signInToManageProfile: "Melden Sie sich an, um Ihr Profil zu verwalten",
    whatYouCanDo: "Was Sie tun können:",
    managePersonalInfo: "Persönliche Informationen verwalten",
    setPreferences: "Einstellungen festlegen",
    trackActivities: "Aktivitäten verfolgen",
    connectWithCommunity: "Mit der Gemeinschaft verbinden",
    myAccount: "Mein Konto",
    myEvents: "Meine Veranstaltungen",
    myJobs: "Meine Jobs",
    supportUs: "Unterstützen",
    aboutUs: "Über Uns",
    createProfile: "Profil erstellen",
    email: "E-Mail",
    fullName: "Vollständiger Name",
    birthdate: "Geburtsdatum",
    birthPlace: "Geburtsort",
    pronouns: "Pronomen",
    gender: "Geschlecht",
    selectPronouns: "Pronomen auswählen",
    pronounHe: "Er",
    pronounShe: "Sie",
    pronounThey: "They",
    heShe: "er/sie",
    theyThem: "they/them",
    erSie: "er/sie",
    other: "Andere",
    selectGender: "Geschlecht auswählen",
    male: "Männlich",
    female: "Weiblich",
    nonBinary: "Nicht-binär",
    preferNotToSay: "Möchte nicht sagen",
    enterEmail: "E-Mail eingeben",
    sending: "Senden...",
    loading: "Laden...",
    authenticationRequired: "Authentifizierung erforderlich",
    signInToAccessThisPage: "Melden Sie sich an, um auf diese Seite zuzugreifen",
    whySignIn: "Warum anmelden?",
    personalizedExperience: "Personalisierte Erfahrung",
    savePreferences: "Einstellungen speichern",
    trackProgress: "Fortschritt verfolgen",
    emailNote: "Wir verwenden Ihre E-Mail zur Kontoverifizierung",
    continueWith: "Fortfahren mit",
    google: "Google",
    outlook: "Outlook",
    checkYourEmail: "Überprüfen Sie Ihre E-Mail",
    cancel: "Abbrechen",
    saveChanges: "Änderungen speichern",
    editProfile: "Profil bearbeiten",
    myProfile: "Mein Profil",
    profileUpdated: "Profil erfolgreich aktualisiert",
    changePhoto: "Foto ändern",
    savingEs: "Guardando...",
    savingEn: "Saving...",
    savingDe: "Speichern...",
    // Page content
    welcomeMessageTitle: "Verbinden Sie sich mit nachhaltigen Projekten in Europa",
    welcomeMessageDescription: "Treten Sie der Gemeinschaft bei, die eine grünere Zukunft aufbaut. Entdecken Sie Veranstaltungen, Projekte und Freiwilligenmöglichkeiten in Ihrer Nähe.",
    exploreEvents: "Veranstaltungen erkunden",
    findJobs: "Jobs finden",
    interactiveMap: "Interaktive Karte",
    showMap: "Karte Anzeigen",
    hideMap: "Karte Ausblenden",
    activeProjects: "Aktive Projekte",

    cities: "Städte",
    connectedVolunteers: "Verbundene Freiwillige",
    availableSpots: "Verfügbare Plätze",
    mapSearchPlaceholder: "Städte, Projekte, Weltweit suchen...",
    mapSearchPlaceholderGlobal: "Städte, Länder, Projekte, Orte suchen...",
    searchingGlobally: "Weltweit suchen...",
    featuredProjects: "Empfohlene Projekte",
    recommendedForYou: "Für dich empfohlen",
    match: "Match",
    communityProjectsTitle: "Gemeinschaftsprojekte",
    proximityReason: "Nur {{distance}} km von dir entfernt",
    interestReason: "Passt zu deinem Interesse an {{category}}",
    preferredTimeReason: "In deiner bevorzugten Zeit ({{time}})",
    difficultyReason: "Passender Schwierigkeitsgrad",
    noNearbyEvents: "Keine Veranstaltungen in der Nähe gefunden",
    eventsNearby: "Veranstaltungen in der Nähe",
    viewAll: "Alle ansehen",
    socialMediaDetectionHeader: "Automatische Projekterkennung",
    interactions: "Interaktionen",
    sustainable: "nachhaltig",
    approved: "Genehmigen",
    rejected: "Ablehnen",
    viewPost: "Beitrag ansehen",
    detectionActive: "Erkennung aktiv",
    detectionPaused: "Erkennung pausiert",
    monitoringSocialMedia: "Überwachung sozialer Medien auf verifizierte Umweltprojekte",
    scanningSocialMedia: "Soziale Medien werden gescannt...",
    noNewProjects: "Keine neuen Projekte gefunden",
    scanning: "Scannen",
    scan: "Escanear",
    detectionSettings: "Erkennungseinstellungen",
    minFollowers: "Mindestfollower",
    minScore: "Mindestpunktzahl",
    communityProjectsPageDesc: "Entdecken Sie von der Gemeinschaft getragene Initiativen, die stärkere und nachhaltigere Nachbarschaften in ganz Europa aufbauen",
    totalVolunteers: "Gesamt Freiwillige",
    backToHome: "Zurück zur Startseite",
    recommendationsForYou: "Empfehlungen für dich",
    basedOnPrefs: "Basierend auf deinen Vorlieben und deinem Standort",
    excellent: "Exzellent",
    veryGood: "Sehr gut",
    good: "Gut",
    fair: "Mittelmäßig",
    howRecsWork: "Wie Empfehlungen funktionieren",
    locationPrio: "Standort: Wir priorisieren Veranstaltungen in deiner Nähe",
    interestsPrio: "Interessen: Basierend auf deinen Lieblingskategorien",
    timesPrio: "Zeitpläne: Wir berücksichtigen deine bevorzugten Zeiten",
    historyPrio: "Verlauf: Wir lernen aus vergangenen Veranstaltungen",
    availabilityPrio: "Verfügbarkeit: Nur Veranstaltungen mit freien Plätzen",
    environmentalProject: "Umweltprojekt",
    educationalProject: "Bildungsprojekt",
    healthServices: "Gesundheitsdienste",
    communityProject: "Gemeinschaftsprojekt",
    foodProject: "Ernährungsprojekt",
    activeProject: "Aktives Projekt",
    exploreCategories: "Kategorien erkunden",
    exploreDescription: "Entdecke regenerative Projekte, Gemeinschaftsveranstaltungen und Wirkungsmöglichkeiten in deiner Nähe.",
    featuredProjectsDesc: "Initiativen, die Europa gerade jetzt verändern.",
    viewProjects: "Projekte anzeigen",
    coursesAndWorkshops: "Kurse und Workshops",
    coursesAndWorkshopsDesc: "Lerne Fähigkeiten für eine nachhaltige Zukunft.",
    exploreCourses: "Kurse erkunden",
    impactSearch: "Wirkungssuche",
    searchPlaceholderExplore: "Wonach suchst du? (z.B. Permakultur, Berlin, Ozeane...)",
    searchResults: "Suchergebnisse",
    found: "gefunden",
    noResultsFor: "Keine Ergebnisse gefunden für",
    clearSearch: "Suche löschen",
    haveInitiative: "Hast du eine Initiative?",
    registerInitiativeDesc: "Registriere sie auf EcoNexo und verbinde dich mit Tausenden von Freiwilligen.",
    publishProject: "Projekt veröffentlichen",
    tagBiodiversity: "Artenvielfalt",
    tagSolarEnergy: "Solarenergie",
    tagZeroWaste: "Zero Waste",
    tagReforestation: "Aufforstung",
    ecoTipsTitle: "Öko-Tipps",
    joinCommunity: "Gemeinschaft beitreten",
    readyToMakeDifference: "Bereit, einen Unterschied zu machen?",
    readyDescription: "Erkunden Sie Projekte, nehmen Sie an Veranstaltungen teil und verbinden Sie sich mit einer Gemeinschaft, die sich für die Zukunft unseres Planeten engagiert",
    letsGo: "Los geht's!",
    viewMap: "Karte anzeigen",
    addToGoogleCalendar: "Zu Google Kalender hinzufügen",
    addToOutlookCalendar: "Zu Outlook hinzufügen",
    downloadICS: "iCal herunterladen",
    addToCalendar: "Zum Kalender hinzufügen",
    liveCommunityActivity: "Live Community Aktivität",
    mentionHashtag: "#EcoNexo auf Social Media",
    questCompleted: "Quest Abgeschlossen",
    questCompletedXP: "Quest Belohnung",
    questOceanGuardian: "Wächter der Ozeane",
    questStepJoinOceans: "Nimm an 2 Ozeanschutz-Events teil",
    questStepShareOcean: "Teile ein Ozean-Projekt",
    questUrbanSustainer: "Urbaner Nachhaltigkeitsförderer",
    questStepSyncEvents: "Synchronisiere 3 Events mit deinem Kalender",
    questStepJoinWorkshop: "Nimm an einem Urban Gardening Workshop teil",
    stats: "Statistiken",
    quests: "Quests",
    viewAllBadges: "Alle Abzeichen ansehen",
    communityRank: "Community-Rang",
    karmaLabel: "Karma",
    impactLabel: "Einfluss",
    dailyChallenges: "Tägliche Herausforderungen",
    challengeGreenerPlanet: "Grünerer Planet",
    challengeGreenerDesc: "Synchronisiere diese Woche 3 Events",
    challengeEcoVoice: "Öko-Stimme",
    challengeEcoVoiceDesc: "Teile 10 Projekte",
    activeQuestsLabel: "Aktive Quests",
    newBadge: "Neues Abzeichen!",
    levelLabelShort: "LVL",
    noActiveQuests: "Keine aktiven Quests. Bleib dran!",
    collectReward: "Belohnung abholen",

    // Community Page
    communityPageTitle: "EcoNexo Gemeinschaft",
    tabFeed: "Feed",
    tabGroups: "Gruppen",
    tabMentoring: "Mentoring",
    tabMarketplace: "Marktplatz",
    recentActivity: "Aktuelle Aktivität",
    noActivityYet: "Noch keine Aktivität",
    createdAnEvent: "hat ein Event erstellt",
    localGroupsTitle: "Lokale Gruppen",
    createGroupBtn: "+ Gruppe erstellen",
    localGroupsDesc: "Verbinde dich mit lokalen Gemeinschaften in deiner Stadt oder Region",
    viewAllGroups: "Alle Gruppen anzeigen",
    mentoringTitle: "Mentoring",
    mentoringDesc: "Verbinde dich mit Experten oder finde Lernende in Nachhaltigkeit",
    findMentorBtn: "Mentor suchen",
    findMentorDesc: "Finde einen Experten, der dich führt",
    becomeMentorBtn: "Mentor werden",
    shareKnowledgeDesc: "Teile dein Wissen",
    marketplaceTitle: "Öko-Marktplatz",
    publishBtn: "+ Veröffentlichen",
    marketplaceDesc: "Kaufe, verkaufe oder tausche nachhaltige Produkte und Dienstleistungen",
    karma: "Karma",
    reputation: "Reputation",
    followers: "Follower",
    quickLinks: "Schnelllinks",
    myProfileLabel: "Mein Profil",

    // Categories
    environmentTitle: "Umwelt",
    environmentDescription: "Naturschutz-, Aufforstungs- und Ökosystemschutzprojekte",
    educationTitle: "Bildung",
    educationDescription: "Bildungsinitiativen und Umweltausbildungsprogramme",
    communityTitle: "Gemeinschaft",
    communityDescription: "Gemeinschaftsentwicklung und sozialer Zusammenhalt",
    healthTitle: "Gesundheit",
    healthDescription: "Gemeinschaftsgesundheit und Wohlbefinden",
    oceansTitle: "Ozeane",
    oceansDescription: "Meeresschutz und Wasserprojekte",
    foodTitle: "Ernährung",
    foodDescription: "Nachhaltige Ernährung und Landwirtschaft",
    // Welcome message values
    sustainabilityTitle: "Nachhaltigkeit",
    sustainabilityDescription: "Wir fördern nachhaltige Praktiken und Umweltschutz in all unseren Aktivitäten und Partnerschaften.",
    globalImpactTitle: "Globaler Einfluss",
    globalImpactDescription: "Unsere Projekte und Initiativen schaffen messbar positive Auswirkungen für Umwelt und Gesellschaft auf globaler Ebene.",
    innovationTitle: "Innovation",
    innovationDescription: "Wir nutzen modernste Technologie und innovative Lösungen, um Umweltprobleme anzugehen und nachhaltige Zukunft zu schaffen.",
    // Local Groups
    localGroupsSubtitle: "Verbinde dich mit lokalen Gemeinschaften in deiner Stadt oder Region",
    becomeMentor: "Mentor werden",
    shareKnowledge: "Teile dein Wissen und unterstütze Lernende bei Nachhaltigkeitsthemen.",
    expertiseAreasPhMentoring: "Energie, Kreislaufwirtschaft...",
    languagesPhMentoring: "Deutsch, Englisch...",
    availabilityPhMentoring: "2 Std./Woche",
    offerMentoring: "Mentoring anbieten",
    createGroup: "Gruppe erstellen",
    allGroups: "Alle",
    myGroups: "Meine Gruppen",
    nearbyGroups: "In der Nähe",
    searchGroups: "Gruppen suchen...",
    noGroupsFound: "Keine Gruppen gefunden",
    createFirstGroup: "Erste Gruppe erstellen",
    eventsLabel: "Events",
    // Mock Groups
    groupBerlinName: "Nachhaltiges Berlin",
    groupBerlinDescription: "Gemeinschaft von Umweltaktivisten in Berlin",
    groupMadridName: "Grünes Madrid",
    groupMadridDescription: "Ökologische Initiativen in Madrid",
    // Labels
    volunteersLabelEs: "voluntarios",
    volunteersLabelEn: "volunteers",
    volunteersLabelDe: "Freiwillige",
    spotsLabelEs: "cupos",
    spotsLabelEn: "spots",
    spotsLabelDe: "Plätze",
    // Locations
    locationBerlin: "Berlin, Deutschland",
    locationParis: "Paris, Frankreich",
    locationMadrid: "Madrid, Spanien",
    // Notifications
    notificationsNotSupported: "Benachrichtigungen nicht unterstützt",
    browserNotSupportPush: "Ihr Browser unterstützt keine Push-Benachrichtigungen. Aktualisieren Sie Ihren Browser oder verwenden Sie einen moderneren.",
    notificationSettings: "Benachrichtigungseinstellungen",
    pushNotifications: "Push-Benachrichtigungen",
    receiveNotificationsAboutEvents: "Erhalten Sie Benachrichtigungen über neue Veranstaltungen und Projekte",
    activated: "Aktiviert",
    deactivating: "Deaktivieren...",
    deactivate: "Deaktivieren",
    activating: "Aktivieren...",
    activate: "Aktivieren",
    notificationsBlocked: "Benachrichtigungen sind blockiert. Gehen Sie zu Ihren Browser-Einstellungen, um sie zu erlauben.",
    subscribedToPushNotifications: "Für Push-Benachrichtigungen angemeldet. Sie erhalten Updates über Veranstaltungen und Projekte.",
    // Profile
    pleaseSelectValidImageEs: "Por favor selecciona un archivo de imagen válido",
    pleaseSelectValidImageEn: "Please select a valid image file",
    pleaseSelectValidImageDe: "Bitte wählen Sie eine gültige Bilddatei aus",
    imageTooLargeEs: "La imagen es demasiado grande. Por favor selecciona una imagen menor a 5MB",
    imageTooLargeEn: "Image is too large. Please select an image smaller than 5MB",
    imageTooLargeDe: "Bild ist zu groß. Bitte wählen Sie ein Bild kleiner als 5MB aus",
    errorLoadingImageEs: "Error al cargar la imagen. Por favor intenta de nuevo.",
    errorLoadingImageEn: "Error loading image. Please try again.",
    errorLoadingImageDe: "Fehler beim Laden des Bildes. Bitte versuchen Sie es erneut.",
    // Events
    categoryEnvironment: "Umwelt",
    categoryEducation: "Bildung",
    categoryCommunity: "Gemeinschaft",
    categoryTechnology: "Technologie",
    pleaseSignInFirstEs: "Por favor inicia sesión primero",
    pleaseSignInFirstEn: "Please sign in first",
    pleaseSignInFirstDe: "Bitte zuerst anmelden",
    linkCopiedEs: "Enlace copiado.",
    linkCopiedEn: "Link copied.",
    linkCopiedDe: "Link kopiert.",
    savedEs: "Guardado",
    savedEn: "Saved",
    savedDe: "Gespeichert",
    saveEs: "Guardar",
    saveEn: "Save",
    saveDe: "Speichern",
    joinEvent: "Veranstaltung beitreten",
    fullyBooked: "Ausgebucht",
    shareEvent: "Veranstaltung teilen",
    // Header
    emailPlaceholderEs: "email@ejemplo.com",
    emailPlaceholderEn: "email@example.com",
    emailPlaceholderDe: "email@beispiel.de",
    profileSavedMessageEs: "Perfil guardado. Ingresa tu correo y presiona login para verificar.",
    profileSavedMessageEn: "Profile saved. Enter your email and press login to verify.",
    profileSavedMessageDe: "Profil gespeichert. Gib deine E‑Mail ein und klicke auf Login, um zu verifizieren.",
    oauthExtractMessageEs: "Se extraerán automáticamente: nombre, email y foto de perfil",
    oauthExtractMessageEn: "Automatically extracted: name, email and profile picture",
    oauthExtractMessageDe: "Automatisch extrahiert: Name, E-Mail und Profilbild",
    signinNotConfiguredEs: "Inicio de sesión no configurado",
    signinNotConfiguredEn: "Sign-in not configured",
    signinNotConfiguredDe: "Anmeldung nicht konfiguriert",
    googleOAuthTitleEs: "Google - Extrae nombre, email y foto",
    googleOAuthTitleEn: "Google - Extracts name, email and photo",
    googleOAuthTitleDe: "Google - Extrahiert Name, E-Mail und Foto",
    outlookOAuthTitleEs: "Outlook/Microsoft - Extrae nombre, email y foto",
    outlookOAuthTitleEn: "Outlook/Microsoft - Extracts name, email and photo",
    outlookOAuthTitleDe: "Outlook/Microsoft - Extrahiert Name, E-Mail und Foto",
    orGoDirectlyToEs: "O ve directamente a:",
    orGoDirectlyToEn: "Or go directly to:",
    orGoDirectlyToDe: "Oder direkt zu:",
    // App metadata
    appTitle: "EcoNexo - Nachhaltige Projekte in Europa",
    appDescription: "Verbinden Sie sich mit nachhaltigen Projekten und Veranstaltungen in ganz Europa. Treten Sie der Gemeinschaft bei, die eine grünere Zukunft aufbaut.",
    // About page
    aboutTitle: "Über EcoNexo",
    aboutSubtitle: "Europas Umweltgemeinschaft für eine nachhaltige Zukunft verbinden",
    aboutMissionTitle: "Unsere Mission",
    aboutMissionP1: "EcoNexo verbindet Umweltaktivisten, Freiwillige und Organisationen in ganz Europa, um eine nachhaltigere Zukunft zu schaffen. Wir glauben, dass Zusammenarbeit und Gemeinschaft der Schlüssel sind, um die Umweltprobleme unserer Zeit anzugehen.",
    aboutMissionP2: "Unsere Plattform ermöglicht es, Umweltprojekte zu entdecken, an Veranstaltungen teilzunehmen, Green Jobs zu finden und sich mit Gleichgesinnten zu vernetzen.",
    aboutOfferTitle: "Was wir anbieten",
    aboutOfferMapTitle: "Interaktive Karte",
    aboutOfferMapDesc: "Entdecke Umweltprojekte und Veranstaltungen in deiner Nähe mit unserer interaktiven Karte.",
    aboutOfferEventsTitle: "Veranstaltungen & Aktivitäten",
    aboutOfferEventsDesc: "Nimm an Umweltveranstaltungen, Workshops und Community‑Aktivitäten in ganz Europa teil.",
    aboutOfferJobsTitle: "Grüne Jobs",
    aboutOfferJobsDesc: "Finde sinnvolle Beschäftigungsmöglichkeiten im Umwelt- und Nachhaltigkeitssektor.",
    aboutOfferChatTitle: "Community‑Chat",
    aboutOfferChatDesc: "Vernetze dich über unsere Community‑Diskussionsforen mit anderen Umweltschützern.",
    aboutOfferProfilesTitle: "Persönliche Profile",
    aboutOfferProfilesDesc: "Erstelle detaillierte Profile, um deine Umweltinteressen und Expertise zu zeigen.",
    aboutOfferMultilingualTitle: "Mehrsprachig",
    aboutOfferMultilingualDesc: "Verfügbar auf Englisch, Spanisch und Deutsch für vielfältige europäische Gemeinschaften.",
    aboutValuesTitle: "Unsere Werte",
    aboutValueSustainabilityTitle: "Nachhaltigkeit",
    aboutValueSustainabilityDesc: "Wir fördern nachhaltige Praktiken und Umweltschutz in all unseren Aktivitäten und Partnerschaften.",
    aboutValueCommunityTitle: "Gemeinschaft",
    aboutValueCommunityDesc: "Wir glauben an die Kraft der Gemeinschaft und Zusammenarbeit für wirksamen Umweltschutz.",
    aboutValueInclusivityTitle: "Inklusivität",
    aboutValueInclusivityDesc: "Wir heißen Menschen aller Hintergründe und Erfahrungsstufen in unserer Umweltgemeinschaft willkommen.",
    aboutValueImpactTitle: "Wirkung",
    aboutValueImpactDesc: "Wir schaffen messbar positive Auswirkungen für Umwelt und Gesellschaft.",
    aboutImpactTitle: "Unsere Wirkung",
    aboutCommunityMembers: "Community‑Mitglieder",
    aboutEuropeanCities: "Europäische Städte",
    aboutGetInvolvedTitle: "Mach mit",
    aboutGetInvolvedDesc: "Bereit, etwas zu bewegen? Schließe dich unserer Community an und trage zu Umweltschutz bei.",
    // Event details
    backToEvents: "Zurück zu Veranstaltungen",
    date: "Datum",
    time: "Zeit",
    location: "Standort",
    contact: "Kontakt",
    volunteerProgress: "Freiwilligenfortschritt",
    spotsLeft: "Plätze übrig",
    requirements: "Anforderungen",
    benefits: "Vorteile",

    // Job form fields
    jobTitle: "Stellenbezeichnung",
    jobTitlePlaceholder: "Stellenbezeichnung eingeben",
    companyName: "Firmenname",
    companyNamePlaceholder: "Firmenname eingeben",
    jobLocation: "Arbeitsort",
    jobLocationPlaceholder: "Wo befindet sich die Stelle?",
    jobDescription: "Stellenbeschreibung",
    jobDescriptionPlaceholder: "Stellenverantwortlichkeiten beschreiben",
    requiredSkills: "Erforderliche Fähigkeiten",
    requiredSkillsPlaceholder: "Für die Stelle benötigte Fähigkeiten",
    benefitsPlaceholder: "Arbeitsvorteile",
    contactEmail: "Kontakt-E-Mail",
    contactEmailPlaceholder: "email@unternehmen.com",
    salaryRange: "Gehaltsspanne",
    salaryRangePlaceholder: "Angebotene Gehaltsspanne",
    jobType: "Arbeitsart",
    jobTypeFullTime: "Vollzeit",
    jobTypePartTime: "Teilzeit",
    jobTypeContract: "Vertrag",
    jobTypeInternship: "Praktikum",
    jobTypeRemote: "Remote",
    jobTypeHybrid: "Hybrid",
    experienceLevel: "Erfahrungslevel",
    experienceEntry: "Einsteiger",
    experienceMid: "Mittelstufe",
    experienceSenior: "Senior",
    experienceExpert: "Experte",
    applicationDeadline: "Bewerbungsschluss",
    createJobOffer: "Stellenausschreibung erstellen",
    saveDraft: "Entwurf speichern",
    publishJob: "Stelle veröffentlichen",
    jobCreated: "Stelle erstellt",

    // Onboarding
    onboardingWelcome: "Willkommen bei EcoNexo!",
    onboardingDescription: "Entdecken Sie nachhaltige Projekte und verbinden Sie sich mit einer Gemeinschaft, die sich für den Umweltschutz engagiert",
    onboardingMap: "Karte erkunden",
    onboardingMapDescription: "Visualisieren Sie nachhaltige Projekte in ganz Europa und finden Sie Möglichkeiten in Ihrer Nähe",
    onboardingExploreMap: "Karte erkunden",
    onboardingEvents: "Umweltveranstaltungen",
    onboardingEventsDescription: "Nehmen Sie an Freiwilligenveranstaltungen und Gemeinschaftsaktivitäten teil",
    onboardingViewEvents: "Veranstaltungen anzeigen",
    viewEvent: "Event anzeigen",
    viewDetails: "Details anzeigen",
    onboardingProjects: "Nachhaltige Projekte",
    onboardingProjectsDescription: "Entdecken Sie Initiativen, die die Welt verändern",
    onboardingCreateProject: "Projekt erstellen",
    onboardingJobs: "Grüne Jobs",
    onboardingJobsDescription: "Finden Sie Karrieremöglichkeiten im nachhaltigen Sektor",
    onboardingViewJobs: "Jobs anzeigen",
    onboardingCommunity: "Gemeinschaft",
    onboardingCommunityDescription: "Verbinden Sie sich mit Menschen, die Ihre Leidenschaft für die Umwelt teilen",
    onboardingJoinChat: "Chat beitreten",
    onboardingNext: "Weiter",
    onboardingSkip: "Überspringen",
    onboardingSelectLanguage: "Wählen Sie Ihre Sprache",
    onboardingLanguageDescription: "EcoNexo ist in mehreren Sprachen verfügbar",
    onboardingLanguageTip: "Sie können die Sprache jederzeit über das Menü ändern",
    // Welcome Intro
    welcomeIntroWelcomeTitle: "Willkommen bei EcoNexo!",
    welcomeIntroWelcomeDescription: "Ihre Plattform für nachhaltige Projekte in Europa",
    welcomeIntroLanguageTitle: "Sprache",
    welcomeIntroLanguageDescription: "Wählen Sie Ihre bevorzugte Sprache für eine bessere Erfahrung",
    welcomeIntroMapTitle: "Interaktive Karte",
    welcomeIntroMapDescription: "Erkunden Sie nachhaltige Projekte in ganz Europa",
    welcomeIntroEventsTitle: "Veranstaltungen",
    welcomeIntroEventsDescription: "Entdecken Sie Umweltveranstaltungen und Freiwilligenmöglichkeiten",
    welcomeIntroJobsTitle: "Jobs",
    welcomeIntroJobsDescription: "Finden Sie Karrieremöglichkeiten im grünen Sektor",
    welcomeIntroChatTitle: "Chat",
    welcomeIntroChatDescription: "Verbinden Sie sich mit der Umweltgemeinschaft",
    welcomeIntroProfileTitle: "Profil",
    welcomeIntroProfileDescription: "Verwalten Sie Ihr Konto und gespeicherte Projekte",
    welcomeIntroSkip: "Überspringen",
    welcomeIntroNext: "Weiter",
    welcomeIntroGetStarted: "Los geht's!",
    welcomeIntroFinish: "Beenden",
    // Profile fields
    personalInformation: "Persönliche Informationen",
    firstName: "Vorname",
    lastName: "Nachname",
    phone: "Telefon",
    city: "Stadt",
    country: "Land",
    aboutMe: "Über mich",
    passions: "Leidenschaften",
    areasOfExpertise: "Fachgebiete",
    hobbies: "Hobbys",
    interests: "Interessen",
    skills: "Fähigkeiten",
    languages: "Sprachen",
    socialMedia: "Soziale Medien",
    linkedin: "LinkedIn",
    twitter: "Twitter",
    instagram: "Instagram",
    firstNamePh: "Vorname eingeben",
    lastNamePh: "Nachname eingeben",
    phonePh: "Telefonnummer eingeben",
    aboutMePh: "Erzähle etwas über dich...",
    linkedinPh: "https://linkedin.com/in/dein-profil",
    twitterPh: "https://twitter.com/dein-benutzername",
    instagramPh: "https://instagram.com/dein-benutzername",
    websitePh: "https://deine-website.com",
    cityPh: "Stadtname",
    countryPh: "Landname",
    addressPh: "Adresse",
    titlePh: "Titel",
    website: "Website",
    useLogo: "Logo verwenden",
    passionsPlaceholder: "Wofür begeisterst du dich?",
    expertisePlaceholder: "Worin bist du Experte?",
    hobbiesPlaceholder: "Was sind deine Hobbys?",
    interestsPlaceholder: "Was interessiert dich?",
    skillsPlaceholder: "Welche Fähigkeiten hast du?",
    languagesPlaceholder: "Welche Sprachen sprichst du?",

    // OAuth Provider Information
    googleInformation: "Google-Informationen",
    microsoftInformation: "Microsoft-Informationen",
    providerInformation: "Anbieter-Informationen",
    extractedFromOAuth: "Diese Informationen wurden beim Anmelden automatisch aus Ihrem OAuth-Konto extrahiert.",
    calculatedAge: "Berechnetes Alter",
    years: "Jahre",
    verifiedEmail: "Bestätigte E-Mail",
    verified: "Verifiziert",
    preferredLanguage: "Bevorzugte Sprache",
    googleWorkspaceDomain: "Google Workspace-Domäne",
    tenantId: "Mandanten-ID",
    preferredUsername: "Bevorzugter Benutzername",
    previewLabel: "Vorschau",
    projectTitle: "Projekt-Titel",
    projectTitlePh: "Z.B.: Nachhaltige städtische Aufforstung",
    locationPh: "Stadt, Land",
    detailedDescription: "Detaillierte Beschreibung",
    detailedDescriptionPh: "Beschreibe Ziele, Auswirkungen und Methodik...",
    targetParticipants: "Zielteilnehmer",
    estimatedBudget: "Geschätztes Budget (€)",
    keyMilestones: "Wichtige Meilensteine",
    keyMilestonesPh: "- Start\n- Erste Implementierungsphase\n- Bewertung",
    accessibilityNeedsPh: "z. B. Rollstuhl, eingeschränkte Mobilität, etc.",
    writeMessagePh: "Nachricht schreiben...",
    roomDestroyed: "Raum zerstört",
    roomDestroyedDesc: "Dieser Raum wurde wie geplant selbstzerstört. Alle Nachrichten wurden dauerhaft gelöscht.",
    participantsCount: "{{count}} Teilnehmer",
    leaveRoom: "Raum verlassen",
    noMessagesYet: "Noch keine Nachrichten. Sei der Erste!",
    signInToSendMessage: "Melde dich an, um Nachrichten zu senden",
    goBack: "Zurück",
    searchConfiguration: "Suchkonfiguration",
    remoteWorkOnly: "Nur Fernarbeit",
    experienceYears: "Erfahrung (Jahre)",
    skillsCommas: "Fähigkeiten (kommagetrennt)",
    salaryMin: "Gehalt (Min)",
    matchingAgentWelcome: "Hallo! Ich bin dein intelligenter Matching-Assistent. Ich kann dir helfen, Projekte zu finden, die perfekt zu deinen Interessen und Präferenzen passen. Welche Art von Projekten interessieren dich?",
    matchingAgentError: "Entschuldigung, beim Suchen nach Projekten ist ein Fehler aufgetreten. Bitte versuche es erneut.",
    signInToUseMatching: "Bitte melde dich an, um den Matching-Assistenten zu verwenden.",
    suggestionsLabel: "Vorschläge:",
    typeSearchPh: "Schreibe deine Suche...",
    searchEventsPh: "Suche nach Veranstaltungen in deiner Stadt...",
    searchEventsTitlePh: "Titel, Ort, Organisator...",
    loadingCalendar: "Kalender wird geladen...",
    loadingEventsIndicator: "Veranstaltungen werden geladen...",
    showFiltersLabel: "Filter anzeigen",
    hideFiltersLabel: "Filter ausblenden",
    noEventsFound: "Keine Veranstaltungen gefunden, die den Filtern entsprechen",
    savingEllipsis: "Speichern...",
    creatingEllipsis: "Erstellen...",
    groupName: "Gruppenname",
    groupNamePh: "z.B. Berlin Nachhaltig",
    groupDescriptionPh: "Beschreibe den Zweck und die Ziele der Gruppe...",
    tagsLabel: "Tags",
    region: "Region",
    add: "Hinzufügen",
    addTagPh: "Tag hinzufügen...",
    publicGroup: "Öffentliche Gruppe (jeder kann beitreten)",
    backToGroups: "Zurück zu Gruppen",
    mustSignInToCreateGroup: "Du musst dich anmelden, um eine Gruppe zu erstellen",
    volunteersLabel: "Freiwillige",
    spotsLabel: "Plätze",

    // Project names and descriptions
    urbanReforestationBerlin: "Städtische Aufforstung Berlin",
    urbanReforestationDesc: "Nimm an unserer städtischen Aufforstungsinitiative in Berlin teil. Hilf beim Pflanzen von Bäumen und schaffe grüne Räume in der Stadt.",
    treesPlanted: "Bäume gepflanzt",
    seineRiverCleanup: "Seine-Fluss Reinigung",
    seineRiverCleanupDesc: "Nimm an der Reinigung der Seine in Paris teil. Hilf beim Entfernen von Abfall und schütze das Wasserleben.",
    wasteCollected: "Tonnen Abfall gesammelt",
    communityGardensMadrid: "Gemeinschaftsgärten Madrid",
    communityGardensDesc: "Engagiere dich in Gemeinschaftsgärten in Madrid. Lerne nachhaltige Landwirtschaft und verbinde dich mit Nachbarn.",
    communityGardensActive: "Gemeinschaftsgärten aktiv",

    // Eco Tips
    ecoTipCategoryFinance: "Finanzen",
    ecoTipsDescription: "Praktische Ideen, um deinen Fußabdruck zu senken und nachhaltig zu leben.",
    // Map filters/search
    filters: "Filter",
    search: "Suche",
    searchProjects: "Suche nach Projekten, Städten oder Ländern",
    categories: "Kategorien",
    onlyAvailableSpots: "Nur mit freien Plätzen",
    maxDistance: "Maximale Entfernung",
    anyDistance: "Beliebige Entfernung",
    dateRange: "Datumsbereich",

    type: "Typ",
    permanent: "Dauerhaft",
    frequency: "Häufigkeit",
    once: "Einmalig",
    regular: "Regelmäßig",
    centerOnLocation: "Auf meinen Standort zentrieren",
    clearFilters: "Filter zurücksetzen",
    ecoTipBankTitle: "Wähle nachhaltiges Banking",
    ecoTipBankDescription: "Wechsle zu Banken, die in erneuerbare Energien investieren und fossile Brennstoffe vermeiden.",
    ecoTipCategoryTransport: "Transport",
    ecoTipTransportTitle: "Nutze öffentliche Verkehrsmittel",
    ecoTipTransportDescription: "Reduziere deinen CO2-Fußabdruck durch die Nutzung von Bussen, Zügen und Fahrrädern statt Autos.",
    ecoTipCategoryFood: "Ernährung",
    ecoTipVegetarianTitle: "Probiere pflanzliche Mahlzeiten",
    ecoTipVegetarianDescription: "Reduziere den Fleischkonsum und probiere leckere vegetarische oder vegane Alternativen.",
    ecoTipCategoryEnergy: "Energie",
    ecoTipEnergyTitle: "Wechsle zu erneuerbaren Energien",
    ecoTipEnergyDescription: "Wähle Energieanbieter, die Solar-, Wind- oder Wasserkraft nutzen.",
    ecoTipCategoryWaste: "Abfall",
    ecoTipWasteTitle: "Reduzieren, Wiederverwenden, Recyceln",
    ecoTipWasteDescription: "Minimiere Abfall durch weniger Kaufen, Wiederverwenden von Gegenständen und ordnungsgemäßes Recyceln.",
    ecoTipCategoryWater: "Wasser",
    ecoTipWaterTitle: "Wasser sparen",
    ecoTipWaterDescription: "Nimm kürzere Duschen, repariere Lecks und nutze wassersparende Geräte.",
    ecoTipCategoryOceans: "Ozeane",
    ecoTipOceanTitle: "Meereslebewesen schützen",
    ecoTipOceanDescription: "Vermeiden Sie Einwegkunststoffe und verwenden Sie rifffreundliche Sonnencreme.",
    ecoTipCategoryHealth: "Gesundheit",
    ecoTipHealthTitle: "Natürliches Wohlbefinden",
    ecoTipHealthDescription: "Verwenden Sie umweltfreundliche Reinigungsmittel für eine bessere Luftqualität.",
    ecoTipCategoryEducation: "Bildung",
    ecoTipEducationTitle: "Lebenslanges Lernen",
    ecoTipEducationDescription: "Teilen Sie Ihr Nachhaltigkeitswissen mit Freunden und Familie für mehr Wirkung.",
    ecoTips: "Öko-Tipps",

    // Event creation
    createEvent: "Event erstellen",

    // Event administrators
    eventAdministrators: "Event-Administratoren",
    addAdministrator: "Administrator hinzufügen",
    administratorEmail: "Administrator-E-Mail",
    searchAdministrator: "Per E-Mail suchen",
    addAsAdministrator: "Als Administrator hinzufügen",
    removeAdministrator: "Administrator entfernen",
    manageAdministrators: "Administratoren verwalten",
    noAdministrators: "Keine Administratoren",
    youAreAdministrator: "Sie sind Administrator",
    eventCreator: "Event-Ersteller",
    administrators: "Administratoren",

    // Chat translations
    communityChat: "Community Chat",
    topicForums: "Themen-Foren",
    generalTopic: "Allgemein",
    topicDescriptionGeneral: "Allgemeine Diskussionen über Nachhaltigkeit",
    environmentTopic: "Umwelt",
    topicDescriptionEnvironment: "Umweltschutz und Klimaschutz",
    educationTopic: "Bildung",
    topicDescriptionEducation: "Umweltbildung und Bewusstsein",
    healthTopic: "Gesundheit",
    topicDescriptionHealth: "Verbindungen zwischen Gesundheit und Umwelt",
    communityTopic: "Gemeinschaft",
    topicDescriptionCommunity: "Gemeinschaftsaufbau und lokale Initiativen",
    oceansTopic: "Ozeane",
    topicDescriptionOceans: "Ozeanschutz und Meeresleben",
    foodTopic: "Ernährung",
    topicDescriptionFood: "Nachhaltige Ernährung und Landwirtschaft",
    chatMessage1: "Großartige Initiative! Wie kann ich mich beteiligen?",
    chatMessage2: "Ich arbeite an ähnlichen Projekten in meiner Gegend",
    chatMessage3: "Das ist genau das, was wir mehr brauchen!",
    chatMessage4: "Hat jemand diesen Ansatz schon einmal ausprobiert?",

    // Chat Rules
    chatRule1: "Respektiere alle Teilnehmer",
    chatRule2: "Halte Gespräche relevant",
    chatRule3: "Teile keine persönlichen Informationen",
    chatRule4: "Verwende angemessene Sprache",
    chatRule5: "Kein Spam",
    chatRule6: "Sei konstruktiv in deinen Kommentaren",
    chatRule7: "Melde unangemessene Inhalte",
    chatRule8: "Genieße und lerne",
    chatRules: "Chat-Regeln",
    chatRulesFooter: "Das Befolgen dieser Regeln schafft eine positive Gemeinschaft",
    chatRulesIntro: "Um eine positive Umgebung zu erhalten, bitte folge diesen Regeln:",
    chatRulesTitle: "Gemeinschaftsregeln",

    // Chat Recommendations
    recommendations: "Empfehlungen",
    popularDiscussions: "Beliebte Diskussionen",
    joinActiveConversations: "Tritt Aktiven Gesprächen Bei",
    newProjects: "Neue Projekte",
    discoverLatestInitiatives: "Entdecke Neueste Initiativen",
    eventsNearYou: "Veranstaltungen In Deiner Nähe",
    findLocalEvents: "Finde Lokale Veranstaltungen",

    // Category translations
    environment: "Umwelt",
    education: "Bildung",
    health: "Gesundheit",
    community: "Gemeinschaft",
    oceans: "Ozeane",
    food: "Ernährung",

    capital: "Hauptstadt",
    population: "Bevölkerung",
    currency: "Währung",
    regionLabel: "Region",
    governmentAPIAvailable: "Regierungs-API verfügbar",
    governmentAPIInDevelopment: "Regierungs-API in Entwicklung",
    integratedGovernmentAPIs: "Integrierte Regierungs-APIs",
    baseURL: "Basis-URL",

    // Additional event titles for calendar
    eventNativeTreePlanting: "Einheimische Baumpflanzung",
    eventSolarEnergyWorkshop: "Solarenergie Workshop",
    eventRiverCleanup: "Flussreinigung",
    eventClimateConference: "Klimawandel Konferenz",
    eventFoodConservationWorkshop: "Lebensmittelkonservierung Workshop",

    // Event titles
    eventVerticalGardens: "Vertikale Gärten Bau",
    eventEcoRace: "Umweltfreundlicher 5K-Lauf",
    eventSolarPanels: "Solarpanels Installation",
    eventEnvironmentalReflection: "Semester Umweltreflexion",
    eventSustainableSummer: "Nachhaltige Sommerfeier",
    eventAutumnPlanting: "Herbstpflanzung",
    eventForestCleanup: "Herbstwaldreinigung",
    eventBirdShelters: "Vogelschutz Bau",
    eventWindEnergy: "Windenergie Workshop",
    eventOrganicProducts: "Bio-Produkte Messe",
    eventWetlandsRestoration: "Feuchtgebiete Restaurierung",
    eventCommunityGardens: "Gemeinschaftsgärten Bau",
    eventSustainableChristmas: "Nachhaltiger Weihnachtsmarkt",
    eventWinterReforestation: "Winteraufforstung",
    eventClimateSeminar: "Klimawandel Seminar",
    eventEcoNewYear: "Umweltfreundliche Silvesterfeier",
    eventMarineConservation: "Meeresarten Schutz",
    eventHydroelectricWorkshop: "Wasserkraft Workshop",

    // Job titles
    jobTitle1: "Städtische Aufforstung Spezialist",
    jobTitle2: "Luftqualität Datenanalyst",
    jobTitle3: "Umweltpädagoge (STEM)",
    jobTitle4: "Flussreinigung Koordinator",
    jobTitle5: "Architekt Kreislaufwirtschaft",
    jobTitle6: "Meeresnaturschutz Spezialist",
    jobTitle7: "Spezialist für grüne Architektur",
    jobTitle8: "Forscher Kreislaufwirtschaft",
    jobTitle9: "Ingenieur Windparkbetrieb",

    // Job descriptions
    jobDesc1: "Leiten Sie städtische Aufforstungsinitiativen in Berlin, verwalten Sie Baumpflanzprojekte, koordinieren Sie mit lokalen Gemeinden und entwickeln Sie nachhaltige Grünflächen. Sie arbeiten mit Umweltexperten, Stadtplanern und Freiwilligen zusammen, um gesündere städtische Umgebungen zu schaffen.",
    jobDesc2: "Analysieren Sie Luftqualitätsdaten mit Python und GIS-Tools, überwachen Sie Sensornetzwerke in Paris und erstellen Sie Berichte für Umweltpolitiker. Sie arbeiten mit Datenwissenschaftlern und Umweltingenieuren zusammen, um Luftqualitätsüberwachungssysteme zu verbessern.",
    jobDesc3: "Entwickeln und liefern Sie STEM-basierte Umweltbildungsprogramme in Madrid, unterrichten Sie Studenten über Solarenergie, Robotik und Nachhaltigkeit. Sie erstellen ansprechende Lehrpläne und arbeiten mit Schulen zusammen, um Umweltbewusstsein zu fördern.",
    jobDesc4: "Koordinieren Sie Flussreinigungsoperationen in Mailand, verwalten Sie Freiwilligenteams, gewährleisten Sie Sicherheitsprotokolle und organisieren Sie Abfallsammlungsinitiativen. Sie arbeiten mit Umweltorganisationen und lokalen Behörden zusammen, um Wasserökosysteme zu schützen.",
    jobDesc5: "Entwerfen und implementieren Sie Kreislaufwirtschaftsstrategien für Unternehmen in Amsterdam. Verlängern Sie Produktlebenszyklen, reduzieren Sie Abfälle und schaffen Sie geschlossene Materialkreisläufe in Zusammenarbeit mit Kommunen und Start-ups.",
    jobDesc6: "Unterstützen Sie Meeresschutzprogramme ab Lissabon. Überwachen Sie die Biodiversität, analysieren Sie Daten von Unterwassersensoren und koordinieren Sie Citizen‑Science‑Strandbefragungen mit NGOs und Universitäten.",
    jobDesc7: "Leiten Sie nachhaltige Architekturprojekte in Amsterdam. Integrieren Sie Energieeffizienz, Passivhaus‑Design und CO₂‑arme Materialien (LEED/BREEAM). Arbeiten Sie eng mit Stadtplanung und Community zusammen.",
    jobDesc8: "Arbeiten Sie an einem Forschungsinstitut in Kopenhagen, um Kreislaufwirtschaft‑Pilotprojekte zu evaluieren. Quantifizieren Sie Auswirkungen (LCA), veröffentlichen Sie Ergebnisse und unterstützen Sie Politikempfehlungen.",
    jobDesc9: "Betreiben und optimieren Sie Windparks rund um Hamburg. Verantwortlich für Wartungspläne, SCADA‑Monitoring, Sicherheitsverfahren und Performance‑Verbesserungen mit OEM‑Partnern.",

    // Technical tags
    sensores: "Sensoren",
    silvicultura: "Forstwirtschaft",
    biodiversidad: "Biodiversität",
    gestióndeproyectos: "Projektmanagement",
    didáctica: "Pädagogik",
    energíasolar: "Solarenergie",
    robóticaeducativa: "Bildungsrobotik",
    gestióndevoluntariado: "Freiwilligenmanagement",
    residuos: "Abfall",
    seguridad: "Sicherheit",
    python: "Python",
    gis: "GIS",
    botánica: "Botanik",
    estadística: "Statistik",
    logística: "Logistik",
    machinelearning: "Maschinelles Lernen",
    comunicación: "Kommunikation",
    biologíamarina: "Meeresbiologie",
    conservación: "Naturschutz",
    investigación: "Forschung",
    buceo: "Tauchen",
    reactnative: "React Native",
    nodejs: "Node.js",
    blockchain: "Blockchain",
    iot: "IoT",
    uxui: "UX/UI",

    // Interface translations
    searchPlaceholderEn: "Nach Jobs suchen...",
    cityLabel: "Stadt",
    contractLabel: "Vertragstyp",
    contract_full_time: "Vollzeit",
    contract_part_time: "Teilzeit",
    contract_contract: "Vertrag",
    contract_internship: "Praktikum",
    remoteOnly: "Nur Remote",
    yearsExp: "Jahre Erfahrung",
    applyBtn: "Bewerben",
    saveBtn: "Speichern",
    jobPostingConditions: "Stellenausschreibungsbedingungen",
    jobPostingInfo: "Stellenausschreibungsinformationen",
    conductGuidelines: "Verhaltensrichtlinien",
    conductGuidelinesText: "Halten Sie eine respektvolle und konstruktive Umgebung aufrecht",

    // Language levels
    spanish: "Spanisch",
    english: "Englisch",
    german: "Deutsch",
    nativeLanguage: "Muttersprache",
    selectLevel: "Niveau auswählen",
    beginner: "Anfänger",
    intermediate: "Mittelstufe",
    advanced: "Fortgeschritten",
    fluent: "Fließend",
    selectLanguage: "Sprache auswählen",
    levelJunior: "Junior",
    levelMid: "Mid-Level",
    levelSenior: "Senior",
    levelLead: "Lead",

    // Admin panel titles
    adminPanelTitle: "Administrations-Panel",
    adminPanelDescription: "Verwalten Sie Veranstaltungen, Projekte und EcoNexo-Benutzer",
    adminTotalUsers: "Gesamtbenutzer",
    adminActiveEvents: "Aktive Veranstaltungen",
    adminProjects: "Projekte",
    adminGreenJobs: "Grüne Jobs",
    adminActiveUsers: "Aktive Benutzer",
    adminLast30Days: "Letzte 30 Tage",
    adminEventManagement: "Veranstaltungsverwaltung",
    adminProjectManagement: "Projektverwaltung",
    adminUserManagement: "Benutzerverwaltung",
    adminEvent: "Veranstaltung",
    adminDate: "Datum",
    adminParticipants: "Teilnehmer",
    adminStatus: "Status",
    adminActions: "Aktionen",
    adminProject: "Projekt",
    adminCreator: "Ersteller",
    adminVolunteers: "Freiwillige",
    adminActive: "Aktiv",
    adminCompleted: "Abgeschlossen",
    adminCancelled: "Storniert",
    adminDraft: "Entwurf",
    adminEdit: "Bearbeiten",
    adminDelete: "Löschen",
    adminApprove: "Genehmigen",
    adminOverview: "Übersicht",
    adminEvents: "Veranstaltungen",
    adminUsers: "Benutzer",
    adminUserManagementInDevelopment: "Benutzerverwaltungsfunktionalität in Entwicklung...",
    adminBeachCleanupBarcelona: "Strandreinigung Barcelona",
    adminTreePlantingMadrid: "Baumpflanzung Madrid",
    adminCommunityGardenValencia: "Gemeinschaftsgarten Valencia",
    adminSolarEnergyNeighborhood: "Solarenergie Nachbarschaft",

    // Gamification titles

    badgeEcoWarrior: "Öko-Krieger",
    badgeCommunityLeader: "Gemeinschaftsführer",
    badgeGreenJobHunter: "Grüner Job-Jäger",
    badgeChatMaster: "Chat-Meister",
    achievementEcoExplorer: "Öko-Entdecker",
    achievementProjectPioneer: "Projekt-Pionier",
    achievementCommunityConnector: "Gemeinschaftsverbinder",
    achievementCarbonNeutral: "Kohlenstoffneutral",

    // Admin page metadata
    adminPageTitle: "Administrations-Panel - EcoNexo",
    adminPageDescription: "Verwalten Sie Veranstaltungen, Projekte und EcoNexo-Benutzer",

    // Map titles
    centerOnMyLocation: "Auf meinen Standort zentrieren",

    // Social media titles
    socialMediaCommunityReforestationBerlin: "Gemeinschaftliche Aufforstung in Berlin",

    // Regions
    all: "Alle",
    northernEurope: "Nordeuropa",
    westernEurope: "Westeuropa",
    centralEurope: "Mitteleuropa",
    southernEurope: "Südeuropa",
    easternEurope: "Osteuropa",
    minExperience: "Mindestberufserfahrung",
    minSalary: "Mindestgehalt",
    allCities: "Alle",
    allContracts: "Alle",
    results: "Ergebnisse",
    calendar: "Kalender",
    month: "Monat",
    list: "Liste",
    applyForJob: "Für Job bewerben",
    yourName: "Dein Name",
    yourEmail: "Deine E‑Mail",
    cvLink: "CV‑Link (optional)",
    motivations: "Motivationsschreiben",
    expertiseAreas: "Fachgebiete",
    expertiseAreasPlaceholder: "Deine Fachgebiete",
    beVolunteer: "Freiwilliger werden",
    description: "Beschreibung",
    impact: "Auswirkung",
    raised: "gesammelt",
    reached: "erreicht",
    goal: "Ziel",
    volunteers: "Freiwillige",
    progressReached: "Fortschritt erreicht",
    donatePaypal: "Mit PayPal spenden",
    donateKlarna: "Mit Klarna spenden",
    donateStripe: "Mit Stripe spenden",
    backToMap: "Zurück zur Karte",
    moreInfo: "Mehr Info",
    rememberToTellThemTooltip: "Denken Sie daran, zu erwähnen, dass Sie von EcoNexo kommen!",
    // Auth Modal translations
    pleaseCompleteSecurityVerification: "Bitte vervollständigen Sie die Sicherheitsüberprüfung",
    errorSendingEmail: "Fehler beim Senden der E-Mail",
    unexpectedError: "Unerwarteter Fehler. Bitte versuchen Sie es erneut.",
    errorStartingGoogleAuth: "Fehler beim Starten der Google-Authentifizierung",
    errorStartingOutlookAuth: "Fehler beim Starten der Outlook-Authentifizierung",
    securityVerificationIncorrect: "Sicherheitsüberprüfung falsch. Bitte versuchen Sie es erneut.",
    welcomeEmailSent: "Willkommens-E-Mail gesendet! Bitte überprüfen Sie Ihren Posteingang und verifizieren Sie Ihr Konto, indem Sie auf den Link klicken.",
    authenticationSuccessful: "Authentifizierung erfolgreich!",
    orContinueWith: "Oder fortfahren mit",
    registerWith: "Registrieren mit",
    or: "Oder",
    emailPlaceholder: "ihre@email.com",
    errorInSecurityVerification: "Fehler bei der Sicherheitsüberprüfung",
    processing: "Wird verarbeitet...",
    signInButton: "Anmelden",
    createAccountButton: "Konto erstellen",
    bySigningIn: "Durch die Anmeldung akzeptieren Sie unsere",
    byRegistering: "Durch die Registrierung akzeptieren Sie unsere",
    termsOfService: "Nutzungsbedingungen",
    and: "und",
    privacyPolicy: "Datenschutzrichtlinie",
    // Captcha translations
    captchaNotConfigured: "reCAPTCHA nicht konfiguriert. Kontaktieren Sie den Administrator.",
    verificationCompleted: "Verifizierung abgeschlossen",
    securityVerification: "Sicherheitsüberprüfung:",
    yourAnswer: "Ihre Antwort",
    verify: "Überprüfen",
    howMuchIs: "Wie viel ist",
    // Gamification & Badges
    badgeFirstStep: "Erster Schritt",
    descFirstStep: "Erste Aktion abgeschlossen",
    badgeExplorer: "Entdecker",
    descExplorer: "Verschiedene Bereiche erkundet",
    badgeHelper: "Helfer",
    descHelper: "Der Gemeinschaft geholfen",
    descFirstEvent: "Erstes Event besucht",
    badgeEventEnthusiast: "Enthusiast",
    descEventEnthusiast: "5 Events besucht",
    badgeEventMaster: "Event-Meister",
    descEventMaster: "10 Events besucht",
    badgeEventCreator: "Organisator",
    descEventCreator: "Erstes Event erstellt",
    badgeProjectJoiner: "Mitarbeiter",
    descProjectJoiner: "Erstem Projekt beigetreten",
    badgeProjectCreator: "Schöpfer",
    descProjectCreator: "Erstes Projekt erstellt",
    badgeProjectVeteran: "Veteran",
    descProjectVeteran: "5 Projekte abgeschlossen",
    badgeConnector: "Verbinder",
    descConnector: "Erste Verbindung hergestellt",
    badgeMentor: "Mentor",
    descMentor: "Mentor geworden",
    badgeMarketplaceSeller: "Grüner Verkäufer",
    descMarketplaceSeller: "Im Marktplatz gepostet",
    badgeGreenCommuter: "Grüner Pendler",
    descGreenCommuter: "Nachhaltigen Transport genutzt",
    badgeWasteWarrior: "Abfall-Krieger",
    descWasteWarrior: "Abfall reduziert",
    badgeCarbonFighter: "CO2-Kämpfer",
    descCarbonFighter: "Fußabdruck ausgeglichen",
    badgeDailyStreak7: "Wöchentliche Serie",
    descDailyStreak7: "7 Tage in Folge",
    badgeDailyStreak30: "Monatliche Serie",
    descDailyStreak30: "30 Tage in Folge",
    badgeProfileComplete: "Profil vollständig",
    descProfileComplete: "Profil vervollständigt",
    badgeSocialSharer: "Teiler",
    descSocialSharer: "10 Mal geteilt",
    badgeCalendarSyncMaster: "Zeitmanager",
    descCalendarSyncMaster: "3 Events synchronisiert",

    // Onboarding Tour
    tourWelcomeTitle: "Willkommen bei EcoNexo!",
    tourWelcomeDesc: "Lassen Sie uns eine kurze Tour machen, um Ihnen den Einstieg zu erleichtern.",
    tourMapTitle: "Interaktive Karte",
    tourMapDesc: "Erkunden Sie Umweltprojekte, Veranstaltungen und lokale Gruppen in Ihrer Nähe.",
    tourCalendarTitle: "Veranstaltungskalender",
    tourCalendarDesc: "Finden und synchronisieren Sie Workshops, Aufräumaktionen und grüne Aktivitäten.",
    tourJobsTitle: "Grüne Jobs",
    tourJobsDesc: "Entdecken Sie Karrieremöglichkeiten im Nachhaltigkeitssektor.",
    tourChatTitle: "Community-Chat",
    tourChatDesc: "Vernetzen Sie sich mit anderen umweltbewussten Mitgliedern.",
    tourMatchingTitle: "Smart Matching",
    tourMatchingDesc: "Erhalten Sie personalisierte Empfehlungen, die zu Ihren Fähigkeiten passen.",
    tourCommunityTitle: "Gemeinschaft & Gruppen",
    tourCommunityDesc: "Treten Sie lokalen Gruppen bei, finden Sie Mentoren und nutzen Sie den Marktplatz.",
    tourProfileTitle: "Ihr Profil",
    tourProfileDesc: "Verfolgen Sie Ihren Einfluss, Abzeichen und Fortschritt.",
    tourAboutTitle: "Über Uns",
    tourAboutDesc: "Erfahren Sie mehr über unsere Mission und Vision.",
    tourNext: "Weiter",
    tourPrev: "Zurück",
    tourDone: "Fertig",

    // Impact Page UI
    yourImpact: "Dein Einfluss",
    impactDescription: "Sie bauen Schritt für Schritt eine grünere Zukunft auf. Hier sind Ihre Fortschritte und Erfolge bei EcoNexo.",
    xpToLevel: "EP bis Level",
    impactMedals: "Medaillen",
    impactStreakDays: "Serien-Tage",
    impactSocialImpact: "Sozialer Einfluss",
    impactShared: "geteilt",
    impactTalleres: "Workshops",
    impactCompleted: "abgeschlossen",
    impactCity: "Stadt",
    impactRanking: "Rangliste",
    impactBadgeCollection: "Medaillensammlung",
    impactRecentActivity: "Kürzliche Aktivität",
    impactNoActivity: "Keine jüngsten Aktivitäten.",
    impactJustNow: "Gerade eben",
    impactKeepGoing: "Weiter so!",
    impactUnlockNextLevel: "Nehmen Sie an weiteren Projekten und Veranstaltungen teil, um Level {{level}} freizuschalten.",
    impactExploreProjects: "Projekte erkunden",
    impactLevel: "Level",

    // Badge Categories
    categoryStarter: "Starter",
    categoryEngagement: "Engagement",
    categoryLifestyle: "Lebensstil",

    // Onboarding
    personalizeEcoNexo: "EcoNexo personalisieren",
    onboardingSubtitle: "Helfen Sie uns, die besten Projekte für Sie zu empfehlen.",
    causesTitle: "Welche Anliegen bewegen Sie?",
    skillsTitle: "Welche Fähigkeiten können Sie einbringen?",
    back: "Zurück",
    skip: "Überspringen",
    next: "Weiter",
    ready: "Fertig!",

    // Skills
    skillTech: "Technologie / IT",
    skillGardening: "Gärtnern / Feldarbeit",
    skillTeaching: "Bildung / Lehre",
    skillManual: "Handwerk / Reparatur",
    skillSocial: "Soziales / Pflege",
    skillArt: "Kunst / Kreativität",


    // Volunteer Page
    name: "Name",
    availability: "Verfügbarkeit",
    availabilityPh: "z.B. 2 Std/Woche",
    notes: "Notizen",
    save: "Speichern",
    saved: "Gespeichert",
    volunteersPageTitle: "Freiwilligenseite",
    localRecords: "Lokale Einträge",
    noVolunteersYet: "Noch keine Freiwilligen",
    backToProject: "Zurück zum Projekt",
    volunteeringPoints: "Projekt beitreten",
    infoNotAvailable: "Offizielle Website nicht verfügbar",
  },
  es: {
    menu: "Menú",

    app: "EcoNexo",
    map: "Mapa",
    events: "Eventos",
    jobs: "Trabajos",
    chat: "Chat",
    home: "Inicio",
    explore: "Explorar",
    workshops: "Talleres",
    badges: "Insignias",
    forum: "Foro",
    volunteering: "Trabajos Verdes",
    profile: "Perfil",
    signIn: "Iniciar sesión",
    signUp: "Registrarse",
    signOut: "Cerrar sesión",
    logout: "Cerrar sesión",
    welcomeBack: "¡Bienvenido de vuelta!",
    joinEcoNexo: "¡Únete a EcoNexo!",
    signInToContinue: "Inicia sesión para continuar",
    createAccountToStart: "Crea una cuenta para comenzar",
    checkEmailForLogin: "Revisa tu email para el enlace de inicio de sesión",
    checkEmailForVerification: "Revisa tu email para verificar tu cuenta",
    continueWithGoogle: "Continuar con Google",
    continueWithGitHub: "Continuar con GitHub",
    continueWithMicrosoft: "Continuar con Microsoft",
    dontHaveAccount: "¿No tienes cuenta?",
    alreadyHaveAccount: "¿Ya tienes cuenta?",
    signInToAccessProfile: "Inicia sesión para acceder a tu perfil",
    accessYourProfile: "Accede a tu perfil",
    signInToManageProfile: "Inicia sesión para gestionar tu perfil",
    whatYouCanDo: "Lo que puedes hacer:",
    managePersonalInfo: "Gestionar información personal",
    setPreferences: "Configurar preferencias",
    trackActivities: "Seguir actividades",
    connectWithCommunity: "Conectar con la comunidad",
    myAccount: "Mi cuenta",
    myEvents: "Mis eventos",
    myJobs: "Mis trabajos",
    supportUs: "Apóyanos",
    aboutUs: "Sobre Nosotros",
    createProfile: "Crear perfil",
    email: "Email",
    fullName: "Nombre completo",
    birthdate: "Fecha de nacimiento",
    birthPlace: "Lugar de nacimiento",
    pronouns: "Pronombres",
    gender: "Género",
    selectPronouns: "Seleccionar pronombres",
    pronounHe: "Él",
    pronounShe: "Ella",
    pronounThey: "Elle",
    heShe: "él/ella",
    theyThem: "they/them",
    erSie: "er/sie",
    other: "Otro",
    selectGender: "Seleccionar género",
    male: "Masculino",
    female: "Femenino",
    nonBinary: "No binario",
    preferNotToSay: "Prefiero no decir",
    enterEmail: "Ingresa tu email",
    sending: "Enviando...",
    loading: "Cargando...",
    authenticationRequired: "Autenticación requerida",
    signInToAccessThisPage: "Inicia sesión para acceder a esta página",
    whySignIn: "¿Por qué iniciar sesión?",
    personalizedExperience: "Experiencia personalizada",
    savePreferences: "Guardar preferencias",
    trackProgress: "Seguir progreso",
    emailNote: "Usaremos tu email para verificar tu cuenta",
    continueWith: "Continuar con",
    google: "Google",
    outlook: "Outlook",
    checkYourEmail: "Revisa tu email",
    cancel: "Cancelar",
    saveChanges: "Guardar cambios",
    editProfile: "Editar perfil",
    myProfile: "Mi perfil",
    profileUpdated: "Perfil actualizado exitosamente",
    changePhoto: "Cambiar foto",
    savingEs: "Guardando...",
    savingEn: "Saving...",
    savingDe: "Speichern...",
    // Page content
    welcomeMessageTitle: "Conecta con proyectos sostenibles en Europa",
    welcomeMessageDescription: "Únete a la comunidad que construye un futuro más verde. Descubre eventos, proyectos y oportunidades de voluntariado cerca de ti.",
    exploreEvents: "Explorar Eventos",
    findJobs: "Encontrar Trabajos",
    interactiveMap: "Mapa Interactivo",
    showMap: "Mostrar Mapa",
    hideMap: "Ocultar Mapa",
    activeProjects: "Proyectos Activos",
    volunteers: "Voluntarios",
    cities: "Ciudades",
    connectedVolunteers: "Voluntarios conectados",
    availableSpots: "Cupos disponibles",
    mapSearchPlaceholder: "Buscar ciudades, proyectos, lugares...",
    mapSearchPlaceholderGlobal: "Buscar ciudades, países, proyectos, lugares...",
    searchingGlobally: "Buscando en el mundo...",
    featuredProjects: "Proyectos Destacados",
    recommendedForYou: "Recomendado para ti",
    match: "Match",
    communityProjectsTitle: "Proyectos Comunitarios",
    proximityReason: "A solo {{distance}} km de ti",
    interestReason: "Coincide con tu interés en {{category}}",
    preferredTimeReason: "En tu horario preferido ({{time}})",
    difficultyReason: "Nivel de dificultad adecuado",
    noNearbyEvents: "No se encontraron eventos cercanos.",
    eventsNearby: "Eventos en los alrededores",
    viewAll: "Ver todos",
    socialMediaDetectionHeader: "Detección Automática de Proyectos",
    interactions: "interacciones",
    sustainable: "sostenible",
    approved: "Aprobar",
    rejected: "Rechazar",
    viewPost: "Ver post",
    detectionActive: "Detección activa",
    detectionPaused: "Detección pausada",
    monitoringSocialMedia: "Monitoreando redes sociales para proyectos ambientales verificados",
    scanningSocialMedia: "Escaneando redes sociales...",
    noNewProjects: "No se encontraron proyectos nuevos",
    scanning: "Escaneando",
    scan: "Escanear",
    detectionSettings: "Configuración de Detección",
    minFollowers: "Seguidores mínimos",
    minScore: "Puntuación mínima",
    communityProjectsPageDesc: "Descubre iniciativas impulsadas por la comunidad que están construyendo barrios más fuertes y sostenibles en toda Europa",
    totalVolunteers: "Voluntarios Totales",
    backToHome: "Volver al Inicio",
    basedOnPrefs: "Basado en tus preferencias y ubicación",
    excellent: "Excelente",
    veryGood: "Muy bueno",
    good: "Bueno",
    fair: "Regular",
    howRecsWork: "Cómo funcionan las recomendaciones",
    locationPrio: "Ubicación: Priorizamos eventos cerca de ti",
    interestsPrio: "Intereses: Basado en tus categorías favoritas",
    timesPrio: "Horarios: Consideramos tus momentos preferidos",
    historyPrio: "Historial: Aprendemos de eventos anteriores",
    availabilityPrio: "Disponibilidad: Solo eventos con plazas libres",
    environmentalProject: "Proyecto ambiental",
    educationalProject: "Proyecto educativo",
    healthServices: "Servicios de salud",
    communityProject: "Proyecto comunitario",
    foodProject: "Proyecto alimentario",
    activeProject: "Proyecto activo",
    exploreCategories: "Explorar Categorías",
    exploreDescription: "Descubre proyectos regenerativos, eventos comunitarios y oportunidades de impacto en tu zona.",
    featuredProjectsDesc: "Iniciativas que están cambiando Europa ahora mismo.",
    viewProjects: "Ver Proyectos",
    coursesAndWorkshops: "Cursos y Talleres",
    coursesAndWorkshopsDesc: "Aprende habilidades para un futuro sostenible.",
    exploreCourses: "Explorar Cursos",
    impactSearch: "Buscador de Impacto",
    searchPlaceholderExplore: "¿Qué buscas? (ej: Permacultura, Barcelona, Océanos...)",
    searchResults: "Resultados de búsqueda",
    found: "encontrados",
    noResultsFor: "No encontramos resultados para",
    clearSearch: "Limpiar búsqueda",
    haveInitiative: "¿Tienes una iniciativa?",
    registerInitiativeDesc: "Regístrala en EcoNexo y conecta con miles de voluntarios.",
    publishProject: "Publicar Proyecto",
    tagBiodiversity: "Biodiversidad",
    tagSolarEnergy: "Energía Solar",
    tagZeroWaste: "Zero Waste",
    tagReforestation: "Reforestación",
    ecoTipsTitle: "Consejos Ecológicos",
    joinCommunity: "Unirse a la Comunidad",
    readyToMakeDifference: "Vuélvete agente de cambio!",
    readyDescription: "Explora proyectos, únete a eventos y conecta con una comunidad comprometida con el futuro del planeta",
    letsGo: "¡Empezar!",
    viewMap: "Ver Mapa",
    addToGoogleCalendar: "Añadir a Google Calendar",
    addToOutlookCalendar: "Añadir a Outlook",
    downloadICS: "Descargar iCal",
    addToCalendar: "Añadir al Calendario",
    liveCommunityActivity: "Actividad de la Comunidad en Vivo",
    mentionHashtag: "#EcoNexo en Redes Sociales",
    questCompleted: "Misión Completada",
    questCompletedXP: "Recompensa de Misión",
    questOceanGuardian: "Guardián del Océano",
    questStepJoinOceans: "Únete a 2 eventos de conservación marina",
    questStepShareOcean: "Comparte un proyecto del océano",
    questUrbanSustainer: "Sustentador Urbano",
    questStepSyncEvents: "Sincroniza 3 eventos en tu calendario",
    questStepJoinWorkshop: "Asiste a un taller de huerto urbano",
    stats: "Estadísticas",
    quests: "Misiones",
    viewAllBadges: "Ver Todas las Insignias",
    communityRank: "Ranking Comunitario",
    karmaLabel: "Karma",
    impactLabel: "Impacto",
    dailyChallenges: "Retos Diarios",
    challengeGreenerPlanet: "Planeta Más Verde",
    challengeGreenerDesc: "Sincroniza 3 eventos esta semana",
    challengeEcoVoice: "Voz Eco",
    challengeEcoVoiceDesc: "Comparte 10 proyectos",
    activeQuestsLabel: "Misiones Activas",
    newBadge: "¡Nueva Insignia!",
    levelLabelShort: "NIV",
    noActiveQuests: "No hay misiones activas. ¡Mantente atento!",
    collectReward: "Recoger Recompensa",
    // Categories
    environmentTitle: "Medio Ambiente",
    environmentDescription: "Proyectos de conservación, reforestación y protección de ecosistemas",
    educationTitle: "Educación",
    educationDescription: "Iniciativas educativas y programas de formación ambiental",
    communityTitle: "Comunidad",
    communityDescription: "Proyectos de desarrollo comunitario y cohesión social",
    healthTitle: "Salud",
    healthDescription: "Iniciativas de salud comunitaria y bienestar",
    oceansTitle: "Océanos",
    oceansDescription: "Proyectos de protección marina y conservación acuática",
    foodTitle: "Alimentación",
    foodDescription: "Agricultura sostenible y sistemas alimentarios locales",
    // Welcome message values
    sustainabilityTitle: "Sostenibilidad",
    sustainabilityDescription: "Promovemos prácticas sostenibles y la protección ambiental en todas nuestras actividades y alianzas.",
    globalImpactTitle: "Impacto Global",
    globalImpactDescription: "Nuestros proyectos e iniciativas crean impactos positivos medibles para el medio ambiente y la sociedad a escala global.",
    innovationTitle: "Innovación",
    innovationDescription: "Utilizamos tecnología de vanguardia y soluciones innovadoras para abordar los desafíos ambientales y crear futuros sostenibles.",
    // Local Groups
    localGroupsSubtitle: "Conecta con comunidades locales en tu ciudad o región",
    becomeMentor: "Convertirse en Mentor",
    shareKnowledge: "Comparte tu conocimiento y apoya a aprendices en temas de sostenibilidad.",

    expertiseAreasPhMentoring: "Energía, Circularidad...",
    languagesPhMentoring: "Español, Inglés...",
    availabilityPhMentoring: "2 h/semana",
    offerMentoring: "Ofrecer Mentoring",
    createGroup: "Crear Grupo",
    allGroups: "Todos",
    myGroups: "Mis Grupos",
    nearbyGroups: "Cercanos",
    searchGroups: "Buscar grupos...",
    noGroupsFound: "No se encontraron grupos",
    createFirstGroup: "Crear el primer grupo",
    eventsLabel: "eventos",
    // Mock Groups
    groupBerlinName: "Berlín Sostenible",
    groupBerlinDescription: "Comunidad de activistas ambientales en Berlín",
    groupMadridName: "Madrid Verde",
    groupMadridDescription: "Iniciativas ecológicas en Madrid",
    // Labels
    volunteersLabelEs: "voluntarios",
    volunteersLabelEn: "volunteers",
    volunteersLabelDe: "Freiwillige",
    spotsLabelEs: "cupos",
    spotsLabelEn: "spots",
    spotsLabelDe: "Plätze",
    // Locations
    locationBerlin: "Berlín, Alemania",
    locationParis: "París, Francia",
    locationMadrid: "Madrid, España",
    // Notifications
    notificationsNotSupported: "Notificaciones no soportadas",
    browserNotSupportPush: "Tu navegador no soporta notificaciones push. Actualiza tu navegador o usa uno más moderno.",
    notificationSettings: "Configuración de Notificaciones",
    pushNotifications: "Notificaciones Push",
    receiveNotificationsAboutEvents: "Recibe notificaciones sobre nuevos eventos y proyectos",
    activated: "Activadas",
    deactivating: "Desactivando...",
    deactivate: "Desactivar",
    activating: "Activando...",
    activate: "Activar",
    notificationsBlocked: "Las notificaciones están bloqueadas. Ve a la configuración de tu navegador para permitirlas.",
    subscribedToPushNotifications: "Suscrito a notificaciones push. Recibirás actualizaciones sobre eventos y proyectos.",
    // Profile
    pleaseSelectValidImageEs: "Por favor selecciona un archivo de imagen válido",
    pleaseSelectValidImageEn: "Please select a valid image file",
    pleaseSelectValidImageDe: "Bitte wählen Sie eine gültige Bilddatei aus",
    imageTooLargeEs: "La imagen es demasiado grande. Por favor selecciona una imagen menor a 5MB",
    imageTooLargeEn: "Image is too large. Please select an image smaller than 5MB",
    imageTooLargeDe: "Bild ist zu groß. Bitte wählen Sie ein Bild kleiner als 5MB aus",
    errorLoadingImageEs: "Error al cargar la imagen. Por favor intenta de nuevo.",
    errorLoadingImageEn: "Error loading image. Please try again.",
    errorLoadingImageDe: "Fehler beim Laden des Bildes. Bitte versuchen Sie es erneut.",
    // Events
    categoryEnvironment: "Medio ambiente",
    categoryEducation: "Educación",
    categoryCommunity: "Comunidad",
    categoryTechnology: "Tecnología",
    welcomeToForum: "¡Bienvenido al foro de {{topic}}!",
    startDiscussionMsg: "Inicia una discusión o responde preguntas.",
    pleaseSignInFirstEs: "Por favor inicia sesión primero",
    pleaseSignInFirstEn: "Please sign in first",
    pleaseSignInFirstDe: "Bitte zuerst anmelden",
    linkCopiedEs: "Enlace copiado.",
    linkCopiedEn: "Link copied.",
    linkCopiedDe: "Link kopiert.",
    savedEs: "Guardado",
    savedEn: "Saved",
    savedDe: "Gespeichert",
    saveEs: "Guardar",
    saveEn: "Save",
    saveDe: "Speichern",
    joinEvent: "Unirse al evento",
    fullyBooked: "Completo",
    shareEvent: "Compartir evento",
    // Header
    emailPlaceholderEs: "email@ejemplo.com",
    emailPlaceholderEn: "email@example.com",
    emailPlaceholderDe: "email@beispiel.de",
    profileSavedMessageEs: "Perfil guardado. Ingresa tu correo y presiona login para verificar.",
    profileSavedMessageEn: "Profile saved. Enter your email and press login to verify.",
    profileSavedMessageDe: "Profil gespeichert. Gib deine E‑Mail ein und klicke auf Login, um zu verifizieren.",
    oauthExtractMessageEs: "Se extraerán automáticamente: nombre, email y foto de perfil",
    oauthExtractMessageEn: "Automatically extracted: name, email and profile picture",
    oauthExtractMessageDe: "Automatisch extrahiert: Name, E-Mail und Profilbild",
    signinNotConfiguredEs: "Inicio de sesión no configurado",
    signinNotConfiguredEn: "Sign-in not configured",
    signinNotConfiguredDe: "Anmeldung nicht konfiguriert",
    googleOAuthTitleEs: "Google - Extrae nombre, email y foto",
    googleOAuthTitleEn: "Google - Extracts name, email and photo",
    googleOAuthTitleDe: "Google - Extrahiert Name, E-Mail und Foto",
    outlookOAuthTitleEs: "Outlook/Microsoft - Extrae nombre, email y foto",
    outlookOAuthTitleEn: "Outlook/Microsoft - Extracts name, email and photo",
    outlookOAuthTitleDe: "Outlook/Microsoft - Extrahiert Name, E-Mail und Foto",
    orGoDirectlyToEs: "O ve directamente a:",
    orGoDirectlyToEn: "Or go directly to:",
    orGoDirectlyToDe: "Oder direkt zu:",
    // App metadata
    appTitle: "EcoNexo - Proyectos Sostenibles en Europa",
    appDescription: "Conecta con proyectos y eventos sostenibles en toda Europa. Únete a la comunidad que está construyendo un futuro más verde.",
    // About page
    aboutTitle: "Sobre EcoNexo",
    aboutSubtitle: "Conectando la comunidad ambiental de Europa para un futuro sostenible",
    aboutMissionTitle: "Nuestra Misión",
    aboutMissionP1: "EcoNexo está dedicado a conectar activistas, voluntarios y organizaciones ambientales de toda Europa para crear un futuro más sostenible. Creemos que la colaboración y la comunidad son clave para abordar los desafíos ambientales de nuestro tiempo.",
    aboutMissionP2: "Nuestra plataforma permite descubrir proyectos ambientales, unirse a eventos, encontrar empleos verdes y conectar con personas afines que comparten su pasión por la protección del medio ambiente.",
    aboutOfferTitle: "Qué Ofrecemos",
    aboutOfferMapTitle: "Mapa Interactivo",
    aboutOfferMapDesc: "Descubre proyectos y eventos ambientales cerca de ti con nuestro mapa interactivo.",
    aboutOfferEventsTitle: "Eventos y Actividades",
    aboutOfferEventsDesc: "Únete a eventos ambientales, talleres y actividades comunitarias en toda Europa.",
    aboutOfferJobsTitle: "Empleos Verdes",
    aboutOfferJobsDesc: "Encuentra oportunidades laborales significativas en el sector ambiental y de sostenibilidad.",
    aboutOfferChatTitle: "Chat de Comunidad",
    aboutOfferChatDesc: "Conéctate con otros ambientalistas en nuestros foros de discusión comunitarios.",
    aboutOfferProfilesTitle: "Perfiles Personales",
    aboutOfferProfilesDesc: "Crea perfiles detallados para mostrar tus intereses y experiencia ambiental.",
    aboutOfferMultilingualTitle: "Multilingüe",
    aboutOfferMultilingualDesc: "Disponible en inglés, español y alemán para servir a comunidades europeas diversas.",
    aboutValuesTitle: "Nuestros Valores",
    aboutValueSustainabilityTitle: "Sostenibilidad",
    aboutValueSustainabilityDesc: "Promovemos prácticas sostenibles y la protección ambiental en todas nuestras actividades y alianzas.",
    aboutValueCommunityTitle: "Comunidad",
    aboutValueCommunityDesc: "Creemos en el poder de la comunidad y la colaboración para generar cambios ambientales significativos.",
    aboutValueInclusivityTitle: "Inclusión",
    aboutValueInclusivityDesc: "Damos la bienvenida a personas de todos los orígenes y niveles para unirse a nuestra comunidad ambiental.",
    aboutValueImpactTitle: "Impacto",
    aboutValueImpactDesc: "Buscamos crear un impacto positivo medible para el medio ambiente y nuestras comunidades.",
    aboutImpactTitle: "Nuestro Impacto",
    aboutCommunityMembers: "Miembros de la comunidad",
    aboutEuropeanCities: "Ciudades europeas",
    aboutGetInvolvedTitle: "Involúcrate",
    aboutGetInvolvedDesc: "¿Listo para marcar la diferencia? Únete a nuestra comunidad y comienza a contribuir al cambio ambiental hoy.",
    // Event details
    backToEvents: "Volver a eventos",
    date: "Fecha",
    time: "Hora",
    location: "Ubicación",
    contact: "Contacto",
    volunteerProgress: "Progreso de voluntarios",
    spotsLeft: "cupos restantes",
    requirements: "Requisitos",
    benefits: "Beneficios",
    rememberToTellThemTooltip: "¡Recuerda decir que llegaste por EcoNexo!",
    // Onboarding
    onboardingWelcome: "¡Bienvenido a EcoNexo!",
    onboardingDescription: "Descubre proyectos sostenibles y conecta con una comunidad comprometida con el medio ambiente",
    onboardingMap: "Explora el Mapa",
    onboardingMapDescription: "Visualiza proyectos sostenibles en toda Europa y encuentra oportunidades cerca de ti",
    onboardingExploreMap: "Explorar Mapa",
    onboardingEvents: "Eventos Ambientales",
    onboardingEventsDescription: "Participa en eventos de voluntariado y actividades comunitarias",
    onboardingViewEvents: "Ver Eventos",
    onboardingProjects: "Proyectos Sostenibles",
    onboardingProjectsDescription: "Descubre iniciativas que están cambiando el mundo",
    onboardingCreateProject: "Crear Proyecto",
    onboardingJobs: "Trabajos Verdes",
    onboardingJobsDescription: "Encuentra oportunidades de carrera en el sector sostenible",
    onboardingViewJobs: "Ver Trabajos",
    onboardingCommunity: "Comunidad",
    onboardingCommunityDescription: "Conecta con personas que comparten tu pasión por el medio ambiente",
    onboardingJoinChat: "Unirse al Chat",
    onboardingNext: "Siguiente",
    onboardingSkip: "Saltar",
    onboardingSelectLanguage: "Selecciona tu idioma",
    onboardingLanguageDescription: "EcoNexo está disponible en múltiples idiomas",
    onboardingLanguageTip: "Puedes cambiar el idioma en cualquier momento desde el menú",
    // Welcome Intro
    welcomeIntroWelcomeTitle: "¡Bienvenido a EcoNexo!",
    welcomeIntroWelcomeDescription: "Tu plataforma para proyectos sostenibles en Europa",
    welcomeIntroLanguageTitle: "Idioma",
    welcomeIntroLanguageDescription: "Elige tu idioma preferido para una mejor experiencia",
    welcomeIntroMapTitle: "Mapa Interactivo",
    welcomeIntroMapDescription: "Explora proyectos sostenibles en toda Europa",
    welcomeIntroEventsTitle: "Eventos",
    welcomeIntroEventsDescription: "Descubre eventos ambientales y oportunidades de voluntariado",
    welcomeIntroJobsTitle: "Trabajos",
    welcomeIntroJobsDescription: "Encuentra oportunidades de carrera en el sector verde",
    welcomeIntroChatTitle: "Chat",
    welcomeIntroChatDescription: "Conecta con la comunidad ambiental",
    welcomeIntroProfileTitle: "Perfil",
    welcomeIntroProfileDescription: "Gestiona tu cuenta y proyectos guardados",
    welcomeIntroSkip: "Saltar",
    welcomeIntroNext: "Siguiente",
    welcomeIntroGetStarted: "¡Empezar!",
    welcomeIntroFinish: "Finalizar",
    // Profile fields
    personalInformation: "Información Personal",
    firstName: "Nombre",
    lastName: "Apellido",
    phone: "Teléfono",
    city: "Ciudad",
    country: "País",
    aboutMe: "Sobre mí",
    passions: "Pasiones",
    areasOfExpertise: "Áreas de Experiencia",
    hobbies: "Pasatiempos",
    interests: "Intereses",
    skills: "Habilidades",
    languages: "Idiomas",
    socialMedia: "Redes Sociales",
    linkedin: "LinkedIn",
    twitter: "Twitter",
    instagram: "Instagram",
    firstNamePh: "Ingresa tu nombre",
    lastNamePh: "Ingresa tu apellido",
    phonePh: "Ingresa tu teléfono",
    aboutMePh: "Cuéntanos sobre ti...",
    linkedinPh: "https://linkedin.com/in/tu-perfil",
    twitterPh: "https://twitter.com/tu-usuario",
    instagramPh: "https://instagram.com/tu-usuario",
    websitePh: "https://tu-sitio-web.com",
    website: "Sitio Web",
    useLogo: "Usar logo",
    passionsPlaceholder: "¿Qué te apasiona?",
    expertisePlaceholder: "¿En qué eres experto?",
    hobbiesPlaceholder: "¿Cuáles son tus pasatiempos?",
    interestsPlaceholder: "¿Qué te interesa?",
    skillsPlaceholder: "¿Qué habilidades tienes?",
    languagesPlaceholder: "¿Qué idiomas hablas?",

    // OAuth Provider Information
    googleInformation: "Información de Google",
    microsoftInformation: "Información de Microsoft",
    providerInformation: "Información del Proveedor",
    extractedFromOAuth: "Esta información fue extraída automáticamente de tu cuenta al iniciar sesión.",
    calculatedAge: "Edad calculada",
    years: "años",
    verifiedEmail: "Correo verificado",
    verified: "Verificado",
    preferredLanguage: "Idioma preferido",
    googleWorkspaceDomain: "Dominio Google Workspace",
    tenantId: "Tenant ID",
    preferredUsername: "Usuario preferido",
    projectTitle: "Título del Proyecto",
    projectTitlePh: "Ej: Reforestación Urbana Sostenible",
    locationPh: "Ciudad, País",
    detailedDescription: "Descripción Detallada",
    detailedDescriptionPh: "Describe los objetivos, impacto y metodología...",
    targetParticipants: "Meta de Participantes",
    estimatedBudget: "Presupuesto Estimado (€)",
    keyMilestones: "Hitos Principales",
    keyMilestonesPh: "- Lanzamiento\n- Primera fase de implementación\n- Evaluación",
    publishing: "Publicando...",
    errorSavingProject: "Error al guardar el proyecto",
    dietaryRestrictionsPh: "ej: Vegetariano, alergias, etc.",
    accessibilityNeedsPh: "ej: Silla de ruedas, movilidad reducida, etc.",
    writeMessagePh: "Escribe un mensaje...",
    roomDestroyed: "Sala Destruida",
    roomDestroyedDesc: "Esta sala se ha autodestruido según lo programado. Todos los mensajes han sido eliminados permanentemente.",
    participantsCount: "{{count}} participantes",
    leaveRoom: "Salir de la sala",
    noMessagesYet: "No hay mensajes aún. ¡Sé el primero en escribir!",
    signInToSendMessage: "Inicia sesión para enviar mensajes",
    goBack: "Volver",
    searchConfiguration: "Configuración de Búsqueda",
    remoteWorkOnly: "Solo trabajo remoto",
    experienceYears: "Experiencia (años)",
    skillsCommas: "Habilidades (separadas por comas)",
    salaryMin: "Salario (Mínimo)",
    matchingAgentWelcome: "¡Hola! Soy tu asistente de matching inteligente. Puedo ayudarte a encontrar proyectos que se ajusten perfectamente a tus intereses y preferencias. ¿Qué tipo de proyectos te interesan?",
    matchingAgentError: "Lo siento, hubo un error al buscar proyectos. Por favor, intenta de nuevo.",
    signInToUseMatching: "Por favor, inicia sesión para usar el asistente de matching.",
    suggestionsLabel: "Sugerencias:",
    typeSearchPh: "Escribe tu búsqueda...",
    searchEventsPh: "Buscar eventos en tu ciudad...",
    searchEventsTitlePh: "Título, ubicación, organizador...",
    loadingCalendar: "Cargando calendario...",
    loadingEventsIndicator: "Cargando eventos...",
    showFiltersLabel: "Mostrar Filtros",
    hideFiltersLabel: "Ocultar Filtros",
    noEventsFound: "No se encontraron eventos que coincidan con los filtros",
    savingEllipsis: "Guardando...",
    creatingEllipsis: "Creando...",
    groupName: "Nombre del Grupo",
    groupNamePh: "Ej: Berlín Sostenible",
    groupDescriptionPh: "Describe el propósito y objetivos del grupo...",
    tagsLabel: "Etiquetas",
    region: "Región",
    add: "Agregar",
    addTagPh: "Agregar etiqueta...",
    publicGroup: "Grupo público (cualquiera puede unirse)",
    backToGroups: "Volver a Grupos",
    mustSignInToCreateGroup: "Debes iniciar sesión para crear un grupo",
    // Traducciones automáticas generadas Sun Oct 19 11:54:11 CEST 2025
    address: "Dirección",
    addressPh: "Ingresa tu dirección",
    anyDistance: "Cualquier distancia",
    applicationDeadline: "Fecha límite de aplicación",
    applicationSent: "Aplicación enviada",
    apply: "Aplicar",
    applyBtn: "Aplicar",
    applyForJob: "Aplicar al trabajo",
    availability: "Disponibilidad",
    availabilityPh: "Tu disponibilidad",
    backToMap: "Volver al mapa",
    backToProject: "Volver al proyecto",
    beVolunteer: "Ser voluntario",
    benefitsPlaceholder: "Beneficios del trabajo",
    calendar: "Calendario",
    capacity: "Capacidad",
    capacityPh: "Capacidad del evento",
    categories: "Categorías",
    category: "Categoría",
    centerOnLocation: "Centrar en ubicación",
    type: "Tipo",
    permanent: "Permanente",
    frequency: "Frecuencia",
    once: "Una vez",
    regular: "Regularmente",
    chatRule1: "Respeta a todos los participantes",
    chatRule2: "Mantén las conversaciones relevantes",
    chatRule3: "No compartas información personal",
    chatRule4: "Usa un lenguaje apropiado",
    chatRule5: "No hagas spam",
    chatRule6: "Sé constructivo en tus comentarios",
    chatRule7: "Reporta contenido inapropiado",
    chatRule8: "Disfruta y aprende",
    chatRules: "Reglas del Chat",
    chatRulesFooter: "Siguiendo estas reglas creamos una comunidad positiva",
    chatRulesIntro: "Para mantener un ambiente positivo, por favor sigue estas reglas:",
    chatRulesTitle: "Reglas de la Comunidad",
    chats: "Chats",
    cityLabel: "Ciudad",
    cityPh: "Selecciona tu ciudad",
    clearFilters: "Limpiar filtros",
    clickToUpload: "Haz clic para subir",
    communityChat: "Chat Comunitario",
    topicForums: "Foros por Tema",
    generalTopic: "General",
    topicDescriptionGeneral: "Discusiones generales sobre sostenibilidad",
    environmentTopic: "Medio Ambiente",
    topicDescriptionEnvironment: "Protección ambiental y acción climática",
    educationTopic: "Educación",
    topicDescriptionEducation: "Educación ambiental y concienciación",
    healthTopic: "Salud",
    topicDescriptionHealth: "Conexiones entre salud y medio ambiente",
    communityTopic: "Comunidad",
    topicDescriptionCommunity: "Construcción comunitaria e iniciativas locales",
    oceansTopic: "Océanos",
    topicDescriptionOceans: "Conservación oceánica y vida marina",
    foodTopic: "Alimentación",
    topicDescriptionFood: "Alimentación sostenible y agricultura",
    chatMessage1: "¡Excelente iniciativa! ¿Cómo puedo participar?",
    chatMessage2: "He estado trabajando en proyectos similares en mi área",
    chatMessage3: "¡Esto es exactamente lo que necesitamos más!",
    chatMessage4: "¿Alguien ha probado este enfoque antes?",

    companyName: "Nombre de la empresa",
    companyNamePlaceholder: "Nombre de tu empresa",
    conductGuidelines: "Pautas de Conducta",
    conductGuidelinesText: "Mantén un ambiente respetuoso y constructivo",
    contactEmail: "Email de contacto",
    contactEmailPlaceholder: "email@empresa.com",
    contractLabel: "Tipo de contrato",
    contract_contract: "Contrato",
    contract_full_time: "Tiempo completo",
    contract_internship: "Prácticas",
    contract_part_time: "Medio tiempo",
    countryPh: "Selecciona tu país",
    createJobOffer: "Crear Oferta de Trabajo",

    // Language levels
    spanish: "Español",
    english: "Inglés",
    german: "Alemán",
    nativeLanguage: "Lengua materna",
    selectLevel: "Seleccionar nivel",
    beginner: "Principiante",
    intermediate: "Intermedio",
    advanced: "Avanzado",
    fluent: "Fluido",
    selectLanguage: "Selecciona un lenguaje",
    levelJunior: "Junior",
    levelMid: "Intermedio",
    levelSenior: "Senior",
    levelLead: "Líder",
    createdEvent: "Evento creado",
    currentTopic: "Tema actual",
    cvLink: "Enlace al CV",
    description: "Descripción",
    discoverLatestInitiatives: "Descubre las últimas iniciativas",
    donatePaypal: "Donar con PayPal",
    donateKlarna: "Donar con Klarna",
    donateStripe: "Donar con Stripe",
    moreInfo: "Más info",
    easy: "Fácil",
    ecoTipsDescription: "Consejos prácticos para vivir de manera más sostenible",
    eventsNearYou: "Eventos cerca de ti",
    existingEvents: "Eventos existentes",
    experienceEntry: "Nivel inicial",
    experienceExpert: "Experto",
    experienceLevel: "Nivel de experiencia",
    experienceMid: "Intermedio",
    experienceSenior: "Senior",
    expertiseAreas: "Áreas de experiencia",
    expertiseAreasPlaceholder: "Tus áreas de experiencia",
    filters: "Filtros",
    findLocalEvents: "Encuentra eventos locales",
    goal: "Objetivo",
    hard: "Difícil",
    highImpact: "Alto impacto",
    id: "ID",
    image: "Imagen",
    impact: "Impacto",
    jobCreated: "Trabajo creado",
    jobDescription: "Descripción del trabajo",
    jobDescriptionPlaceholder: "Describe las responsabilidades del puesto",
    jobLocation: "Ubicación del trabajo",
    jobLocationPlaceholder: "¿Dónde está ubicado el trabajo?",
    jobPostingConditions: "Condiciones de publicación",
    jobPostingInfo: "Información de publicación",
    jobTitle: "Título del trabajo",
    jobTitlePlaceholder: "Título del puesto",
    jobType: "Tipo de trabajo",
    jobTypeContract: "Por contrato",
    jobTypeFullTime: "Tiempo completo",
    jobTypeHybrid: "Híbrido",
    jobTypeInternship: "Prácticas",
    jobTypePartTime: "Medio tiempo",
    jobTypeRemote: "Remoto",
    join: "Unirse",
    joinActiveConversations: "Únete a conversaciones activas",
    list: "Lista",
    localRecords: "Registros locales",
    lowImpact: "Bajo impacto",
    mainCategory: "Categoría principal",
    maxDistance: "Distancia máxima",
    medium: "Medio",
    mediumImpact: "Medio impacto",
    minExperience: "Experiencia mínima",
    minSalary: "Salario mínimo",
    allCities: "Todas",
    allContracts: "Todos",
    results: "resultados",
    month: "Mes",
    motivationLetter: "Carta de motivación",
    motivations: "Motivaciones",
    motivationsPlaceholder: "¿Por qué quieres este trabajo?",
    name: "Nombre",
    newProjects: "Nuevos proyectos",
    noParticipatedEvents: "No has participado en eventos",
    noVolunteersYet: "Aún no hay voluntarios",
    notes: "Notas",
    notesPh: "Notas adicionales",
    onlineUsers: "Usuarios en línea",
    onlyAvailableSpots: "Solo cupos disponibles",
    onlyPdfFiles: "Solo archivos PDF",
    optional: "Opcional",
    optionalCategories: "Categorías opcionales",
    orContinueWith: "O continuar con",
    pdfOnly: "Solo PDF",
    popularDiscussions: "Discusiones populares",
    progressReached: "Progreso alcanzado",
    publishJob: "Publicar trabajo",
    raised: "recaudado",
    reached: "alcanzado",
    recommendations: "Recomendaciones",
    remoteOnly: "Solo remoto",
    removeFile: "Eliminar archivo",
    required: "Requerido",
    requiredSkills: "Habilidades requeridas",
    requiredSkillsPlaceholder: "Habilidades necesarias para el puesto",
    salaryRange: "Rango salarial",
    salaryRangePlaceholder: "Rango de salario ofrecido",
    satellite: "Satélite",
    save: "Guardar",
    saveBtn: "Guardar",
    saveDraft: "Guardar borrador",
    saved: "Guardado",
    search: "Buscar",
    searchProjects: "Buscar proyectos",
    send: "Enviar",
    sendApplication: "Enviar aplicación",
    sendLoginLink: "Enviar enlace de inicio de sesión",
    sendMessage: "Enviar mensaje",
    sendVerificationLink: "Enviar enlace de verificación",
    terrain: "Terreno",
    title: "Título",
    titlePh: "Título del evento",
    typeMessage: "Escribe un mensaje",
    user: "Usuario",
    viewDetails: "Ver detalles",
    viewEvent: "Ver evento",
    volunteersPageTitle: "Página de Voluntarios",
    yearsExp: "años de experiencia",
    yourEmail: "Tu email",
    yourName: "Tu nombre",
    volunteersLabel: "voluntarios",
    spotsLabel: "plazas",
    previewLabel: "Vista previa",
    infoNotAvailable: "Página oficial no disponible",

    // Project names and descriptions
    urbanReforestationBerlin: "Reforestación Urbana Berlín",
    urbanReforestationDesc: "Únete a nuestra iniciativa de reforestación urbana en Berlín. Ayuda a plantar árboles y crear espacios verdes en la ciudad.",
    treesPlanted: "árboles plantados",
    seineRiverCleanup: "Limpieza del Río Sena",
    seineRiverCleanupDesc: "Participa en la limpieza del río Sena en París. Ayuda a remover desechos y proteger la vida acuática.",
    wasteCollected: "toneladas de desechos recolectados",
    communityGardensMadrid: "Huertos Comunitarios Madrid",
    communityGardensDesc: "Involúcrate en huertos comunitarios en Madrid. Aprende agricultura sostenible y conéctate con vecinos.",
    communityGardensActive: "huertos comunitarios activos",

    // Eco Tips
    ecoTipCategoryFinance: "Finanzas",
    ecoTipBankTitle: "Elige Banca Sostenible",
    ecoTipBankDescription: "Cambia a bancos que invierten en energías renovables y evitan inversiones en combustibles fósiles.",
    ecoTipCategoryTransport: "Transporte",
    ecoTipTransportTitle: "Usa Transporte Público",
    ecoTipTransportDescription: "Reduce tu huella de carbono usando autobuses, trenes y bicicletas en lugar de autos.",
    ecoTipCategoryFood: "Alimentación",
    ecoTipVegetarianTitle: "Prueba Comidas Basadas en Plantas",
    ecoTipVegetarianDescription: "Reduce el consumo de carne y prueba deliciosas alternativas vegetarianas o veganas.",
    ecoTipCategoryEnergy: "Energía",
    ecoTipEnergyTitle: "Cambia a Energía Renovable",
    ecoTipEnergyDescription: "Elige proveedores de energía que usen energía solar, eólica o hidroeléctrica.",
    ecoTipCategoryWaste: "Residuos",
    ecoTipWasteTitle: "Reducir, Reutilizar, Reciclar",
    ecoTipWasteDescription: "Minimiza los residuos comprando menos, reutilizando artículos y reciclando materiales correctamente.",
    ecoTipCategoryWater: "Agua",
    ecoTipWaterTitle: "Conserva el Agua",
    ecoTipWaterDescription: "Toma duchas más cortas, repara fugas y usa electrodomésticos eficientes en agua.",
    ecoTipCategoryOceans: "Océanos",
    ecoTipOceanTitle: "Protege la Vida Marina",
    ecoTipOceanDescription: "Evita plásticos de un solo uso y usa protector solar biodegradable.",
    ecoTipCategoryHealth: "Salud",
    ecoTipHealthTitle: "Bienestar Natural",
    ecoTipHealthDescription: "Usa productos de limpieza ecológicos para mejorar la calidad del aire en casa.",
    ecoTipCategoryEducation: "Educación",
    ecoTipEducationTitle: "Aprendizaje Continuoc",
    ecoTipEducationDescription: "Comparte tus conocimientos sobre sostenibilidad con amigos y familiares.",
    ecoTips: "Consejos Ecológicos",

    // Event creation
    createEvent: "Crear Evento",

    // Event administrators
    eventAdministrators: "Administradores del Evento",
    addAdministrator: "Agregar Administrador",
    administratorEmail: "Correo del Administrador",
    searchAdministrator: "Buscar por correo",
    addAsAdministrator: "Agregar como Administrador",
    removeAdministrator: "Remover Administrador",
    manageAdministrators: "Gestionar Administradores",
    noAdministrators: "Sin administradores",
    youAreAdministrator: "Eres un administrador",
    eventCreator: "Creador del Evento",
    administrators: "Administradores",


    capital: "Capital",
    population: "Población",
    currency: "Moneda",
    regionLabel: "Región",
    governmentAPIAvailable: "API Gubernamental Disponible",
    governmentAPIInDevelopment: "API gubernamental en desarrollo",
    integratedGovernmentAPIs: "APIs Gubernamentales Integradas",
    baseURL: "URL Base",

    // Additional event titles for calendar
    eventNativeTreePlanting: "Plantación de árboles nativos",
    eventSolarEnergyWorkshop: "Taller de energía solar",
    eventRiverCleanup: "Limpieza de río",
    eventClimateConference: "Conferencia sobre cambio climático",
    eventFoodConservationWorkshop: "Taller de conservación de alimentos",

    // Event titles
    eventVerticalGardens: "Construcción de jardines verticales",
    eventEcoRace: "Carrera ecológica 5K",
    eventSolarPanels: "Instalación de paneles solares",
    eventEnvironmentalReflection: "Reflexión semestral ambiental",
    eventSustainableSummer: "Celebración de verano sostenible",
    eventAutumnPlanting: "Plantación de otoño",
    eventForestCleanup: "Limpieza de bosque otoñal",
    eventBirdShelters: "Construcción de refugios para aves",
    eventWindEnergy: "Taller de energía eólica",
    eventOrganicProducts: "Feria de productos orgánicos",
    eventWetlandsRestoration: "Restauración de humedales",
    eventCommunityGardens: "Construcción de jardines comunitarios",
    eventSustainableChristmas: "Mercado navideño sostenible",
    eventWinterReforestation: "Reforestación de invierno",
    eventClimateSeminar: "Seminario de cambio climático",
    eventEcoNewYear: "Celebración de fin de año ecológica",
    eventMarineConservation: "Conservación de especies marinas",
    eventHydroelectricWorkshop: "Taller de energía hidroeléctrica",

    // Job titles
    jobTitle1: "Especialista en Reforestación Urbana",
    jobTitle2: "Analista de Datos de Calidad del Aire",
    jobTitle3: "Educador/a ambiental (STEM)",
    jobTitle4: "Coordinador/a de Limpiezas de Ríos",
    jobTitle5: "Arquitecto/a de Economía Circular",
    jobTitle6: "Especialista en Conservación Marina",
    jobTitle7: "Especialista en Arquitectura Sostenible",
    jobTitle8: "Investigador/a en Economía Circular",
    jobTitle9: "Ingeniero/a de Operaciones Eólicas",

    // Job descriptions
    jobDesc1: "Lidera iniciativas de reforestación urbana en Berlín, gestionando proyectos de plantación de árboles, coordinando con comunidades locales y desarrollando espacios verdes sostenibles. Trabajarás con científicos ambientales, planificadores urbanos y voluntarios para crear entornos urbanos más saludables.",
    jobDesc2: "Analiza datos de calidad del aire usando Python y herramientas GIS, monitorea redes de sensores en París y crea informes para responsables de políticas ambientales. Trabajarás con científicos de datos e ingenieros ambientales para mejorar los sistemas de monitoreo de calidad del aire.",
    jobDesc3: "Desarrolla y entrega programas de educación ambiental basados en STEM en Madrid, enseñando a estudiantes sobre energía solar, robótica y sostenibilidad. Crearás currículos atractivos y trabajarás con escuelas para promover la conciencia ambiental.",
    jobDesc4: "Coordina operaciones de limpieza de ríos en Milán, gestionando equipos de voluntarios, asegurando protocolos de seguridad y organizando iniciativas de recolección de residuos. Trabajarás con organizaciones ambientales y autoridades locales para proteger los ecosistemas acuáticos.",
    jobDesc5: "Diseña e implementa estrategias de economía circular para empresas en Ámsterdam. Extiende el ciclo de vida de productos, reduce residuos y crea flujos de materiales en circuito cerrado en colaboración con municipios y startups.",
    jobDesc6: "Apoya programas de conservación marina desde Lisboa. Monitorea biodiversidad, analiza datos de sensores submarinos y coordina muestreos en playas con ONG y universidades.",
    jobDesc7: "Lidera proyectos de arquitectura sostenible en Ámsterdam. Integra eficiencia energética, diseño pasivo y materiales de bajo carbono (LEED/BREEAM). Coordina con urbanismo y actores comunitarios.",
    jobDesc8: "Únete a un instituto en Copenhague para evaluar pilotos de economía circular. Cuantifica impacto (ACV), publica resultados y apoya recomendaciones de política en los países nórdicos.",
    jobDesc9: "Opera y optimiza parques eólicos en torno a Hamburgo. Gestiona planes de mantenimiento, monitoreo SCADA, seguridad y mejoras de rendimiento con fabricantes (OEM).",

    // Technical tags
    sensores: "sensores",
    silvicultura: "silvicultura",
    biodiversidad: "biodiversidad",
    gestióndeproyectos: "gestión de proyectos",
    didáctica: "didáctica",
    energíasolar: "energía solar",
    robóticaeducativa: "robótica educativa",
    gestióndevoluntariado: "gestión de voluntariado",
    residuos: "residuos",
    seguridad: "seguridad",
    python: "Python",
    gis: "GIS",

    // Interface translations
    searchPlaceholderEn: "Buscar trabajos...",

    // Admin panel titles
    adminPanelTitle: "Panel de Administración",
    adminPanelDescription: "Gestiona eventos, proyectos y personas usuarias de EcoNexo",
    adminTotalUsers: "Usuarios Totales",
    adminActiveEvents: "Eventos Activos",
    adminProjects: "Proyectos",
    adminGreenJobs: "Empleos Verdes",
    adminActiveUsers: "Usuarios Activos",
    adminLast30Days: "Últimos 30 días",
    adminEventManagement: "Gestión de Eventos",
    adminProjectManagement: "Gestión de Proyectos",
    adminUserManagement: "Gestión de Usuarios",
    adminEvent: "Evento",
    adminDate: "Fecha",
    adminParticipants: "Participantes",
    adminStatus: "Estado",
    adminActions: "Acciones",
    adminProject: "Proyecto",
    adminCreator: "Creador",
    adminVolunteers: "Voluntarios",
    adminActive: "Activo",
    adminCompleted: "Completado",
    adminCancelled: "Cancelado",
    adminDraft: "Borrador",
    adminEdit: "Editar",
    adminDelete: "Eliminar",
    adminApprove: "Aprobar",
    adminOverview: "Resumen",
    adminEvents: "Eventos",
    adminUsers: "Usuarios",
    adminUserManagementInDevelopment: "Funcionalidad de gestión de personas usuarias en desarrollo...",
    adminBeachCleanupBarcelona: "Limpieza de Playa Barcelona",
    adminTreePlantingMadrid: "Plantación de Árboles Madrid",
    adminCommunityGardenValencia: "Huerto Comunitario Valencia",
    adminSolarEnergyNeighborhood: "Energía Solar Barrio",



    // Admin page metadata
    adminPageTitle: "Panel de Administración - EcoNexo",
    adminPageDescription: "Gestiona eventos, proyectos y personas usuarias de EcoNexo",

    // Map titles
    centerOnMyLocation: "Centrar en mi ubicación",

    // Community Page
    communityPageTitle: "Comunidad EcoNexo",
    tabFeed: "Feed",
    tabGroups: "Grupos",
    tabMentoring: "Mentoring",
    tabMarketplace: "Marketplace",
    recentActivity: "Actividad Reciente",
    noActivityYet: "No hay actividad aún",
    createdAnEvent: "creó un evento",
    localGroupsTitle: "Grupos Locales",
    createGroupBtn: "+ Crear Grupo",
    localGroupsDesc: "Conecta con comunidades locales en tu ciudad o región",
    viewAllGroups: "Ver Todos los Grupos",
    mentoringTitle: "Mentoring",
    mentoringDesc: "Conecta con expertos o encuentra aprendices en sostenibilidad",
    findMentorBtn: "Buscar Mentor",
    findMentorDesc: "Encuentra un experto que te guíe",
    becomeMentorBtn: "Convertirse en Mentor",
    shareKnowledgeDesc: "Comparte tu conocimiento",
    marketplaceTitle: "Marketplace Ecológico",
    publishBtn: "+ Publicar",

    karma: "Karma",
    reputation: "Reputación",
    followers: "Seguidores",
    quickLinks: "Enlaces Rápidos",
    myProfileLabel: "Mi Perfil",

    // Social media titles
    socialMediaCommunityReforestationBerlin: "Reforestación comunitaria en Berlín",

    // Regions
    all: "Todos",
    northernEurope: "Europa del Norte",
    westernEurope: "Europa Occidental",
    centralEurope: "Europa Central",
    southernEurope: "Europa del Sur",
    easternEurope: "Europa del Este",
    // Auth Modal translations
    pleaseCompleteSecurityVerification: "Por favor, completa la verificación de seguridad",
    errorSendingEmail: "Error al enviar el email",
    unexpectedError: "Error inesperado. Intenta de nuevo.",
    errorStartingGoogleAuth: "Error al iniciar autenticación con Google",
    errorStartingOutlookAuth: "Error al iniciar autenticación con Outlook",
    securityVerificationIncorrect: "Verificación de seguridad incorrecta. Intenta de nuevo.",
    welcomeEmailSent: "¡Email de bienvenida enviado! Por favor revisa tu bandeja de entrada y verifica tu cuenta haciendo clic en el enlace.",
    authenticationSuccessful: "¡Autenticación exitosa!",
    registerWith: "Regístrate con",
    or: "O",
    emailPlaceholder: "tu@email.com",
    errorInSecurityVerification: "Error en la verificación de seguridad",
    processing: "Procesando...",
    signInButton: "Iniciar Sesión",
    createAccountButton: "Crear Cuenta",
    bySigningIn: "Al iniciar sesión, aceptas nuestros",
    byRegistering: "Al registrarte, aceptas nuestros",
    termsOfService: "Términos de Servicio",
    and: "y",
    privacyPolicy: "Política de Privacidad",
    // Captcha translations
    captchaNotConfigured: "reCAPTCHA no configurado. Contacta al administrador.",
    verificationCompleted: "Verificación completada",
    securityVerification: "Verificación de seguridad:",
    yourAnswer: "Tu respuesta",
    verify: "Verificar",
    howMuchIs: "¿Cuánto es",
    // Gamification & Badges
    badgeFirstStep: "Primer Paso",
    descFirstStep: "Completaste tu primera acción",
    badgeExplorer: "Explorador",
    descExplorer: "Exploraste diferentes áreas",
    badgeHelper: "Ayudante",
    descHelper: "Ayudaste a la comunidad",
    descFirstEvent: "Asististe a tu primer evento",
    badgeEventEnthusiast: "Entusiasta",
    descEventEnthusiast: "Asististe a 5 eventos",
    badgeEventMaster: "Maestro de Eventos",
    descEventMaster: "Asististe a 10 eventos",
    badgeEventCreator: "Organizador",
    descEventCreator: "Creaste tu primer evento",
    badgeProjectJoiner: "Colaborador",
    descProjectJoiner: "Te uniste a tu primer proyecto",
    badgeProjectCreator: "Creador",
    descProjectCreator: "Creaste tu primer proyecto",
    badgeProjectVeteran: "Veterano",
    descProjectVeteran: "Completaste 5 proyectos",
    badgeConnector: "Conector",
    descConnector: "Hiciste tu primera conexión",
    badgeMentor: "Mentor",
    descMentor: "Te convertiste en mentor",
    badgeMarketplaceSeller: "Vendedor Verde",
    descMarketplaceSeller: "Publicaste en el marketplace",
    badgeGreenCommuter: "Transporte Verde",
    descGreenCommuter: "Usaste transporte sostenible",
    badgeWasteWarrior: "Guerrero Residuos",
    descWasteWarrior: "Redujiste residuos",
    badgeCarbonFighter: "Luchador Carbono",
    descCarbonFighter: "Compensaste tu huella",
    badgeDailyStreak7: "Racha Semanal",
    descDailyStreak7: "7 días consecutivos",
    badgeDailyStreak30: "Racha Mensual",
    descDailyStreak30: "30 días consecutivos",
    badgeProfileComplete: "Perfil Completo",
    descProfileComplete: "Completaste tu perfil",
    badgeSocialSharer: "Compartidor",
    descSocialSharer: "Compartiste 10 veces",
    badgeCalendarSyncMaster: "Gestor del Tiempo",
    descCalendarSyncMaster: "Sincronizó 3 eventos",

    // Onboarding Tour
    tourWelcomeTitle: "¡Bienvenido a EcoNexo!",
    tourWelcomeDesc: "Hagamos un recorrido rápido para ayudarte a comenzar tu viaje sostenible.",
    tourMapTitle: "Mapa Interactivo",
    tourMapDesc: "Explora proyectos ambientales, eventos y grupos locales cerca de ti.",
    tourCalendarTitle: "Calendario de Eventos",
    tourCalendarDesc: "Encuentra y sincroniza talleres, limpiezas y actividades verdes.",
    tourJobsTitle: "Empleos Verdes",
    tourJobsDesc: "Descubre oportunidades laborales en el sector de la sostenibilidad.",
    tourChatTitle: "Chat Comunitario",
    tourChatDesc: "Conéctate con otros miembros y debate sobre temas ambientales.",
    tourMatchingTitle: "Matching Inteligente",
    tourMatchingDesc: "Recibe recomendaciones personalizadas según tus habilidades.",
    tourCommunityTitle: "Comunidad y Grupos",
    tourCommunityDesc: "Únete a grupos locales, encuentra mentores y usa el mercado.",
    tourProfileTitle: "Tu Perfil",
    tourProfileDesc: "Rastrea tu impacto, insignias y progreso.",
    tourAboutTitle: "Sobre Nosotros",
    tourAboutDesc: "Conoce más sobre nuestra misión y visión.",
    tourNext: "Siguiente",
    tourPrev: "Anterior",
    tourDone: "Finalizar",

    // Impact Page UI
    yourImpact: "Tu Impacto",
    impactDescription: "Estás construyendo un futuro más verde, paso a paso. Aquí está tu progreso y logros en EcoNexo.",
    xpToLevel: "XP para Nivel",
    impactMedals: "Medallas",
    impactStreakDays: "Días Racha",
    impactSocialImpact: "Impacto Social",
    impactShared: "compartidos",
    impactTalleres: "Talleres",
    impactCompleted: "completados",
    impactCity: "Ciudad",
    impactRanking: "Ranking",
    impactBadgeCollection: "Colección de Medallas",
    impactRecentActivity: "Actividad Reciente",
    impactNoActivity: "No hay actividad reciente.",
    impactJustNow: "Hace un momento",
    impactKeepGoing: "¡Sigue sumando!",
    impactUnlockNextLevel: "Participa en más proyectos y eventos para desbloquear el nivel {{level}}.",
    impactExploreProjects: "Explorar Proyectos",
    impactLevel: "Nivel",

    // Badge Categories
    categoryStarter: "Inicio",
    categoryEngagement: "Compromiso",
    categoryLifestyle: "Estilo de Vida",

    // Onboarding
    personalizeEcoNexo: "Personaliza EcoNexo",
    onboardingSubtitle: "Ayúdanos a recomendarte los mejores proyectos.",
    causesTitle: "¿Qué causas te mueven?",
    skillsTitle: "¿Qué habilidades puedes aportar?",
    back: "Atrás",
    skip: "Saltar",
    next: "Siguiente",
    ready: "¡Listo!",

    // Skills
    skillTech: "Tecnología / IT",
    skillGardening: "Jardinería / Campo",
    skillTeaching: "Educación / Enseñanza",
    skillManual: "Trabajo Manual / Reparación",
    skillSocial: "Trabajo Social / Cuidados",
    skillArt: "Arte / Creatividad",

  },
  fr: {
    // Job titles
    jobTitle1: "Spécialiste en Reboisement Urbain",
    jobTitle2: "Analyste de Données de Qualité de l'Air",
    jobTitle3: "Éducateur Environnemental (STEM)",
    jobTitle4: "Coordinateur de Nettoyage de Rivières",

    // Job descriptions
    jobDesc1: "Dirigez les initiatives de reboisement urbain à Berlin, gérez les projets de plantation d'arbres, coordonnez avec les communautés locales et développez des espaces verts durables. Vous travaillerez avec des scientifiques environnementaux, des urbanistes et des bénévoles pour créer des environnements urbains plus sains.",
    jobDesc2: "Analysez les données de qualité de l'air en utilisant Python et les outils GIS, surveillez les réseaux de capteurs à Paris et créez des rapports pour les décideurs politiques environnementaux. Vous travaillerez avec des scientifiques de données et des ingénieurs environnementaux pour améliorer les systèmes de surveillance de la qualité de l'air.",
    jobDesc3: "Développez et livrez des programmes d'éducation environnementale basés sur STEM à Madrid, enseignez aux étudiants l'énergie solaire, la robotique et la durabilité. Vous créerez des programmes d'études engageants et travaillerez avec les écoles pour promouvoir la sensibilisation environnementale.",
    jobDesc4: "Coordonnez les opérations de nettoyage de rivières à Milan, gérez les équipes de bénévoles, assurez les protocoles de sécurité et organisez les initiatives de collecte de déchets. Vous travaillerez avec des organisations environnementales et les autorités locales pour protéger les écosystèmes aquatiques.",

    // Technical tags
    sensores: "capteurs",
    silvicultura: "sylviculture",
    biodiversidad: "biodiversité",
    gestióndeproyectos: "gestion de projet",
    didáctica: "pédagogie",
    energíasolar: "énergie solaire",
    robóticaeducativa: "robotique éducative",
    gestióndevoluntariado: "gestion du bénévolat",
    residuos: "déchets",
    seguridad: "sécurité",
    python: "Python",
    gis: "GIS",

    // Interface translations
    searchPlaceholderEn: "Rechercher des emplois...",
    cityLabel: "Ville",
    contractLabel: "Type de contrat",
    contract_full_time: "Temps plein",
    contract_part_time: "Temps partiel",
    contract_contract: "Contrat",
    contract_internship: "Stage",
    remoteOnly: "Télétravail uniquement",
    yearsExp: "années d'expérience",
    applyBtn: "Postuler",
    saveBtn: "Sauvegarder",
    jobPostingConditions: "Conditions de Publication",
    jobPostingInfo: "Informations de publication",
    conductGuidelines: "Directives de Conduite",
    conductGuidelinesText: "Maintenez un environnement respectueux et constructif",
    createJobOffer: "Créer une Offre d'Emploi",

    // Chat Rules
    chatRules: "Règles Du Chat",
    chatRule1: "Respecte Tous Les Participants",
    chatRule2: "Garde Les Conversations Pertinentes",
    chatRule3: "Ne Partage Pas D'Informations Personnelles",
    chatRule4: "Utilise Un Langage Approprié",
    chatRule5: "Pas De Spam",
    chatRule6: "Sois Constructif Dans Tes Commentaires",
    chatRule7: "Signale Le Contenu Inapproprié",
    chatRule8: "Profite Et Apprends",
    chatRulesFooter: "Suivre Ces Règles Crée Une Communauté Positive",
    chatRulesIntro: "Pour Maintenir Un Environnement Positif, Veuillez Suivre Ces Règles:",
    chatRulesTitle: "Règles De La Communauté",

    // Chat Recommendations
    recommendations: "Recommandations",
    popularDiscussions: "Discussions Populaires",
    joinActiveConversations: "Rejoins Les Conversations Actives",
    newProjects: "Nouveaux Projets",
    discoverLatestInitiatives: "Découvre Les Dernières Initiatives",
    eventsNearYou: "Événements Près De Toi",
    findLocalEvents: "Trouve Des Événements Locaux",
    menu: "Menu",
    app: "ÉcoNexo",
    map: "Carte",
    events: "Événements",
    jobs: "Emplois",
    chat: "Chat",
    home: "Maison",
    explore: "Explorer",
    workshops: "Ateliers",
    badges: "Insignes",
    forum: "Forum",
    volunteering: "Volontariat",
    profile: "Profil",
    signIn: "Se connecter",
    signUp: "S'inscrire",
    signOut: "Se déconnecter",
    logout: "Déconnexion",
    welcomeBack: "Content de te revoir!",
    joinEcoNexo: "Rejoignez EcoNexo !",
    signInToContinue: "Connectez-vous pour continuer",
    createAccountToStart: "Créez un compte pour commencer",
    checkEmailForLogin: "Vérifiez votre courrier électronique pour le lien de connexion",
    checkEmailForVerification: "Vérifiez votre courrier électronique pour vérifier votre compte",
    continueWithGoogle: "Continuer avec Google",
    continueWithGitHub: "Continuer avec GitHub",
    continueWithMicrosoft: "Continuer avec Microsoft",
    dontHaveAccount: "Vous n'avez pas de compte ?",
    alreadyHaveAccount: "Vous avez déjà un compte ?",
    signInToAccessProfile: "Connectez-vous pour accéder à votre profil",
    accessYourProfile: "Accédez à votre profil",
    signInToManageProfile: "Connectez-vous pour gérer votre profil",
    whatYouCanDo: "Ce que vous pouvez faire :",
    managePersonalInfo: "Gérer les informations personnelles",
    setPreferences: "Définir les préférences",
    trackActivities: "Suivre les activités",
    connectWithCommunity: "Connectez-vous avec la communauté",
    myAccount: "Mon compte",
    myEvents: "Mes événements",
    myJobs: "Mes emplois",
    supportUs: "Soutenez-nous",
    aboutUs: "À propos de nous",
    createProfile: "Créer un profil",
    email: "E-mail",
    fullName: "Nom et prénom",
    birthdate: "Date de naissance",
    birthPlace: "Lieu de naissance",
    pronouns: "Pronoms",
    gender: "Genre",
    selectPronouns: "Sélectionnez des pronoms",
    pronounHe: "Il",
    pronounShe: "Elle",
    pronounThey: "Ils",
    heShe: "il/elle",
    theyThem: "ils/eux",
    erSie: "euh/sie",
    other: "Autre",
    selectGender: "Sélectionnez le sexe",
    male: "Mâle",
    female: "Femelle",
    nonBinary: "Non binaire",
    preferNotToSay: "Je préfère ne pas dire",
    enterEmail: "Entrez votre email",
    sending: "Envoi...",
    loading: "Chargement...",
    authenticationRequired: "Authentification requise",
    signInToAccessThisPage: "Connectez-vous pour accéder à cette page",
    whySignIn: "Pourquoi se connecter ?",
    personalizedExperience: "Expérience personnalisée",
    savePreferences: "Enregistrer les préférences",
    trackProgress: "Suivre les progrès",
    emailNote: "Nous utiliserons votre e-mail pour vérifier votre compte",
    continueWith: "Continuez avec",
    google: "Google",
    outlook: "Perspectives",
    checkYourEmail: "Vérifiez votre courrier électronique",
    cancel: "Annuler",
    saveChanges: "Enregistrer les modifications",
    editProfile: "Modifier le profil",
    myProfile: "Mon profil",
    profileUpdated: "Profil mis à jour avec succès",
    changePhoto: "Changer de photo",
    savingEs: "Garder...",
    savingEn: "Économie...",
    savingDe: "Speichern....",
    welcomeMessageTitle: "Connectez-vous avec des projets durables en Europe",
    welcomeMessageDescription: "Rejoignez la communauté pour construire un avenir plus vert. Découvrez les événements, les projets et les opportunités de bénévolat près de chez vous.",
    exploreEvents: "Explorer les événements",
    findJobs: "Trouver un emploi",
    interactiveMap: "Carte interactive",
    showMap: "Afficher la carte",
    hideMap: "Masquer la carte",
    activeProjects: "Projets actifs",
    cities: "Villes",
    connectedVolunteers: "Des bénévoles connectés",
    availableSpots: "Places disponibles",
    mapSearchPlaceholder: "Rechercher des villes, des projets, du monde...",
    mapSearchPlaceholderGlobal: "Rechercher des villes, des pays, des projets, des lieux...",
    searchingGlobally: "Recherche à l'échelle mondiale...",
    featuredProjects: "Projets en vedette",
    recommendedForYou: "Recommandé pour vous",
    match: "Correspondre",
    communityProjectsTitle: "Projets communautaires",
    proximityReason: "À seulement {{distance}} km de chez vous",
    interestReason: "Correspond à votre intérêt pour {{category}}",
    preferredTimeReason: "À l'heure de votre choix ({{time}})",
    difficultyReason: "Niveau de difficulté approprié",
    noNearbyEvents: "Aucun événement à proximité trouvé",
    eventsNearby: "Événements à proximité",
    viewAll: "Voir tout",
    socialMediaDetectionHeader: "Détection automatique des projets",
    interactions: "interactions",
    sustainable: "durable",
    approved: "Approuver",
    rejected: "Rejeter",
    viewPost: "Voir le message",
    detectionActive: "Détection active",
    detectionPaused: "Détection interrompue",
    monitoringSocialMedia: "Surveillance des médias sociaux pour les projets environnementaux vérifiés",
    scanningSocialMedia: "Scanner les réseaux sociaux...",
    noNewProjects: "Aucun nouveau projet trouvé",
    scanning: "Balayage",
    scan: "Balayage",
    detectionSettings: "Paramètres de détection",
    minFollowers: "Nombre minimum d'abonnés",
    minScore: "Note minimale",
    communityProjectsPageDesc: "Découvrez des initiatives communautaires qui construisent des quartiers plus forts et plus durables à travers l'Europe.",
    totalVolunteers: "Total des bénévoles",
    backToHome: "Retour à la maison",
    basedOnPrefs: "En fonction de vos préférences et de votre emplacement",
    excellent: "Excellent",
    veryGood: "Très bien",
    communityPageTitle: "Communauté EcoNexo",
    tabFeed: "Alimentation",
    tabGroups: "Groupes",
    tabMentoring: "Mentorat",
    tabMarketplace: "Marché",
    recentActivity: "Activité récente",
    noActivityYet: "Aucune activité pour le moment",
    createdAnEvent: "créé un événement",
    localGroupsTitle: "Groupes locaux",
    createGroupBtn: "+ Créer un groupe",
    localGroupsDesc: "Connectez-vous avec les communautés locales de votre ville ou région",
    viewAllGroups: "Afficher tous les groupes",
    mentoringTitle: "Mentorat",
    mentoringDesc: "Connectez-vous avec des experts ou trouvez des apprenants en matière de développement durable",
    findMentorBtn: "Trouver un mentor",
    findMentorDesc: "Trouvez un expert pour vous guider",
    becomeMentorBtn: "Devenez mentor",
    shareKnowledgeDesc: "Partagez vos connaissances",
    marketplaceTitle: "Marché écologique",
    publishBtn: "+ Publier",
    marketplaceDesc: "Acheter, vendre ou échanger des produits et services durables",
    karma: "Karma",
    reputation: "Réputation",
    followers: "Abonnés",
    quickLinks: "Liens rapides",
    myProfileLabel: "Mon profil",
    good: "Bien",
    fair: "Équitable",
    howRecsWork: "Comment fonctionnent les recommandations",
    locationPrio: "Localisation : Nous priorisons les événements près de chez vous",
    interestsPrio: "Centres d'intérêt : en fonction de vos catégories préférées",
    timesPrio: "Horaires : Nous prenons en compte vos horaires préférés",
    historyPrio: "Histoire : nous apprenons des événements passés",
    availabilityPrio: "Disponibilité : Uniquement les événements avec des places gratuites",
    environmentalProject: "Projet environnemental",
    educationalProject: "Projet pédagogique",
    healthServices: "Services de santé",
    communityProject: "Projet communautaire",
    foodProject: "Projet alimentaire",
    activeProject: "Projet actif",
    exploreCategories: "Explorer les catégories",
    exploreDescription: "Découvrez des projets régénératifs, des événements communautaires et des opportunités d'impact dans votre région.",
    featuredProjectsDesc: "Des initiatives qui changent l’Europe en ce moment.",
    viewProjects: "Voir les projets",
    coursesAndWorkshops: "Cours et ateliers",
    coursesAndWorkshopsDesc: "Acquérir des compétences pour un avenir durable.",
    exploreCourses: "Explorer les cours",
    impactSearch: "Recherche d'impact",
    searchPlaceholderExplore: "Qu'est-ce que tu cherches? (ex. Permaculture, Barcelone, Océans...)",
    searchResults: "Résultats de la recherche",
    found: "trouvé",
    noResultsFor: "Aucun résultat trouvé pour",
    clearSearch: "Effacer la recherche",
    haveInitiative: "Avez-vous une initiative?",
    registerInitiativeDesc: "Enregistrez-le sur EcoNexo et connectez-vous avec des milliers de bénévoles.",
    publishProject: "Publier le projet",
    tagBiodiversity: "Biodiversité",
    tagSolarEnergy: "Énergie solaire",
    tagZeroWaste: "Zéro Déchet",
    tagReforestation: "Reboisement",
    ecoTipsTitle: "Conseils écologiques",
    ecoTipsDescription: "Des idées pratiques pour réduire votre empreinte et vivre durablement.",
    filters: "Filtres",
    search: "Recherche",
    searchProjects: "Rechercher des projets, des villes ou des pays",
    categories: "Catégories",
    onlyAvailableSpots: "Uniquement dans la limite des places disponibles",
    maxDistance: "Distance maximale",
    anyDistance: "N'importe quelle distance",
    dateRange: "Plage de dates",
    type: "Taper",
    permanent: "Permanent",
    frequency: "Fréquence",
    once: "Une fois",
    regular: "Régulier",
    centerOnLocation: "Centrer sur ma position",
    clearFilters: "Effacer les filtres",
    joinCommunity: "Rejoindre la communauté",
    readyToMakeDifference: "Prêt à faire la différence ?",
    readyDescription: "Explorez des projets, participez à des événements et connectez-vous avec une communauté engagée pour l'avenir de notre planète",
    letsGo: "Allons-y!",
    viewMap: "Voir la carte",
    addToGoogleCalendar: "Ajouter à Google Agenda",
    addToOutlookCalendar: "Ajouter à Outlook",
    downloadICS: "Télécharger iCal",
    addToCalendar: "Ajouter au calendrier",
    liveCommunityActivity: "Activité communautaire en direct",
    mentionHashtag: "#EcoNexo sur les réseaux sociaux",
    questCompleted: "Quête terminée",
    questCompletedXP: "Récompense de quête",
    questOceanGuardian: "Gardien de l'océan",
    questStepJoinOceans: "Rejoignez 2 événements sur la conservation des océans",
    questStepShareOcean: "Partager un projet océan",
    questUrbanSustainer: "Durabilité urbaine",
    questStepSyncEvents: "Synchronisez 3 événements avec votre calendrier",
    questStepJoinWorkshop: "Participez à un atelier de jardinage urbain",
    stats: "Statistiques",
    quests: "Quêtes",
    viewAllBadges: "Voir tous les badges",
    communityRank: "Classement communautaire",
    karmaLabel: "Karma",
    impactLabel: "Impact",
    dailyChallenges: "Défis quotidiens",
    challengeGreenerPlanet: "Une planète plus verte",
    challengeGreenerDesc: "Synchronisez 3 événements cette semaine",
    challengeEcoVoice: "Voix Éco",
    challengeEcoVoiceDesc: "Partagez 10 projets",
    activeQuestsLabel: "Quêtes actives",
    newBadge: "Nouveau Badge !",
    levelLabelShort: "NIV",
    noActiveQuests: "Aucune quête active. Restez à l'écoute !",
    collectReward: "Récupérez la récompense",
    environmentTitle: "Environnement",
    environmentDescription: "Projets de conservation, de reboisement et de protection des écosystèmes",
    educationTitle: "Éducation",
    educationDescription: "Initiatives éducatives et programmes de formation environnementale",
    communityTitle: "Communauté",
    communityDescription: "Projets de développement communautaire et de cohésion sociale",
    healthTitle: "Santé",
    healthDescription: "Initiatives communautaires en matière de santé et de bien-être",
    oceansTitle: "Océans",
    oceansDescription: "Projets de protection marine et de conservation aquatique",
    foodTitle: "Nourriture",
    foodDescription: "Agriculture durable et systèmes alimentaires locaux",
    sustainabilityTitle: "Durabilité",
    sustainabilityDescription: "Nous nous engageons à promouvoir des pratiques durables et la protection de l'environnement à travers toutes nos activités et partenariats.",
    globalImpactTitle: "Impact Global",
    globalImpactDescription: "Nos projets et initiatives créent des impacts positifs mesurables pour l'environnement et la société à l'échelle mondiale.",
    innovationTitle: "Innovation",
    innovationDescription: "Nous exploitons une technologie de pointe et des solutions innovantes pour relever les défis environnementaux et créer des futurs durables.",
    localGroupsSubtitle: "Connectez-vous avec les communautés locales de votre ville ou région",
    createGroup: "Create Group",
    allGroups: "All",
    myGroups: "My Groups",
    nearbyGroups: "Nearby",
    searchGroups: "Search groups...",
    noGroupsFound: "No groups found",
    createFirstGroup: "Create the first group",
    eventsLabel: "events",
    groupBerlinName: "Sustainable Berlin",
    groupBerlinDescription: "Community of environmental activists in Berlin",
    groupMadridName: "Green Madrid",
    groupMadridDescription: "Ecological initiatives in Madrid",
    volunteersLabelEs: "voluntarios",
    volunteersLabelEn: "volunteers",
    volunteersLabelDe: "Freiwillige",
    spotsLabelEs: "cupos",
    spotsLabelEn: "spots",
    spotsLabelDe: "Plätze",
    locationBerlin: "Berlin, Germany",
    locationParis: "Paris, France",
    locationMadrid: "Madrid, Spain",
    notificationsNotSupported: "Notifications not supported",
    browserNotSupportPush: "Your browser doesn't support push notifications. Update your browser or use a more modern one.",
    notificationSettings: "Notification Settings",
    pushNotifications: "Push Notifications",
    receiveNotificationsAboutEvents: "Receive notifications about new events and projects",
    activated: "Activated",
    deactivating: "Deactivating...",
    deactivate: "Deactivate",
    activating: "Activating...",
    activate: "Activate",
    notificationsBlocked: "Notifications are blocked. Go to your browser settings to allow them.",
    subscribedToPushNotifications: "Subscribed to push notifications. You will receive updates about events and projects.",
    pleaseSelectValidImageEs: "Por favor selecciona un archivo de imagen válido",
    pleaseSelectValidImageEn: "Please select a valid image file",
    pleaseSelectValidImageDe: "Bitte wählen Sie eine gültige Bilddatei aus",
    imageTooLargeEs: "La imagen es demasiado grande. Por favor selecciona una imagen menor a 5MB",
    imageTooLargeEn: "Image is too large. Please select an image smaller than 5MB",
    imageTooLargeDe: "Bild ist zu groß. Bitte wählen Sie ein Bild kleiner als 5MB aus",
    errorLoadingImageEs: "Error al cargar la imagen. Por favor intenta de nuevo.",
    errorLoadingImageEn: "Error loading image. Please try again.",
    errorLoadingImageDe: "Fehler beim Laden des Bildes. Bitte versuchen Sie es erneut.",
    categoryEnvironment: "Environnement",
    categoryEducation: "Éducation",
    categoryCommunity: "Communauté",
    categoryTechnology: "Technologie",
    pleaseSignInFirstEs: "Por favor inicia sesión primero",
    pleaseSignInFirstEn: "Please sign in first",
    pleaseSignInFirstDe: "Bitte zuerst anmelden",
    linkCopiedEs: "Enlace copiado.",
    linkCopiedEn: "Link copied.",
    linkCopiedDe: "Link kopiert.",
    savedEs: "Guardado",
    savedEn: "Saved",
    savedDe: "Gespeichert",
    saveEs: "Guardar",
    saveEn: "Save",
    saveDe: "Speichern",
    joinEvent: "Join Event",
    fullyBooked: "Fully Booked",
    shareEvent: "Share Event",
    emailPlaceholderEs: "email@ejemplo.com",
    emailPlaceholderEn: "email@example.com",
    emailPlaceholderDe: "email@beispiel.de",
    profileSavedMessageEs: "Perfil guardado. Ingresa tu correo y presiona login para verificar.",
    profileSavedMessageEn: "Profile saved. Enter your email and press login to verify.",
    profileSavedMessageDe: "Profil gespeichert. Gib deine E‑Mail ein und klicke auf Login, um zu verifizieren.",
    oauthExtractMessageEs: "Se extraerán automáticamente: nombre, email y foto de perfil",
    oauthExtractMessageEn: "Automatically extracted: name, email and profile picture",
    oauthExtractMessageDe: "Automatisch extrahiert: Name, E-Mail und Profilbild",
    signinNotConfiguredEs: "Inicio de sesión no configurado",
    signinNotConfiguredEn: "Sign-in not configured",
    signinNotConfiguredDe: "Anmeldung nicht konfiguriert",
    googleOAuthTitleEs: "Google - Extrae nombre, email y foto",
    googleOAuthTitleEn: "Google - Extracts name, email and photo",
    googleOAuthTitleDe: "Google - Extrahiert Name, E-Mail und Foto",
    outlookOAuthTitleEs: "Outlook/Microsoft - Extrae nombre, email y foto",
    outlookOAuthTitleEn: "Outlook/Microsoft - Extracts name, email and photo",
    outlookOAuthTitleDe: "Outlook/Microsoft - Extrahiert Name, E-Mail und Foto",
    orGoDirectlyToEs: "O ve directamente a:",
    orGoDirectlyToEn: "Or go directly to:",
    orGoDirectlyToDe: "Oder direkt zu:",
    appTitle: "EcoNexo - Sustainable Projects in Europe",
    appDescription: "Connect with sustainable projects and events across Europe. Join the community building a greener future.",
    aboutTitle: "About EcoNexo",
    aboutSubtitle: "Connecting Europe's environmental community for a sustainable future",
    aboutMissionTitle: "Our Mission",
    aboutMissionP1: "EcoNexo is dedicated to connecting environmental activists, volunteers, and organizations across Europe to create a more sustainable future. We believe that collaboration and community are key to addressing the environmental challenges of our time.",
    aboutMissionP2: "Our platform enables users to discover environmental projects, join events, find green jobs, and connect with like-minded individuals who share their passion for environmental protection.",
    aboutOfferTitle: "What We Offer",
    aboutOfferMapTitle: "Interactive Map",
    aboutOfferMapDesc: "Discover environmental projects and events near you with our interactive map interface.",
    aboutOfferEventsTitle: "Events & Activities",
    aboutOfferEventsDesc: "Join environmental events, workshops, and community activities happening across Europe.",
    aboutOfferJobsTitle: "Green Jobs",
    aboutOfferJobsDesc: "Find meaningful employment opportunities in the environmental and sustainability sector.",
    aboutOfferChatTitle: "Community Chat",
    aboutOfferChatDesc: "Connect with fellow environmentalists through our community discussion forums.",
    aboutOfferProfilesTitle: "Personal Profiles",
    aboutOfferProfilesDesc: "Create detailed profiles to showcase your environmental interests and expertise.",
    aboutOfferMultilingualTitle: "Multilingual",
    aboutOfferMultilingualDesc: "Available in English, Spanish, and German to serve diverse European communities.",
    aboutValuesTitle: "Our Values",
    aboutValueSustainabilityTitle: "Sustainability",
    aboutValueSustainabilityDesc: "We are committed to promoting sustainable practices and environmental protection across all our activities and partnerships.",
    aboutValueCommunityTitle: "Community",
    aboutValueCommunityDesc: "We believe in the power of community and collaboration to create meaningful environmental change.",
    aboutValueInclusivityTitle: "Inclusivity",
    aboutValueInclusivityDesc: "We welcome people from all backgrounds and skill levels to join our environmental community.",
    aboutValueImpactTitle: "Impact",
    aboutValueImpactDesc: "We focus on creating measurable positive impact for the environment and our communities.",
    aboutImpactTitle: "Our Impact",
    aboutCommunityMembers: "Community Members",
    aboutEuropeanCities: "European Cities",
    aboutGetInvolvedTitle: "Get Involved",
    aboutGetInvolvedDesc: "Ready to make a difference? Join our community and start contributing to environmental change today.",
    backToEvents: "Back to events",
    date: "Date",
    time: "Time",
    location: "Location",
    contact: "Contact",
    volunteerProgress: "Volunteer progress",
    spotsLeft: "spots left",
    requirements: "Requirements",
    benefits: "Benefits",
    rememberToTellThemTooltip: "Remember to tell them you came from EcoNexo!",
    onboardingWelcome: "Bienvenue sur EcoNexo !",
    onboardingDescription: "Découvrez des projets durables et connectez-vous avec une communauté engagée dans la protection de l'environnement",
    onboardingMap: "Explorez la carte",
    onboardingMapDescription: "Visualisez des projets durables à travers l'Europe et trouvez des opportunités près de chez vous",
    onboardingExploreMap: "Explorer la carte",
    onboardingEvents: "Événements environnementaux",
    onboardingEventsDescription: "Participez à des événements de bénévolat et à des activités communautaires",
    onboardingViewEvents: "Voir les événements",
    onboardingProjects: "Projets durables",
    onboardingProjectsDescription: "Découvrez des initiatives qui changent le monde",
    onboardingCreateProject: "Créer un projet",
    onboardingJobs: "Emplois verts",
    onboardingJobsDescription: "Trouvez des opportunités de carrière dans le secteur durable",
    onboardingViewJobs: "Voir les emplois",
    onboardingCommunity: "Communauté",
    onboardingCommunityDescription: "Connectez-vous avec des personnes qui partagent votre passion pour l'environnement",
    onboardingJoinChat: "Rejoindre le chat",
    onboardingNext: "Suivant",
    onboardingSkip: "Passer",
    onboardingSelectLanguage: "Sélectionnez votre langue",
    onboardingLanguageDescription: "EcoNexo est disponible en plusieurs langues",
    onboardingLanguageTip: "Vous pouvez changer la langue à tout moment depuis le menu",
    welcomeIntroWelcomeTitle: "Bienvenue sur EcoNexo !",
    welcomeIntroWelcomeDescription: "Votre plateforme pour les projets durables en Europe",
    welcomeIntroLanguageTitle: "Langue",
    welcomeIntroLanguageDescription: "Choisissez votre langue préférée pour une meilleure expérience",
    welcomeIntroMapTitle: "Carte Interactive",
    welcomeIntroMapDescription: "Explorez des projets durables à travers l'Europe",
    welcomeIntroEventsTitle: "Événements",
    welcomeIntroEventsDescription: "Découvrez des événements environnementaux et des opportunités de bénévolat",
    welcomeIntroJobsTitle: "Emplois",
    welcomeIntroJobsDescription: "Trouvez des opportunités de carrière dans le secteur vert",
    welcomeIntroChatTitle: "Chat",
    welcomeIntroChatDescription: "Connectez-vous avec la communauté environnementale",
    welcomeIntroProfileTitle: "Profil",
    welcomeIntroProfileDescription: "Gérez votre compte et vos projets sauvegardés",
    welcomeIntroSkip: "Passer",
    welcomeIntroNext: "Suivant",
    welcomeIntroGetStarted: "Commencer !",
    welcomeIntroFinish: "Terminer",
    personalInformation: "Personal Information",
    firstName: "First Name",
    lastName: "Last Name",
    phone: "Phone",
    city: "City",
    country: "Country",
    aboutMe: "About Me",
    passions: "Passions",
    areasOfExpertise: "Areas of Expertise",
    hobbies: "Hobbies",
    interests: "Interests",
    skills: "Skills",
    languages: "Languages",
    socialMedia: "Social Media",
    googleInformation: "Google Information",
    microsoftInformation: "Microsoft Information",
    providerInformation: "Provider Information",
    extractedFromOAuth: "This information was automatically extracted from your OAuth account upon login.",
    calculatedAge: "Calculated Age",
    years: "years",
    verifiedEmail: "Verified Email",
    verified: "Verified",
    preferredLanguage: "Preferred Language",
    googleWorkspaceDomain: "Google Workspace Domain",
    tenantId: "Tenant ID",
    preferredUsername: "Preferred Username",
    linkedin: "LinkedIn",
    twitter: "Twitter",
    instagram: "Instagram",
    firstNamePh: "Enter first name",
    lastNamePh: "Enter last name",
    phonePh: "Enter phone number",
    aboutMePh: "Tell us about yourself...",
    linkedinPh: "https://linkedin.com/in/your-profile",
    twitterPh: "https://twitter.com/your-username",
    instagramPh: "https://instagram.com/your-username",
    websitePh: "https://your-website.com",
    website: "Website",
    useLogo: "Use logo",
    passionsPlaceholder: "What are you passionate about?",
    expertisePlaceholder: "What are you an expert in?",
    hobbiesPlaceholder: "What are your hobbies?",
    interestsPlaceholder: "What interests you?",
    skillsPlaceholder: "What skills do you have?",
    languagesPlaceholder: "What languages do you speak?",
    spotsLabel: "spots",
    projectTitle: "Project Title",
    projectTitlePh: "Ex: Sustainable Urban Reforestation",
    locationPh: "City, Country",
    detailedDescription: "Detailed Description",
    detailedDescriptionPh: "Describe goals, impact, and methodology...",
    targetParticipants: "Target Participants",
    estimatedBudget: "Estimated Budget (€)",
    keyMilestones: "Key Milestones",
    keyMilestonesPh: "- Launch\n- First implementation phase\n- Evaluation",
    accessibilityNeedsPh: "e.g. Wheelchair, limited mobility, etc.",
    writeMessagePh: "Type a message...",
    roomDestroyed: "Room Destroyed",
    roomDestroyedDesc: "This room has self-destructed as scheduled. All messages have been permanently deleted.",
    participantsCount: "{{count}} participants",
    leaveRoom: "Leave room",
    noMessagesYet: "No messages yet. Be the first to write!",
    signInToSendMessage: "Sign in to send messages",
    goBack: "Go Back",
    searchConfiguration: "Search Configuration",
    remoteWorkOnly: "Remote work only",
    experienceYears: "Experience (years)",
    skillsCommas: "Skills (comma separated)",
    salaryMin: "Salary (Min)",
    matchingAgentWelcome: "Hello! I'm your intelligent matching assistant. I can help you find projects that perfectly match your interests and preferences. What type of projects interest you?",
    matchingAgentError: "Sorry, there was an error searching for projects. Please try again.",
    signInToUseMatching: "Please sign in to use the matching assistant.",
    suggestionsLabel: "Suggestions:",
    typeSearchPh: "Type your search...",
    searchEventsPh: "Rechercher des événements dans votre ville...",
    searchEventsTitlePh: "Titre, lieu, organisateur...",
    loadingCalendar: "Chargement du calendrier...",
    loadingEventsIndicator: "Chargement des événements...",
    showFiltersLabel: "Afficher les filtres",
    hideFiltersLabel: "Masquer les filtres",
    noEventsFound: "Aucun événement trouvé correspondant aux filtres",
    becomeMentor: "Devenir Mentor",
    shareKnowledge: "Partagez vos connaissances et soutenez les apprenants sur les thèmes de la durabilité.",
    expertiseAreas: "Domaines d'expertise",
    expertiseAreasPhMentoring: "Énergie, Circularité...",
    languagesPhMentoring: "Allemand, Anglais...",
    availabilityPhMentoring: "2 h/semaine",
    offerMentoring: "Proposer du mentorat",
    savingEllipsis: "Enregistrement...",
    creatingEllipsis: "Création...",
    groupName: "Nom du Groupe",
    groupNamePh: "ex. Berlin Durable",
    groupDescriptionPh: "Décrivez le but et les objectifs du groupe...",
    tagsLabel: "Étiquettes",
    region: "Région",
    add: "Ajouter",
    addTagPh: "Ajouter une étiquette...",
    publicGroup: "Groupe public (tout le monde peut rejoindre)",
    backToGroups: "Retour aux Groupes",
    mustSignInToCreateGroup: "Vous devez vous connecter pour créer un groupe",
    volunteersLabel: "bénévoles",
    previewLabel: "Aperçu",
    urbanReforestationBerlin: "Reforestation Urbaine Berlin",
    urbanReforestationDesc: "Rejoignez notre initiative de reforestation urbaine à Berlin. Aidez à planter des arbres et créez des espaces verts en ville.",
    treesPlanted: "arbres plantés",
    seineRiverCleanup: "Nettoyage de la Seine",
    seineRiverCleanupDesc: "Participez au nettoyage de la Seine à Paris. Aidez à éliminer les déchets et à protéger la vie aquatique.",
    wasteCollected: "tonnes de déchets collectées",
    communityGardensMadrid: "Jardins Communautaires Madrid",
    communityGardensDesc: "Impliquez-vous dans les jardins communautaires de Madrid. Apprenez l'agriculture durable et connectez-vous avec vos voisins.",
    communityGardensActive: "jardins communautaires actifs",
    ecoTipCategoryFinance: "Finance",
    ecoTipBankTitle: "Choisissez une banque durable",
    ecoTipBankDescription: "Passez à des banques qui investissent dans l'énergie renouvelable et évitent les combustibles fossiles.",
    ecoTipCategoryTransport: "Transport",
    ecoTipTransportTitle: "Utilisez les transports en commun",
    ecoTipTransportDescription: "Réduisez votre empreinte carbone en utilisant les bus, les trains et les vélos plutôt que la voiture.",
    ecoTipCategoryFood: "Alimentation",
    ecoTipVegetarianTitle: "Essayez les repas à base de plantes",
    ecoTipVegetarianDescription: "Réduisez la consommation de viande et essayez de délicieuses alternatives végétariennes ou végétaliennes.",
    ecoTipCategoryEnergy: "Énergie",
    ecoTipEnergyTitle: "Passez aux énergies renouvelables",
    ecoTipEnergyDescription: "Choisissez des fournisseurs d'énergie qui utilisent l'énergie solaire, éolienne ou hydroélectrique.",
    ecoTipCategoryWaste: "Déchets",
    ecoTipWasteTitle: "Réduire, Réutiliser, Recycler",
    ecoTipWasteDescription: "Minimisez les déchets en achetant moins, en réutilisant les objets et en recyclant correctement les matériaux.",
    ecoTipCategoryWater: "Eau",
    ecoTipWaterTitle: "Économiser l'eau",
    ecoTipWaterDescription: "Prenez des douches plus courtes, réparez les fuites et utilisez des appareils économes en eau.",
    ecoTipCategoryOceans: "Océans",
    ecoTipOceanTitle: "Protéger la vie marine",
    ecoTipOceanDescription: "Évitez les plastiques à usage unique et utilisez de la crème solaire respectueuse des récifs pour protéger nos océans.",
    ecoTipCategoryHealth: "Santé",
    ecoTipHealthTitle: "Bien-être naturel",
    ecoTipHealthDescription: "Utilisez des produits de nettoyage écologiques pour améliorer la qualité de l'air intérieur et la santé.",
    ecoTipCategoryEducation: "Éducation",
    ecoTipEducationTitle: "Apprentissage continu",
    ecoTipEducationDescription: "Partagez vos connaissances sur la durabilité avec vos amis et votre famille pour amplifier l'impact.",
    ecoTips: "Eco Tips",
    createEvent: "Créer un Événement",
    eventAdministrators: "Administrateurs de l'Événement",
    addAdministrator: "Ajouter un Administrateur",
    administratorEmail: "Email de l'Administrateur",
    searchAdministrator: "Rechercher par email",
    addAsAdministrator: "Ajouter comme Administrateur",
    removeAdministrator: "Supprimer l'Administrateur",
    manageAdministrators: "Gérer les Administrateurs",
    noAdministrators: "Pas d'administrateurs",
    youAreAdministrator: "Vous êtes administrateur",
    eventCreator: "Créateur de l'Événement",
    administrators: "Administrateurs",
    ofRevenueDonated: "des revenus reversés à des causes environnementales",
    subscribe: "S'abonner",
    communityChat: "Chat Communautaire",
    topicForums: "Forums thématiques",
    generalTopic: "Général",
    topicDescriptionGeneral: "Discussions générales sur la durabilité",
    environmentTopic: "Environnement",
    topicDescriptionEnvironment: "Protection de l'environnement et action climatique",
    educationTopic: "Éducation",
    topicDescriptionEducation: "Éducation et sensibilisation à l'environnement",
    healthTopic: "Santé",
    topicDescriptionHealth: "Sante et connexions environnementales",
    communityTopic: "Communauté",
    topicDescriptionCommunity: "Renforcement de la communauté et initiatives locales",
    oceansTopic: "Océans",
    topicDescriptionOceans: "Conservation des océans et vie marine",
    foodTopic: "Alimentation",
    topicDescriptionFood: "Alimentation et agriculture durables",
    chatMessage1: "Excellente initiative ! Comment puis-je m'impliquer ?",
    chatMessage2: "J'ai travaillé sur des projets similaires dans ma région",
    chatMessage3: "C'est exactement ce dont nous avons besoin de plus !",
    chatMessage4: "Quelqu'un a-t-il déjà essayé cette approche ?",
    environment: "Environnement",
    education: "Éducation",
    health: "Santé",
    community: "Communauté",
    oceans: "Océans",
    food: "Alimentation",
    capital: "Capitale",
    population: "Population",
    currency: "Devise",
    regionLabel: "Région",
    governmentAPIAvailable: "API Gouvernementale Disponible",
    governmentAPIInDevelopment: "API Gouvernementale en développement",
    integratedGovernmentAPIs: "APIs Gouvernementales Intégrées",
    baseURL: "URL de base",
    eventNativeTreePlanting: "Plantation d'arbres natifs",
    eventSolarEnergyWorkshop: "Atelier sur l'énergie solaire",
    eventRiverCleanup: "Nettoyage de rivière",
    eventClimateConference: "Conférence sur le changement climatique",
    eventFoodConservationWorkshop: "Atelier de conservation des aliments",
    eventVerticalGardens: "Construction de jardins verticaux",
    eventEcoRace: "Course 5K éco-responsable",
    eventSolarPanels: "Installation de panneaux solaires",
    eventEnvironmentalReflection: "Réflexion environnementale du semestre",
    eventSustainableSummer: "Célébration d'un été durable",
    eventAutumnPlanting: "Plantation d'automne",
    eventForestCleanup: "Nettoyage de forêt d'automne",
    eventBirdShelters: "Construction d'abris pour oiseaux",
    eventWindEnergy: "Atelier sur l'énergie éolienne",
    eventOrganicProducts: "Foire aux produits biologiques",
    eventWetlandsRestoration: "Restauration des zones humides",
    eventCommunityGardens: "Construction de jardins communautaires",
    eventSustainableChristmas: "Marché de Noël durable",
    eventWinterReforestation: "Reforestation hivernale",
    eventClimateSeminar: "Séminaire sur le changement climatique",
    eventEcoNewYear: "Célébration du Nouvel An éco-responsable",
    eventMarineConservation: "Conservation des espèces marines",
    eventHydroelectricWorkshop: "Atelier sur l'énergie hydroélectrique",
    jobTitle5: "Architecte en Économie Circulaire",
    jobTitle6: "Spécialiste en Conservation des Océans",
    jobTitle7: "Spécialiste en Architecture Verte",
    jobTitle8: "Chercheur en Économie Circulaire",
    jobTitle9: "Ingénieur d'Exploitation de Parc Éolien",
    jobDesc5: "Concevoir et mettre en œuvre des stratégies d'économie circulaire pour les entreprises à Amsterdam. Travailler avec les équipes produits pour prolonger le cycle de vie, réduire les déchets et créer des flux de matériaux en boucle fermée. Collaborer avec les municipalités et les startups.",
    jobDesc6: "Soutenir les programmes de conservation marine depuis Lisbonne. Surveiller la biodiversité, analyser les données des capteurs sous-marins et coordonner les enquêtes sur les plages avec les ONG et les universités locales.",
    jobDesc7: "Diriger des projets d'architecture durable à Amsterdam. Intégrer l'efficacité énergétique, la conception passive et les matériaux à faible teneur en carbone (LEED/BREEAM). Coordonner avec les urbanistes et les parties prenantes communautaires.",
    jobDesc8: "Rejoindre un institut de recherche à Copenhague pour évaluer les projets pilotes d'économie circulaire. Quantifier l'impact (ACV), publier les résultats et soutenir les recommandations politiques dans les pays nordiques.",
    jobDesc9: "Exploiter et optimiser les parcs éoliens près de Hambourg. Superviser les calendriers de maintenance, la surveillance SCADA, les procédures de sécurité et les améliorations de performance avec les partenaires OEM.",
    machinelearning: "apprentissage automatique",
    buceo: "plongée",
    reactnative: "React Native",
    nodejs: "Node.js",
    blockchain: "blockchain",
    iot: "IdO (IoT)",
    uxui: "UX/UI",
    jobTitle: "Titre du poste",
    jobTitlePlaceholder: "Entrez le titre du poste",
    companyName: "Nom de l'entreprise",
    companyNamePlaceholder: "Entrez le nom de votre entreprise",
    jobLocation: "Lieu du poste",
    jobLocationPlaceholder: "Où se situe le poste ?",
    jobDescription: "Description du poste",
    jobDescriptionPlaceholder: "Décrivez les responsabilités du poste",
    requiredSkills: "Compétences requises",
    requiredSkillsPlaceholder: "Compétences nécessaires pour le poste",
    benefitsPlaceholder: "Avantages du poste",
    contactEmail: "Email de contact",
    contactEmailPlaceholder: "email@entreprise.com",
    salaryRange: "Échelle salariale",
    salaryRangePlaceholder: "Échelle salariale proposée",
    jobType: "Type de poste",
    jobTypeFullTime: "Temps plein",
    jobTypePartTime: "Temps partiel",
    jobTypeContract: "Contrat à durée déterminée",
    jobTypeInternship: "Stage",
    jobTypeRemote: "À distance",
    jobTypeHybrid: "Hybride",
    experienceLevel: "Niveau d'expérience",
    experienceEntry: "Niveau débutant",
    experienceMid: "Niveau intermédiaire",
    experienceSenior: "Niveau senior",
    experienceExpert: "Niveau expert",
    applicationDeadline: "Date limite de candidature",
    saveDraft: "Enregistrer le brouillon",
    publishJob: "Publier le poste",
    jobCreated: "Poste créé",
    spanish: "Espagnol",
    english: "Anglais",
    german: "Allemand",
    nativeLanguage: "Langue maternelle",
    selectLevel: "Sélectionnez le niveau",
    beginner: "Débutant",
    intermediate: "Intermédiaire",
    advanced: "Avancé",
    fluent: "Courant",
    selectLanguage: "Sélectionnez la langue",
    levelJunior: "Junior",
    levelMid: "Intermédiaire",
    levelSenior: "Senior",
    levelLead: "Lead",
    adminPanelTitle: "Panneau d'Administration",
    adminPanelDescription: "Gérer les événements, les projets et les utilisateurs d'EcoNexo",
    adminTotalUsers: "Total des Utilisateurs",
    adminActiveEvents: "Événements Actifs",
    adminProjects: "Projets",
    adminGreenJobs: "Emplois Verts",
    adminActiveUsers: "Utilisateurs Actifs",
    adminLast30Days: "Derns. 30 jours",
    adminEventManagement: "Gestion des Événements",
    adminProjectManagement: "Gestion des Projets",
    adminUserManagement: "Gestion des Utilisateurs",
    adminEvent: "Événement",
    adminDate: "Date",
    adminParticipants: "Participants",
    adminStatus: "Statut",
    adminActions: "Actions",
    adminProject: "Projet",
    adminCreator: "Créateur",
    adminVolunteers: "Bénévoles",
    adminActive: "Actif",
    adminCompleted: "Terminé",
    adminCancelled: "Annulé",
    adminDraft: "Brouillon",
    adminEdit: "Modifier",
    adminDelete: "Supprimer",
    adminApprove: "Approuver",
    adminOverview: "Aperçu",
    adminEvents: "Événements",
    adminUsers: "Utilisateurs",
    adminUserManagementInDevelopment: "Fonctionnalité de gestion des utilisateurs en développement...",
    adminBeachCleanupBarcelona: "Nettoyage de plage Barcelone",
    adminTreePlantingMadrid: "Plantation d'arbres Madrid",
    adminCommunityGardenValencia: "Jardin communautaire Valence",
    adminSolarEnergyNeighborhood: "Énergie solaire quartier",
    badgeEcoWarrior: "Éco Guerrier",
    badgeCommunityLeader: "Leader Communautaire",
    badgeGreenJobHunter: "Chasseur d'Emplois Verts",
    badgeChatMaster: "Maître du Chat",
    level: "Niveau",
    achievementEcoExplorer: "Éco Explorateur",
    achievementProjectPioneer: "Pionnier de Projet",
    achievementCommunityConnector: "Connecteur Communautaire",
    achievementCarbonNeutral: "Carbone Neutre",
    adminPageTitle: "Panneau d'Administration - EcoNexo",
    adminPageDescription: "Gérer les événements, les projets et les utilisateurs d'EcoNexo",
    centerOnMyLocation: "Centrer sur ma position",
    socialMediaCommunityReforestationBerlin: "Reforestation communautaire à Berlin",
    all: "Tous",
    northernEurope: "Europe du Nord",
    westernEurope: "Europe de l'Ouest",
    centralEurope: "Europe Centrale",
    southernEurope: "Europe du Sud",
    easternEurope: "Europe de l'Est",
    minExperience: "Expérience minimale",
    minSalary: "Salaire minimal",
    allCities: "Toutes les villes",
    allContracts: "Tous les contrats",
    results: "résultats",
    calendar: "Calendrier",
    required: "Obligatoire",
    titlePh: "Titre de l'événement",
    mainCategory: "Catégorie principale",
    cityPh: "Nom de la ville",
    countryPh: "Nom du pays",
    optional: "Optionnel",
    addressPh: "Adresse de l'événement",
    websitePlaceholderEn: "https://exemple.fr",
    image: "Image",
    imageUrlPlaceholderEn: "https://exemple.fr/image.jpg",
    optionalCategories: "Catégories optionnelles",
    capacityPh: "Nombre maximum de participants",
    notes: "Notes",
    notesPh: "Notes additionnelles",
    save: "Enregistrer",
    existingEvents: "Événements existants",
    refresh: "Actualiser",
    category: "Catégorie",
    noParticipatedEvents: "Aucun événement rejoint",
    month: "Mois",
    list: "Liste",
    viewEvent: "Voir l'événement",
    viewDetails: "Voir les détails",
    join: "Rejoindre",
    createdEvent: "Événement créé",
    eventCreatedMessage: "a été créé pour",
    beVolunteer: "Devenir bénévole",
    description: "Description",
    impact: "Impact",
    raised: "récolté",
    reached: "atteint",
    goal: "Objectif",
    volunteers: "Bénévoles",
    progressReached: "Progrès atteint",
    donatePaypal: "Donner avec PayPal",
    donateKlarna: "Donner avec Klarna",
    donateStripe: "Donner avec Stripe",
    backToMap: "Retour à la carte",
    moreInfo: "Plus d'infos",
    pleaseCompleteSecurityVerification: "Veuillez compléter la vérification de sécurité",
    errorSendingEmail: "Erreur lors de l'envoi de l'email",
    unexpectedError: "Erreur inattendue. Veuillez réessayer.",
    errorStartingGoogleAuth: "Erreur lors du démarrage de l'authentification Google",
    errorStartingOutlookAuth: "Erreur lors du démarrage de l'authentification Outlook",
    securityVerificationIncorrect: "Vérification de sécurité incorrecte. Veuillez réessayer.",
    welcomeEmailSent: "Email de bienvenue envoyé ! Veuillez consulter votre boîte de réception et vérifier votre compte en cliquant sur le lien.",
    authenticationSuccessful: "Authentification réussie !",
    orContinueWith: "Ou continuer avec",
    registerWith: "S'inscrire avec",
    or: "Ou",
    emailPlaceholder: "votre@email.com",
    errorInSecurityVerification: "Erreur dans la vérification de sécurité",
    processing: "Traitement...",
    signInButton: "Se connecter",
    createAccountButton: "Create Account",
    bySigningIn: "By signing in, you accept our",
    byRegistering: "By registering, you accept our",
    termsOfService: "Terms of Service",
    and: "and",
    privacyPolicy: "Politique de confidentialité",
    captchaNotConfigured: "reCAPTCHA non configuré. Contactez l'administrateur.",
    verificationCompleted: "Vérification terminée",
    securityVerification: "Vérification de sécurité :",
    yourAnswer: "Votre réponse",
    verify: "Vérifier",
    howMuchIs: "Combien font",
    badgeFirstStep: "First Step",
    descFirstStep: "Completed your first action",
    badgeExplorer: "Explorer",
    descExplorer: "Explored different sections",
    badgeHelper: "Helper",
    descHelper: "Helped the community",
    descFirstEvent: "Attended your first event",
    badgeEventEnthusiast: "Enthousiaste",
    descEventEnthusiast: "A participé à 5 événements",
    badgeEventMaster: "Maître des Événements",
    descEventMaster: "A participé à 10 événements",
    badgeEventCreator: "Organisateur",
    descEventCreator: "A créé son premier événement",
    badgeProjectJoiner: "Collaborateur",
    descProjectJoiner: "A rejoint son premier projet",
    badgeProjectCreator: "Créateur",
    descProjectCreator: "A créé son premier projet",
    badgeProjectVeteran: "Vétéran",
    descProjectVeteran: "A complété 5 projets",
    badgeConnector: "Connecteur",
    descConnector: "A établi sa première connexion",
    badgeMentor: "Mentor",
    descMentor: "Est devenu mentor",
    badgeMarketplaceSeller: "Green Seller",
    descMarketplaceSeller: "Posted in marketplace",
    badgeGreenCommuter: "Green Commuter",
    descGreenCommuter: "Used sustainable transport",
    badgeWasteWarrior: "Waste Warrior",
    descWasteWarrior: "Reduced waste",
    badgeCarbonFighter: "Carbon Fighter",
    descCarbonFighter: "Offset your carbon footprint",
    badgeDailyStreak7: "Weekly Streak",
    descDailyStreak7: "7 consecutive days",
    badgeDailyStreak30: "Monthly Streak",
    descDailyStreak30: "30 consecutive days",
    badgeProfileComplete: "Profile Complete",
    descProfileComplete: "Completed your profile",
    badgeSocialSharer: "Sharer",
    descSocialSharer: "Shared 10 times",
    badgeCalendarSyncMaster: "Time Manager",
    descCalendarSyncMaster: "Synced 3 events",
    tourWelcomeTitle: "Bienvenue sur EcoNexo !",
    tourWelcomeDesc: "Faisons un tour rapide pour vous aider à démarrer votre parcours écologique.",
    tourMapTitle: "Carte Interactive",
    tourMapDesc: "Explorez les projets environnementaux, événements et groupes locaux près de chez vous.",
    tourCalendarTitle: "Calendrier des Événements",
    tourCalendarDesc: "Trouvez et synchronisez des ateliers, nettoyages et activités vertes.",
    tourJobsTitle: "Emplois Verts",
    tourJobsDesc: "Découvrez des opportunités de carrière dans le secteur du développement durable.",
    tourChatTitle: "Chat Communautaire",
    tourChatDesc: "Connectez-vous avec d'autres membres éco-responsables et discutez de sujets pertinents.",
    tourMatchingTitle: "Matching Intelligent",
    tourMatchingDesc: "Obtenez des recommandations personnalisées adaptées à vos compétences.",
    tourCommunityTitle: "Communauté & Groupes",
    tourCommunityDesc: "Rejoignez des groupes locaux, trouvez des mentors et échangez sur la place de marché.",
    tourProfileTitle: "Votre Profil",
    tourProfileDesc: "Suivez votre impact, vos badges et vos progrès.",
    tourAboutTitle: "À Propos de Nous",
    tourAboutDesc: "En savoir plus sur notre mission et notre vision.",
    tourNext: "Suivant",
    tourPrev: "Précédent",
    tourDone: "Terminer",

    // Impact Page UI
    yourImpact: "Votre Impact",
    impactDescription: "Vous construisez un avenir plus vert, étape par étape. Voici vos progrès et accomplissements sur EcoNexo.",
    xpToLevel: "XP avant niveau supérieur",
    impactMedals: "Médailles",
    impactStreakDays: "Jours consécutifs",
    impactSocialImpact: "Impact Social",
    impactShared: "partagés",
    impactTalleres: "Ateliers",
    impactCompleted: "terminés",
    impactCity: "Ville",
    impactRanking: "Classement",
    impactBadgeCollection: "Collection de Badges",
    impactRecentActivity: "Activité Récente",
    impactNoActivity: "Aucune activité récente.",
    impactJustNow: "À l'instant",
    impactKeepGoing: "Continuez comme ça !",
    impactUnlockNextLevel: "Participez à plus de projets et événements pour débloquer le niveau {{level}}.",
    impactExploreProjects: "Explorer les Projets",
    impactLevel: "Niveau",
    categoryStarter: "Débutant",
    categoryEngagement: "Engagement",
    categoryLifestyle: "Mode de vie",
    volunteersPageTitle: "Page des Volontaires",
    localRecords: "Dossiers Locaux",
    noVolunteersYet: "Pas encore de volontaires",
    backToProject: "Retour au Projet",
    volunteeringPoints: "Rejoindre le projet",
    infoNotAvailable: "Site officiel non disponible",
    welcomeToForum: "Bienvenue sur le forum {{topic}} !",
    startDiscussionMsg: "Démarrez une discussion ou répondez aux questions.",
    personalizeEcoNexo: "Personnaliser EcoNexo",
    onboardingSubtitle: "Aidez-nous à vous recommander les meilleurs projets.",
    causesTitle: "Quelles causes vous animent ?",
    skillsTitle: "Quelles compétences pouvez-vous apporter ?",
    back: "Retour",
    skip: "Passer",
    next: "Suivant",
    ready: "Prêt !",
    skillTech: "Technologie / Info",
    skillGardening: "Jardinage / Terrain",
    skillTeaching: "Éducation / Enseignement",
    skillManual: "Travail manuel / Réparation",
    skillSocial: "Travail social / Soins",
    skillArt: "Art / Créativité",
  },
  it: {
    // Job titles
    jobTitle1: "Specialista in Riforestazione Urbana",
    jobTitle2: "Analista Dati Qualità dell'Aria",
    jobTitle3: "Educatore Ambientale (STEM)",
    jobTitle4: "Coordinatore Pulizia Fiumi",

    // Job descriptions
    jobDesc1: "Dirigi le iniziative di riforestazione urbana a Berlino, gestisci progetti di piantagione di alberi, coordina con le comunità locali e sviluppa spazi verdi sostenibili. Lavorerai con scienziati ambientali, urbanisti e volontari per creare ambienti urbani più sani.",
    jobDesc2: "Analizza i dati sulla qualità dell'aria utilizzando Python e strumenti GIS, monitora le reti di sensori a Parigi e crea report per i responsabili delle politiche ambientali. Lavorerai con scienziati dei dati e ingegneri ambientali per migliorare i sistemi di monitoraggio della qualità dell'aria.",
    jobDesc3: "Sviluppa e consegna programmi di educazione ambientale basati su STEM a Madrid, insegnando agli studenti energia solare, robotica e sostenibilità. Creerai curricula coinvolgenti e lavorerai con le scuole per promuovere la consapevolezza ambientale.",
    jobDesc4: "Coordina le operazioni di pulizia dei fiumi a Milano, gestisci squadre di volontari, assicura protocolli di sicurezza e organizza iniziative di raccolta rifiuti. Lavorerai con organizzazioni ambientali e autorità locali per proteggere gli ecosistemi acquatici.",

    // Technical tags
    sensores: "sensori",
    silvicultura: "silvicoltura",
    biodiversidad: "biodiversità",
    gestióndeproyectos: "gestione progetti",
    didáctica: "didattica",
    energíasolar: "energia solare",
    robóticaeducativa: "robotica educativa",
    gestióndevoluntariado: "gestione volontariato",
    residuos: "rifiuti",
    seguridad: "sicurezza",
    python: "Python",
    gis: "GIS",

    // Interface translations
    searchPlaceholderEn: "Cerca lavori...",
    cityLabel: "Città",
    contractLabel: "Tipo di contratto",
    contract_full_time: "Tempo pieno",
    contract_part_time: "Part-time",
    contract_contract: "Contratto",
    contract_internship: "Stage",
    remoteOnly: "Solo remoto",
    yearsExp: "anni di esperienza",
    applyBtn: "Candidati",
    saveBtn: "Salva",
    jobPostingConditions: "Condizioni di Pubblicazione",
    jobPostingInfo: "Informazioni di pubblicazione",
    conductGuidelines: "Linee Guida di Condotta",
    conductGuidelinesText: "Mantieni un ambiente rispettoso e costruttivo",
    createJobOffer: "Crea Offerta di Lavoro",

    // Chat Rules
    chatRules: "Regole Della Chat",
    chatRule1: "Rispetta Tutti I Partecipanti",
    chatRule2: "Mantieni Le Conversazioni Rilevanti",
    chatRule3: "Non Condividere Informazioni Personali",
    chatRule4: "Usa Un Linguaggio Appropriato",
    chatRule5: "Niente Spam",
    chatRule6: "Sii Costruttivo Nei Tuoi Commenti",
    chatRule7: "Segnala Contenuti Inappropriati",
    chatRule8: "Divertiti E Impara",
    chatRulesFooter: "Seguire Queste Regole Crea Una Comunità Positiva",
    chatRulesIntro: "Per Mantenere Un Ambiente Positivo, Segui Queste Regole:",
    chatRulesTitle: "Regole Della Comunità",

    // Chat Recommendations
    recommendations: "Raccomandazioni",
    popularDiscussions: "Discussioni Popolari",
    joinActiveConversations: "Unisciti Alle Conversazioni Attive",
    newProjects: "Nuovi Progetti",
    discoverLatestInitiatives: "Scopri Le Ultime Iniziative",
    eventsNearYou: "Eventi Vicino A Te",
    findLocalEvents: "Trova Eventi Locali",
  },
  pl: {
    // Job titles
    jobTitle1: "Specjalista ds. Reforestacji Miejskiej",
    jobTitle2: "Analityk Danych Jakości Powietrza",
    jobTitle3: "Edukator Środowiskowy (STEM)",
    jobTitle4: "Koordynator Czyszczenia Rzek",

    // Job descriptions
    jobDesc1: "Kieruj inicjatywami reforestacji miejskiej w Berlinie, zarządzaj projektami sadzenia drzew, koordynuj z lokalnymi społecznościami i rozwijaj zrównoważone tereny zielone. Będziesz pracować z naukowcami środowiskowymi, urbanistami i wolontariuszami, aby tworzyć zdrowsze środowiska miejskie.",
    jobDesc2: "Analizuj dane dotyczące jakości powietrza używając Pythona i narzędzi GIS, monitoruj sieci czujników w Paryżu i twórz raporty dla decydentów polityki środowiskowej. Będziesz pracować z analitykami danych i inżynierami środowiskowymi, aby ulepszać systemy monitorowania jakości powietrza.",
    jobDesc3: "Rozwijaj i dostarczaj programy edukacji środowiskowej oparte na STEM w Madrycie, ucząc studentów o energii słonecznej, robotyce i zrównoważonym rozwoju. Będziesz tworzyć angażujące programy nauczania i pracować ze szkołami, aby promować świadomość środowiskową.",
    jobDesc4: "Koordynuj operacje czyszczenia rzek w Mediolanie, zarządzaj zespołami wolontariuszy, zapewniaj protokoły bezpieczeństwa i organizuj inicjatywy zbierania odpadów. Będziesz pracować z organizacjami środowiskowymi i lokalnymi władzami, aby chronić ekosystemy wodne.",

    // Technical tags
    sensores: "czujniki",
    silvicultura: "leśnictwo",
    biodiversidad: "bioróżnorodność",
    gestióndeproyectos: "zarządzanie projektami",
    didáctica: "dydaktyka",
    energíasolar: "energia słoneczna",
    robóticaeducativa: "robotyka edukacyjna",
    gestióndevoluntariado: "zarządzanie wolontariatem",
    residuos: "odpady",
    seguridad: "bezpieczeństwo",
    python: "Python",
    gis: "GIS",

    // Interface translations
    searchPlaceholderEn: "Szukaj pracy...",
    cityLabel: "Miasto",
    contractLabel: "Typ umowy",
    contract_full_time: "Pełny etat",
    contract_part_time: "Część etatu",
    contract_contract: "Umowa",
    contract_internship: "Staż",
    remoteOnly: "Tylko zdalnie",
    yearsExp: "lat doświadczenia",
    applyBtn: "Aplikuj",
    saveBtn: "Zapisz",
    jobPostingConditions: "Warunki Publikacji",
    jobPostingInfo: "Informacje o publikacji",
    conductGuidelines: "Wytyczne Postępowania",
    conductGuidelinesText: "Utrzymuj szanujące i konstruktywne środowisko",
    createJobOffer: "Utwórz Ofertę Pracy",

    // Chat Rules
    chatRules: "Zasady Czatu",
    chatRule1: "Szanuj Wszystkich Uczestników",
    chatRule2: "Utrzymuj Rozmowy Na Temat",
    chatRule3: "Nie Udostępniaj Informacji Osobistych",
    chatRule4: "Używaj Odpowiedniego Języka",
    chatRule5: "Brak Spamu",
    chatRule6: "Bądź Konstruktywny W Swoich Komentarzach",
    chatRule7: "Zgłaszaj Nieodpowiednie Treści",
    chatRule8: "Ciesz Się I Ucz Się",
    chatRulesFooter: "Przestrzeganie Tych Zasad Tworzy Pozytywną Społeczność",
    chatRulesIntro: "Aby Utrzymać Pozytywne Środowisko, Proszę Przestrzegać Tych Zasad:",
    chatRulesTitle: "Zasady Społeczności",

    // Chat Recommendations
    recommendations: "Rekomendacje",
    popularDiscussions: "Popularne Dyskusje",
    joinActiveConversations: "Dołącz Do Aktywnych Rozmów",
    newProjects: "Nowe Projekty",
    discoverLatestInitiatives: "Odkryj Najnowsze Inicjatywy",
    eventsNearYou: "Wydarzenia W Twojej Okolicy",
    findLocalEvents: "Znajdź Lokalne Wydarzenia",
  },
  nl: {
    // Job titles
    jobTitle1: "Stedelijke Herbebossing Specialist",
    jobTitle2: "Luchtkwaliteit Data Analist",
    jobTitle3: "Milieu Educateur (STEM)",
    jobTitle4: "Rivier Opruiming Coördinator",

    // Job descriptions
    jobDesc1: "Leid stedelijke herbebossingsinitiatieven in Berlijn, beheer boomplantprojecten, coördineer met lokale gemeenschappen en ontwikkel duurzame groene ruimtes. Je werkt samen met milieuwetenschappers, stadsplanners en vrijwilligers om gezondere stedelijke omgevingen te creëren.",
    jobDesc2: "Analyseer luchtkwaliteitsgegevens met Python en GIS-tools, monitor sensornetwerken in Parijs en maak rapporten voor milieubeleidsmakers. Je werkt samen met datawetenschappers en milieu-ingenieurs om luchtkwaliteitsmonitoringssystemen te verbeteren.",
    jobDesc3: "Ontwikkel en lever STEM-gebaseerde milieu-educatieprogramma's in Madrid, leer studenten over zonne-energie, robotica en duurzaamheid. Je creëert boeiende curricula en werkt samen met scholen om milieubewustzijn te bevorderen.",
    jobDesc4: "Coördineer rivieropruimingsoperaties in Milaan, beheer vrijwilligersteams, zorg voor veiligheidsprotocollen en organiseer afvalinzamelingsinitiatieven. Je werkt samen met milieuorganisaties en lokale autoriteiten om waterecosystemen te beschermen.",

    // Technical tags
    sensores: "sensoren",
    silvicultura: "bosbouw",
    biodiversidad: "biodiversiteit",
    gestióndeproyectos: "projectmanagement",
    didáctica: "didactiek",
    energíasolar: "zonne-energie",
    robóticaeducativa: "educatieve robotica",
    gestióndevoluntariado: "vrijwilligersmanagement",
    residuos: "afval",
    seguridad: "veiligheid",
    python: "Python",
    gis: "GIS",

    // Interface translations
    searchPlaceholderEn: "Zoek banen...",
    cityLabel: "Stad",
    contractLabel: "Contracttype",
    contract_full_time: "Volledig",
    contract_part_time: "Deeltijd",
    contract_contract: "Contract",
    contract_internship: "Stage",
    remoteOnly: "Alleen op afstand",
    yearsExp: "jaar ervaring",
    applyBtn: "Solliciteer",
    saveBtn: "Opslaan",
    jobPostingConditions: "Vacature Voorwaarden",
    jobPostingInfo: "Vacature informatie",
    conductGuidelines: "Gedragsrichtlijnen",
    conductGuidelinesText: "Houd een respectvolle en constructieve omgeving",
    createJobOffer: "Vacature Aanmaken",

    // Chat Rules
    chatRules: "Chat Regels",
    chatRule1: "Respecteer Alle Deelnemers",
    chatRule2: "Houd Gesprekken Relevant",
    chatRule3: "Deel Geen Persoonlijke Informatie",
    chatRule4: "Gebruik Geschikte Taal",
    chatRule5: "Geen Spam",
    chatRule6: "Wees Constructief In Je Reacties",
    chatRule7: "Meld Ongepaste Inhoud",
    chatRule8: "Geniet En Leer",
    chatRulesFooter: "Het Volgen Van Deze Regels Creëert Een Positieve Gemeenschap",
    chatRulesIntro: "Om Een Positieve Omgeving Te Behouden, Volg Deze Regels:",
    chatRulesTitle: "Gemeenschapsregels",

    // Chat Recommendations
    recommendations: "Aanbevelingen",
    popularDiscussions: "Populaire Discussies",
    joinActiveConversations: "Sluit Je Aan Bij Actieve Gesprekken",
    newProjects: "Nieuwe Projecten",
    discoverLatestInitiatives: "Ontdek De Nieuwste Initiatieven",
    eventsNearYou: "Evenementen Bij Je In De Buurt",
    findLocalEvents: "Vind Lokale Evenementen",
  },
};




// Helper functions for dynamic content
export function locationLabel(original: string, locale: Locale) {
  if (locale === "es") return original;

  const locationMap: Record<string, Record<Locale, string>> = {
    "Berlín": { en: "Berlin", de: "Berlin", es: "Berlín", fr: "Berlin", it: "Berlino", pl: "Berlin", nl: "Berlijn" },
    "Madrid": { en: "Madrid", de: "Madrid", es: "Madrid", fr: "Madrid", it: "Madrid", pl: "Madryt", nl: "Madrid" },
    "París": { en: "Paris", de: "Paris", es: "París", fr: "Paris", it: "Parigi", pl: "Paryż", nl: "Parijs" },
    "Alemania": { en: "Germany", de: "Deutschland", es: "Alemania", fr: "Allemagne", it: "Germania", pl: "Niemcy", nl: "Duitsland" },
    "España": { en: "Spain", de: "Spanien", es: "España", fr: "Espagne", it: "Spagna", pl: "Hiszpania", nl: "Spanje" },
    "Francia": { en: "France", de: "Frankreich", es: "Francia", fr: "France", it: "Francia", pl: "Francja", nl: "Frankrijk" },
  };

  return locationMap[original]?.[locale] || original;
}

export function categoryLabel(category: string, locale: Locale) {
  const categoryMap: Record<string, Record<Locale, string>> = {
    "Medio ambiente": { en: "Environment", de: "Umwelt", es: "Medio ambiente", fr: "Environnement", it: "Ambiente", pl: "Środowisko", nl: "Milieu" },
    "Educación": { en: "Education", de: "Bildung", es: "Educación", fr: "Éducation", it: "Educazione", pl: "Edukacja", nl: "Onderwijs" },
    "Comunidad": { en: "Community", de: "Gemeinschaft", es: "Comunidad", fr: "Communauté", it: "Comunità", pl: "Społeczność", nl: "Gemeenschap" },
    "Salud": { en: "Health", de: "Gesundheit", es: "Salud", fr: "Santé", it: "Salute", pl: "Zdrowie", nl: "Gezondheid" },
    "Océanos": { en: "Oceans", de: "Ozeane", es: "Océanos", fr: "Océans", it: "Oceani", pl: "Oceany", nl: "Oceanen" },
    "Alimentación": { en: "Food", de: "Ernährung", es: "Alimentación", fr: "Alimentation", it: "Alimentazione", pl: "Żywność", nl: "Voeding" },
    "Tecnología": { en: "Technology", de: "Technologie", es: "Tecnología", fr: "Technologie", it: "Tecnologia", pl: "Technologia", nl: "Technologie" },
  };

  // 1. Try exact match
  if (categoryMap[category]?.[locale]) {
    return categoryMap[category][locale];
  }

  // 2. Try case-insensitive match
  const lowerCat = category.toLowerCase();
  const foundKey = Object.keys(categoryMap).find(k => k.toLowerCase() === lowerCat);
  if (foundKey && categoryMap[foundKey]?.[locale]) {
    return categoryMap[foundKey][locale];
  }

  // 3. Fallback: Capitalize first letter if not found
  if (locale === "es" || !categoryMap[category]) {
    return category.charAt(0).toUpperCase() + category.slice(1);
  }

  return category;
}

export function projectNameLabel(id: string, name: string, locale: Locale) {
  if (locale === "es") return name;

  // For now, return the original name
  // In the future, this could be enhanced with a database lookup
  return name;
}

export function projectDescriptionLabel(projectId: string, original: string, locale: Locale) {
  if (!original) return original;
  if (locale === "es") return original;

  // Lightweight auto-translation fallback for common project phrasing
  const ES_EN: Record<string, string> = {
    "Huertos": "Gardens",
    "urbanos": "urban",
    "Jardín": "Garden",
    "comunitario": "community",
    "comunitarios": "community",
    "barrio": "neighborhood",
    "barriales": "neighborhood",
    "alimentos": "food",
    "sostenible": "sustainable",
    "sostenibles": "sustainable",
    "educación": "education",
    "ambiental": "environmental",
    "reciclaje": "recycling",
    "reutilización": "reuse",
    "economía circular": "circular economy",
    "plantación": "planting",
    "árboles": "trees",
  };
  const ES_DE: Record<string, string> = {
    "Huertos": "Gärten",
    "urbanos": "urban",
    "Jardín": "Garten",
    "comunitario": "gemeinschaftlich",
    "comunitarios": "gemeinschaftlich",
    "barrio": "Stadtteil",
    "barriales": "Stadtteil",
    "alimentos": "Lebensmittel",
    "sostenible": "nachhaltig",
    "sostenibles": "nachhaltig",
    "educación": "Bildung",
    "ambiental": "Umwelt",
    "reciclaje": "Recycling",
    "reutilización": "Wiederverwendung",
    "economía circular": "Kreislaufwirtschaft",
    "plantación": "Pflanzung",
    "árboles": "Bäume",
  };
  const dict = locale === "en" ? ES_EN : ES_DE;
  const entries = Object.entries(dict).sort((a, b) => b[0].length - a[0].length);
  let out = original;
  // Nuclear obfuscation for Tailwind parser
  // [ = 91, \ = 92, ] = 93, | = 124, etc.
  const escapeRe = new RegExp("[" + String.fromCharCode(45, 47, 92, 94, 36, 42, 43, 63, 46, 40, 41, 124, 91, 93, 123, 125) + "]", "g");

  for (const [es, tr] of entries) {
    const esEscaped = es.replace(escapeRe, '\\$&');
    const re = new RegExp(esEscaped, "gi");
    out = out.replace(re, (m) => {
      const isCap = m[0] === m[0].toUpperCase();
      const rep = tr;
      return isCap ? rep.charAt(0).toUpperCase() + rep.slice(1) : rep;
    });
  }
  return out;
}

export function impactTagLabel(original: string, locale: Locale) {
  if (locale === "es") return original;

  const impactMap: Record<string, Record<Locale, string>> = {
    "Alto impacto": { en: "High impact", de: "Hohe Wirkung", es: "Alto impacto", fr: "Impact élevé", it: "Impatto alto", pl: "Wysoki wpływ", nl: "Hoge impact" },
    "Medio impacto": { en: "Medium impact", de: "Mittlere Wirkung", es: "Medio impacto", fr: "Impact moyen", it: "Impatto medio", pl: "Średni wpływ", nl: "Gemiddelde impact" },
    "Bajo impacto": { en: "Low impact", de: "Niedrige Wirkung", es: "Bajo impacto", fr: "Impact faible", it: "Impatto basso", pl: "Niski wpływ", nl: "Lage impact" },
    "Huertos urbanos": { en: "Urban gardens", de: "Städtische Gärten", es: "Huertos urbanos", fr: "Jardins urbains", it: "Orti urbani", pl: "Ogrody miejskie", nl: "Stedelijke tuinen" },
    "Comunidad": { en: "Community", de: "Gemeinschaft", es: "Comunidad", fr: "Communauté", it: "Comunità", pl: "Społeczność", nl: "Gemeenschap" },
    "Medio ambiente": { en: "Environment", de: "Umwelt", es: "Medio ambiente", fr: "Environnement", it: "Ambiente", pl: "Środowisko", nl: "Milieu" },
    "Educación": { en: "Education", de: "Bildung", es: "Educación", fr: "Éducation", it: "Educazione", pl: "Edukacja", nl: "Onderwijs" },
    "Salud": { en: "Health", de: "Gesundheit", es: "Salud", fr: "Santé", it: "Salute", pl: "Zdrowie", nl: "Gezondheid" },
    "Océanos": { en: "Oceans", de: "Ozeane", es: "Océanos", fr: "Océans", it: "Oceani", pl: "Oceany", nl: "Oceanen" },
    "Alimentación": { en: "Food", de: "Nahrung", es: "Alimentación", fr: "Alimentation", it: "Alimentazione", pl: "Żywność", nl: "Voedsel" },
    "Integración": { en: "Integration", de: "Integration", es: "Integración", fr: "Intégration", it: "Integrazione", pl: "Integracja", nl: "Integratie" },
    "Cultura": { en: "Culture", de: "Kultur", es: "Cultura", fr: "Culture", it: "Cultura", pl: "Kultura", nl: "Cultuur" },
    // Impact tags
    "Reforestación": { en: "Reforestation", de: "Aufforstung", es: "Reforestación", fr: "Reforestation", it: "Rimboschimento", pl: "Rekultywacja", nl: "Herbebossing" },
    "Calidad del aire": { en: "Air quality", de: "Luftqualität", es: "Calidad del aire", fr: "Qualité de l'air", it: "Qualità dell'aria", pl: "Jakość powietrza", nl: "Luchtkwaliteit" },
    "STEM": { en: "STEM", de: "MINT", es: "STEM", fr: "STEM", it: "STEM", pl: "STEM", nl: "STEM" },
    "Inclusión": { en: "Inclusion", de: "Inklusion", es: "Inclusión", fr: "Inclusion", it: "Inclusione", pl: "Włączenie", nl: "Inclusie" },
    "Prevención": { en: "Prevention", de: "Prävention", es: "Prevención", fr: "Prévention", it: "Prevenzione", pl: "Zapobieganie", nl: "Preventie" },
    "Acceso": { en: "Access", de: "Zugang", es: "Acceso", fr: "Accès", it: "Accesso", pl: "Dostęp", nl: "Toegang" },
    "Playas limpias": { en: "Clean beaches", de: "Saubere Strände", es: "Playas limpias", fr: "Plages propres", it: "Spiagge pulite", pl: "Czyste plaże", nl: "Schone stranden" },
    "Biodiversidad": { en: "Biodiversity", de: "Biodiversität", es: "Biodiversidad", fr: "Biodiversité", it: "Biodiversità", pl: "Różnorodność biologiczna", nl: "Biodiversiteit" },
    "Conservación marina": { en: "Marine conservation", de: "Meeresschutz", es: "Conservación marina", fr: "Conservation marine", it: "Conservazione marina", pl: "Ochrona morska", nl: "Mariene bescherming" },
    "Agricultura sostenible": { en: "Sustainable agriculture", de: "Nachhaltige Landwirtschaft", es: "Agricultura sostenible", fr: "Agriculture durable", it: "Agricoltura sostenibile", pl: "Zrównoważone rolnictwo", nl: "Duurzame landbouw" },
    "Seguridad alimentaria": { en: "Food security", de: "Ernährungssicherheit", es: "Seguridad alimentaria", fr: "Sécurité alimentaire", it: "Sicurezza alimentare", pl: "Bezpieczeństwo żywnościowe", nl: "Voedselzekerheid" },
  };

  if (impactMap[original]?.[locale]) return impactMap[original][locale];

  // Humanize fallback if unknown tag comes in camelCase or with separators
  const spaced = original
    .replace(/([a-z])([A-Z])/g, "$1 $2")
    .replace(/[-_.]/g, " ")
    .replace(/\s+/g, " ")
    .trim();
  return spaced.charAt(0).toUpperCase() + spaced.slice(1);
}

// Event translation dictionaries and helper
const EVENT_PHRASES_EN: Record<string, string> = {
  "Taller": "Workshop",
  "Plantación": "Planting",
  "árboles": "trees",
  "Limpieza": "Cleanup",
  "de": "of",
  "río": "river",
  "playas": "beaches",
  "Mercado": "Market",
  "productos": "products",
  "locales": "local",
  "energía": "energy",
  "solar": "solar",
  "hidroeléctrica": "hydropower",
  "Reciclaje": "Recycling",
  "Huertos": "Gardens",
  "urbanos": "urban",
  "Conferencia": "Conference",
  "cambio climático": "climate change",
  "Monitoreo": "Monitoring",
  "calidad del aire": "air quality",
  "Fiesta": "Festival",
  "sostenibilidad": "sustainability",
  // Common requirements/benefits
  "Bolsa reutilizable": "Reusable bag",
  "Dinero para compras": "Money for purchases",
  "Descuentos especiales": "Special discounts",
  "Degustaciones": "Tastings",
  "Red de productores": "Producers network",
  "Entrada gratuita": "Free entry",
  "Registro previo": "Prior registration",
  "Guía marina": "Marine guide",
  "Certificado": "Certificate",
  "Material educativo": "Educational material",
};

const EVENT_PHRASES_DE: Record<string, string> = {
  "Taller": "Workshop",
  "Plantación": "Aufforstung",
  "árboles": "Bäume",
  "Limpieza": "Reinigung",
  "de": "von",
  "río": "Fluss",
  "playas": "Strände",
  "Mercado": "Markt",
  "productos": "Produkte",
  "locales": "lokal",
  "energía": "Energie",
  "solar": "Solar",
  "hidroeléctrica": "Wasserkraft",
  "Reciclaje": "Recycling",
  "Huertos": "Gärten",
  "urbanos": "urban",
  "Conferencia": "Konferenz",
  "cambio climático": "Klimawandel",
  "Monitoreo": "Überwachung",
  "calidad del aire": "Luftqualität",
  "Fiesta": "Fest",
  "sostenibilidad": "Nachhaltigkeit",
  // Common requirements/benefits
  "Bolsa reutilizable": "Wiederverwendbare Tasche",
  "Dinero para compras": "Geld für Einkäufe",
  "Descuentos especiales": "Sonderrabatte",
  "Degustaciones": "Verkostungen",
  "Red de productores": "Netz der Produzenten",
  "Entrada gratuita": "Freier Eintritt",
  "Registro previo": "Vorherige Anmeldung",
  "Guía marina": "Meeresführer",
  "Certificado": "Zertifikat",
  "Material educativo": "Bildungsmaterial",
  // Specific translations for e16
  "conservación de alimentos": "Lebensmittelkonservierung",
  "conservar alimentos": "Lebensmittel konservieren",
  "reducir el desperdicio": "Abfall reduzieren",
  "técnicas tradicionales": "traditionelle Techniken",
  "técnicas modernas": "moderne Techniken",
  "Delantal": "Schürze",
  "Cuaderno": "Notizbuch",
  "Contenedores": "Behälter",
  "Alimentos conservados": "Konservierte Lebensmittel",
  "Recetas": "Rezepte",
  // Specific translations for e16b
  "compost en casa": "Kompostieren zu Hause",
  "compostar en casa": "zu Hause kompostieren",
  "forma sencilla": "einfache Weise",
  "forma segura": "sichere Weise",
  "principiantes": "Anfänger",
  "familias": "Familien",
  "Residuo orgánico limpio": "Saubere organische Abfälle",
  "Kit de compostaje": "Kompostierungs-Kit",
  "Manual práctico": "Praktisches Handbuch",
  "Soporte online": "Online-Support",
};

export function autoTranslateEventText(text: string, locale: string): string {
  if (!text) return text;
  if (locale === "es") return text;
  const dict = locale === "en" ? EVENT_PHRASES_EN : EVENT_PHRASES_DE;
  // Replace longer phrases first
  const entries = Object.entries(dict).sort((a, b) => b[0].length - a[0].length);
  let out = text;

  // Use hex escapes to hide special chars from Tailwind parser
  const escapeChars = "-\/\\^$*+?.()\x7C\x5B\x5D{}";
  const escapePattern = "\x5B" + escapeChars.replace(new RegExp("\x5B" + escapeChars + "\x5D", "g"), "\\$&") + "\x5D";

  for (const [es, tr] of entries) {
    // word boundaries break on accents/compound words; do a safer global replace
    const esEscaped = es.replace(new RegExp(escapePattern, "g"), "\\$&");
    const re = new RegExp(esEscaped, "gi");
    out = out.replace(re, (m) => {
      const isCap = m[0] === m[0].toUpperCase();
      const rep = tr;
      return isCap ? rep.charAt(0).toUpperCase() + rep.slice(1) : rep;
    });
  }
  return out;
}
