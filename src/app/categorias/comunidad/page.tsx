...
export default function ComunidadPage() {
  ...
  const projects = categoryProjects['Comunidad'];
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gls-secondary opacity-90 max-w-3xl mx-auto">
            {locale === 'es' 
              ? 'Proyectos que fortalecen los lazos comunitarios, promueven la inclusión social y construyen comunidades más cohesionadas.'
              : locale === 'de'
              ? 'Projekte, die Gemeinschaftsbindungen stärken, soziale Inklusion fördern und zusammenhängendere Gemeinschaften aufbauen.'
              : 'Projects that strengthen community bonds, promote social inclusion and build more cohesive communities.'
            }
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project) => (
            <div key={project.id} className="bg-gls-secondary rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow">
              {/* Project Image */}
              <div className="h-48">
                <CategoryImage category="comunidad" className="w-full h-full" />
              </div>
